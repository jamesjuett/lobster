...
function getObjectString(obj: CPPObject) {
  ...
  if (name.startsWith('[')) {
    ...
    } else if (obj.isTyped(isCompleteClassType) && obj.type.className === 'string') {
      // TODO make this robust to check for the actual string, not just something named string.
      return htmlDecoratedObject(
        getValueString((<CPPObject<PointerType<Char>>>obj.getMemberObject('data_ptr')).getValue())
      );
    } else {
      return htmlDecoratedObject('');
    }
...
export class FunctionOutlet extends ConstructOutlet<RuntimeFunction> {
  ...
  public constructor(element: JQuery, rtFunc: RuntimeFunction, listener?: ObserverType) {
    ...
    if (!this.construct.declaration.isConstructor) {
      var returnTypeElem = $(
        '<span class="code-returnType">' + this.construct.type.returnType.toString() + '</span>'
      );
...
export class ParameterOutlet {
  ...
  public constructor(element: JQuery, paramDef: CompiledParameterDeclaration) {
    ...
    this.element.append(
      paramDef.type.typeString(false, htmlDecoratedName(paramDef.name || '', paramDef.type), true)
    );
  }
...
export class DeclarationStatementOutlet extends StatementOutlet<RuntimeDeclarationStatement> {
  ...
  public constructor(
    ...
    declarations.forEach((declaration, i) => {
      ...
      declElem.append(
        declaration.type.declaratorString(htmlDecoratedName(declaration.name, declaration.type))
      );
...
export class AtomicDefaultInitializerOutlet extends InitializerOutlet<
  RuntimeAtomicDefaultInitializer
> {
  // Nothing to add based on being atomic
}

export class ArrayDefaultInitializerOutlet extends InitializerOutlet<
  RuntimeArrayDefaultInitializer
> {
  ...
  public constructor(
    ...
    if (this.construct.elementInitializers) {
      this.elementInitializerOutlets = this.construct.elementInitializers.map(elemInit =>
...
export class ClassDefaultInitializerOutlet extends InitializerOutlet<
  RuntimeClassDefaultInitializer
> {
...
export class AtomicDirectInitializerOutlet extends InitializerOutlet<
  RuntimeAtomicDirectInitializer
> {
...
export class ReferenceDirectInitializerOutlet extends InitializerOutlet<
  RuntimeReferenceDirectInitializer
> {
  public readonly argOutlet: ExpressionOutlet;
...
export class ReferenceCopyInitializerOutlet extends InitializerOutlet<
  RuntimeReferenceDirectInitializer
> {
  public readonly argOutlet: ExpressionOutlet;
...
export class ArrayAggregateInitializerOutlet extends InitializerOutlet<
  RuntimeArrayAggregateInitializer
> {
  ...
  public constructor(
    ...
    this.elemInitializerOutlets = construct.elemInitializers.map((elemInit, i) => {
      ...
      return <DirectInitializerOutlet | ValueInitializerOutlet>(
        createInitializerOutlet($('<span></span>').appendTo(this.element), elemInit, this)
      );
    });
  }
}
...
> extends PotentialFullExpressionOutlet<RT> {
  ...
  public constructor(
    ...
    this.element.append(
      "<span class='exprType'>" + encode(this.construct.type.toString()) + '</span>'
    );
  }
...
export class CompoundAssignmentExpressionOutlet extends ExpressionOutlet<
  RuntimeCompoundAssignmentExpression
> {
  public readonly lhs: ExpressionOutlet;
  public readonly rhs: ExpressionOutlet;
  ...
  public constructor(
    ...
    this.exprElem.append(
      ' ' + htmlDecoratedOperator(this.construct.operator, 'code-compoundAssignmentOp') + ' '
    );
...
export class FunctionCallExpressionOutlet extends ExpressionOutlet<RuntimeFunctionCallExpression>
  ...
  public constructor(
    ...
    this.operandOutlet = createExpressionOutlet(
      $('<span class="functionCall-operand"></span>').appendTo(this.exprElem),
      construct.operand,
      this
    );
...
export class MagicFunctionCallExpressionOutlet extends ExpressionOutlet<
  RuntimeMagicFunctionCallExpression
> {
...
export class OutputOperatorExpressionOutlet extends ExpressionOutlet<
  RuntimeOutputOperatorExpression
> {
  public readonly left: ExpressionOutlet;
  public readonly right: ExpressionOutlet;
...
export class InputOperatorExpressionOutlet extends ExpressionOutlet<
  RuntimeInputOperatorExpression
> {
  public readonly left: ExpressionOutlet;
  public readonly right: ExpressionOutlet;
...
export class UnaryOperatorExpressionOutlet extends ExpressionOutlet<
  RuntimeUnaryOperatorExpression
> {
  ...
  public constructor(
    ...
    if (this.construct.individualElementInitializers.length > 0) {
      ...
      this.individualElementInitializerOutlets = this.construct.individualElementInitializers.map(
        elemInit =>
          createInitializerOutlet($('<span></span>').appendTo(this.exprElem), elemInit, this)
      );
      this.exprElem.append(' }');
    } else {
...
export class PostfixIncrementExpressionOutlet extends ExpressionOutlet<
  RuntimePostfixIncrementExpression
> {
  public readonly operand: ExpressionOutlet;
...
> {
  ...
  public constructor(
    element: JQuery,
    construct: CompiledObjectDotExpression<CompleteObjectType> | CompiledFunctionDotExpression,
    parent?: ConstructOutlet
  ) {
...
> {
  ...
  public constructor(
    element: JQuery,
    construct: CompiledObjectArrowExpression<CompleteObjectType> | CompiledFunctionArrowExpression,
    parent?: ConstructOutlet
  ) {
    super(element, construct, parent, false);
    this.operand = addChildExpressionOutlet(this.exprElem, this.construct.operand, this);
...
export class StringLiteralExpressionOutlet extends ExpressionOutlet<
  RuntimeStringLiteralExpression
> {
  public constructor(
    element: JQuery,
