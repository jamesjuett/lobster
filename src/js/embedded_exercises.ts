...
$(() => {
    ...
    $(".lobster-ex").each(function () {
        ...
        let filename =
            $(this).find(".lobster-ex-file-name").html()?.trim() ?? "file.cpp";
        let exKey =
            $(this).find(".lobster-ex-key").html()?.trim() ??
            $(this).find(".lobster-ex-project-name").html()?.trim() ??
            "UnnamedProject";

        let exerciseSpec = getExerciseSpecification(exKey) ?? DEFAULT_EXERCISE;

        let completionMessage =
            $(this).find(".lobster-ex-completion-message").html()?.trim() ??
            $(this).find(".lobster-ex-complete-message").html()?.trim();
        if (completionMessage) {
            exerciseSpec.completionMessage = completionMessage;
        }
        let initCode = decode(
            $(this).find(".lobster-ex-starter-code").html()?.trim() ??
                $(this).find(".lobster-ex-init-code").html()?.trim() ??
                ""
        );
        if (initCode) {
