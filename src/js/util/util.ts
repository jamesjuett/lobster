...
export function createMethodMixin<TargetType, MethodName extends keyof TargetType>(
  mix: TargetType[MethodName]
) {
...
export function htmlDecoratedName(name: string, type?: Type) {
  if (type) {
    return (
      '<span class="code-name"><span class = "lobster-highlight"></span><span class="type">' +
      encode(type.englishString(false)) +
      '</span>' +
      name +
      '</span>'
    );
    ...
    return (
      '<span class="code-name"><span class = "lobster-highlight"></span>' + encode(name) + '</span>'
    );
  }
}
