...
export abstract class TestVerifier {
    ...
    protected abstract verifyImpl(
        program: Program
    ): Omit<VerificationStatus, "verifierName">;
}
    ...
    protected verifyImpl(
        ...
        if (!program.notes.hasErrors && !program.notes.hasWarnings) {
            ...
            return {
                status: "failure",
                message:
                    "There were errors or warnings, but there shouldn't have been.",
            };
        }
    }
}
...
export class NoteVerifier extends TestVerifier {
    ...
    protected verifyImpl(
        ...
        for (let id in notesMap) {
            notesMap[id].forEach((line) =>
                extraNotes.push({ line: line, id: id })
            );
        }
    ...
    protected verifyImpl(
        ...
        if (!program.isRunnable()) {
            return {
                status: "failure",
                message:
                    "The program either failed to compile or is missing a main function.",
            };
        }
        ...
        if (sim.hasEventOccurred(SimulationEvent.ASSERTION_FAILURE)) {
            return {
                status: "failure",
                message: "An assertion in the program failed when run.",
            };
        } else {
            return VERIFICATION_SUCCESSFUL;
        }
    ...
    protected verifyImpl(
        ...
        if (sim.allOutput === this.expectedOutput) {
            ...
            return {
                status: "failure",
                message:
                    "The program's output did not match what was expected.",
            };
        }
    }
}
    ...
    protected verifyImpl(
        ...
        if (this.criteria(sim)) {
            ...
            return {
                status: "failure",
                message:
                    "The programs end-of-main state did not match what was expected.",
            };
        }
    }
}
        ...
        for (let i = 0; i < eventsToCheck.length; ++i) {
            ...
            if (sim.hasEventOccurred(event)) {
                return {
                    status: "failure",
                    message:
                        "An unexpected runtime event (" + event + ") occurred.",
                };
            }
        }
...
function checkState(sim1: Simulation, sim2: Simulation) {
    if (sim1.printState() !== sim2.printState()) {
        throw {
            status: "failure",
            message: "The program's state was not what was expected.",
        };
    }
}
...
export class ProgramTest {
    ...
    public constructor(
        ...
        assert(
            reporter !== undefined,
            "Individual reporter or default reporter must be specified."
        );
        ...
        this.program = new Program(
            sourceFiles,
            new Set<string>(translationUnits)
        );
...
export class SingleTranslationUnitTest extends ProgramTest {
    public constructor(
        ...
        super(
            name,
            [new SourceFile("test.cpp", sourceText)],
            ["test.cpp"],
            verifiers
        );
    }
