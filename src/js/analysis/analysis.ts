...
function analyze2(program: Program) {
  ...
  let localDefs = findConstructs(
    program,
    Predicates.byKinds(['local_variable_definition', 'global_variable_definition'])
  );

  // 2. Narrow those down to only the ones that define pointer variables
  let pointerDefs = findConstructs(program, Predicates.byTypedDeclaration(isPointerType)).filter(
    Predicates.byKind('local_variable_definition')
  );
