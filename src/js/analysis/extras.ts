...
const EXTRAS : {[index: string]: readonly StaticAnalysisExtra[] | undefined} = {
    ...
    "loop_hardcoded_condition": [(program: Program) => {
        ...
        if (hardcodedLimit) {
            program.addNote(new CompilerNote(loop.condition, NoteKind.STYLE, "heuristic_loop_hardcoded_size",
    ...
    "loop_condition_vector_off_by_one": [(program: Program) => {
        let loop = findFirstConstruct(program, Predicates.isLoop);
        if (!loop) {
            return false;
        }
                ...
                if (conditionOperator.operator === "<=" || conditionOperator.operator === ">=") {
                    if (!indexingOperations.some(indexingOp => findFirstConstruct(indexingOp,
                        Predicates.byKinds([
                            ...
                            program.addNote(new CompilerNote(conditionOperator, NoteKind.STYLE, "loop_condition_vector_off_by_one",
