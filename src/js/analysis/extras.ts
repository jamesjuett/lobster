...
const EXTRAS: { [index: string]: readonly StaticAnalysisExtra[] | undefined } =
    {
        ...
        loop_condition_vector_off_by_one: [
            (program: Program) => {
                ...
                let indexingOperations = findConstructs(
                    loop.body,
                    Predicates.isIndexingOperation
                );
        ...
        eecs280_ex_swap_by_pointer: [
            (program: Program) => {
                let swapPtdInts = findConstructs(
                    program,
                    constructTest(FunctionDefinition)
                ).find((def) => def.declaration.name === "swap");
                let main = findConstructs(
                    program,
                    constructTest(FunctionDefinition)
                ...
                let assignments = findConstructs(
                    swapPtdInts,
                    Predicates.byKind("assignment_expression")
                );
                let pointerAssignments = assignments.filter(
                ...
                let pointerDefs = localDefs.filter(
                    Predicates.byTypedDeclaration(isPointerType)
                );
                let intDefs = localDefs.filter(
                    Predicates.byTypedDeclaration(isType(Int))
                );

                let intParams = swapPtdInts.parameters.filter(
                    Predicates.byTypedDeclaration(isType(Int))
                );
