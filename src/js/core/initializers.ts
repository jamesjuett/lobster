...
import {
    ...
    NewArrayEntity,
    DynamicLengthArrayNextElementEntity,
...
import {
    ...
    ReferenceCopyInitializerOutlet,
    AtomicCopyInitializerOutlet,
    ClassDefaultInitializerOutlet,
    ClassDirectInitializerOutlet,
    ClassCopyInitializerOutlet,
    CtorInitializerOutlet,
    ArrayAggregateInitializerOutlet,
    ArrayMemberInitializerOutlet,
...
export interface CompiledInitializer<
    T extends ObjectEntityType = ObjectEntityType
> extends Initializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target:
        | ObjectEntity<Exclude<T, ReferenceType>>
        | UnboundReferenceEntity<Extract<T, ReferenceType>>;
}

export abstract class RuntimeInitializer<
    C extends CompiledInitializer = CompiledInitializer
> extends RuntimePotentialFullExpression<C> {
...
export abstract class DefaultInitializer extends Initializer {
    public static create(
        ...
        target: UnboundReferenceEntity
    ): ReferenceDefaultInitializer;
    public static create(
        context: TranslationUnitContext,
        target: ObjectEntity<AtomicType>
    ...
    public static create(
        context: TranslationUnitContext,
        target: ObjectEntity<CompleteObjectType>
    ):
        | AtomicDefaultInitializer
        | ArrayDefaultInitializer
        | ClassDefaultInitializer;
    public static create(
        context: TranslationUnitContext,
        target: ObjectEntity | UnboundReferenceEntity
    ):
        | ReferenceDefaultInitializer
        | AtomicDefaultInitializer
        | ArrayDefaultInitializer
    ...
    public static create(
        ...
        if (target.type.isReferenceType()) {
            return new ReferenceDefaultInitializer(
                context,
                <UnboundReferenceEntity>target
            );
        } else if (target.type.isAtomicType()) {
            return new AtomicDefaultInitializer(
                context,
                <ObjectEntity<AtomicType>>target
            );
        } else if (target.type.isBoundedArrayType()) {
            return new ArrayDefaultInitializer(
                context,
                <ObjectEntity<BoundedArrayType>>target
            );
        } else if (target.type.isCompleteClassType()) {
            return new ClassDefaultInitializer(
                context,
                <ObjectEntity<CompleteClassType>>target
            );
        } else {
    ...
    public abstract createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledDefaultInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeDefaultInitializer<T>;
}

export interface CompiledDefaultInitializer<
...
export abstract class RuntimeDefaultInitializer<
    T extends CompleteObjectType = CompleteObjectType,
    C extends CompiledDefaultInitializer<T> = CompiledDefaultInitializer<T>
> extends RuntimeInitializer<C> {
    protected constructor(model: C, parent: RuntimeConstruct) {
        super(model, parent);
    }
}
...
export class ReferenceDefaultInitializer extends DefaultInitializer {
    ...
    public createRuntimeInitializer(parent: RuntimeConstruct): never {
        return assertFalse(
            "A default initializer for a reference is not allowed."
        );
    }

    public createDefaultOutlet(element: JQuery, parent?: ConstructOutlet) {
        return assertFalse(
            "Cannot create an outlet for a reference default initializer, since such an initializer is always ill-formed."
        );
    }
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        return {
            message:
                "As a reference, " +
                (targetDesc.name || targetDesc.message) +
                " must be bound to something. (It cannot be left un-initialized.)",
        };
    }
}
...
export class AtomicDefaultInitializer extends DefaultInitializer {
    ...
    public createRuntimeInitializer<T extends AtomicType>(
        this: CompiledAtomicDefaultInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeAtomicDefaultInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledDefaultInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    public createRuntimeInitializer<T extends AtomicType>(
    ...
    public createDefaultOutlet(
        this: CompiledAtomicDefaultInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): AtomicDefaultInitializerOutlet {
...
export interface CompiledAtomicDefaultInitializer<
    T extends AtomicType = AtomicType
> extends AtomicDefaultInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target: ObjectEntity<T>;
}

export class RuntimeAtomicDefaultInitializer<
    T extends AtomicType = AtomicType
> extends RuntimeDefaultInitializer<T, CompiledAtomicDefaultInitializer<T>> {
    public constructor(
        model: CompiledAtomicDefaultInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
    }
...
export class ArrayDefaultInitializer extends DefaultInitializer {
    ...
    public constructor(
        ...
        if (type.elemType instanceof AtomicType) {
            ...
            for (let i = 0; i < type.numElems; ++i) {
                let elemInit = DefaultInitializer.create(
                    context,
                    new ArraySubobjectEntity(this.target, i)
                );
                this.elementInitializers.push(elemInit);
                this.attach(elemInit);
                if (elemInit.notes.hasErrors) {
                    this.addNote(
                        CPPError.declaration.init.array_default_init(this)
                    );
                    break;
                }
            }
    ...
    public createRuntimeInitializer<T extends BoundedArrayType>(
        this: CompiledArrayDefaultInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeArrayDefaultInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledDefaultInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public createDefaultOutlet(
        this: CompiledArrayDefaultInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): ArrayDefaultInitializerOutlet {
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        if (targetType.numElems === 0) {
            return {
                message:
                    "No initialization is performed for " +
                    (targetDesc.name || targetDesc.message) +
                    "because the array has length 0.",
            };
        } else if (targetType.elemType instanceof AtomicType) {
            return {
                message:
                    "No initialization will take place. The elements of " +
                    (targetDesc.name || targetDesc.message) +
            ...
            return {
                message:
                    "Each element of " +
                    (targetDesc.name || targetDesc.message) +
                    " will be default-initialized. For example, " +
...
export interface CompiledArrayDefaultInitializer<
    T extends BoundedArrayType = BoundedArrayType
> extends ArrayDefaultInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target: ObjectEntity<T>;
...
export class RuntimeArrayDefaultInitializer<
    T extends BoundedArrayType = BoundedArrayType
> extends RuntimeDefaultInitializer<T, CompiledArrayDefaultInitializer<T>> {
    ...
    public constructor(
        model: CompiledArrayDefaultInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        if (this.model.elementInitializers) {
            this.elementInitializers = this.model.elementInitializers.map(
                (elemInit) => {
    ...
    protected upNextImpl() {
        if (
            this.elementInitializers &&
            this.index < this.elementInitializers.length
        ) {
...
export class ClassDefaultInitializer extends DefaultInitializer {
    ...
    public constructor(
        ...
        let overloadResult = overloadResolution(
            target.type.classDefinition.constructors,
            []
        );
        if (!overloadResult.selected) {
            this.addNote(
                CPPError.declaration.init.no_default_constructor(
                    this,
                    this.target
                )
            );
        ...
        this.ctorCall = new FunctionCall(
            context,
            this.ctor,
            [],
            target.type.cvUnqualified()
        );
        this.attach(this.ctorCall);
        // this.args = this.ctorCall.args;
    }

    public createRuntimeInitializer<T extends CompleteClassType>(
        this: CompiledClassDefaultInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeClassDefaultInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledDefaultInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public createDefaultOutlet(
        this: CompiledClassDefaultInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): ClassDefaultInitializerOutlet {
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        return {
            message:
                ...
                this.ctorCall!.describe(sim, rtConstruct).message,
        };
    }
}

export interface CompiledClassDefaultInitializer<
    T extends CompleteClassType = CompleteClassType
> extends ClassDefaultInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeClassDefaultInitializer<
    T extends CompleteClassType = CompleteClassType
> extends RuntimeDefaultInitializer<T, CompiledClassDefaultInitializer<T>> {
    public readonly ctorCall: RuntimeFunctionCall<FunctionType<VoidType>>;

    private index = "callCtor";

    public constructor(
        model: CompiledClassDefaultInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        this.ctorCall = this.model.ctorCall.createRuntimeFunctionCall(
            this,
            this.model.target.runtimeLookup(this)
        );
    }
...
export abstract class ValueInitializer extends Initializer {
    ...
    public static create(
        context: TranslationUnitContext,
        target: ObjectEntity<CompleteObjectType>
    ): AtomicValueInitializer | ArrayValueInitializer | ClassValueInitializer;
    public static create(
        context: TranslationUnitContext,
        target: ObjectEntity | UnboundReferenceEntity
    ):
    ...
    public static create(
        ...
        if (target.type.isReferenceType()) {
            return new ReferenceValueInitializer(
                context,
                <UnboundReferenceEntity>target
            );
        } else if (target.type.isAtomicType()) {
            return new AtomicValueInitializer(
                context,
                <ObjectEntity<AtomicType>>target
            );
        } else if (target.type.isBoundedArrayType()) {
            return new ArrayValueInitializer(
                context,
                <ObjectEntity<BoundedArrayType>>target
            );
        } else if (target.type.isCompleteClassType()) {
            return new ClassValueInitializer(
                context,
                <ObjectEntity<CompleteClassType>>target
            );
        } else {
    ...
    public abstract createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledValueInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeValueInitializer<T>;
}

export interface CompiledValueInitializer<
    T extends CompleteObjectType = CompleteObjectType
> extends ValueInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target: ObjectEntity<T>;
}

export abstract class RuntimeValueInitializer<
    T extends CompleteObjectType = CompleteObjectType,
    C extends CompiledValueInitializer<T> = CompiledValueInitializer<T>
> extends RuntimeInitializer<C> {
    protected constructor(model: C, parent: RuntimeConstruct) {
        super(model, parent);
    }
...
export class AtomicValueInitializer extends ValueInitializer {
    ...
    public createRuntimeInitializer<T extends AtomicType>(
        this: CompiledAtomicValueInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeAtomicValueInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledValueInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public createDefaultOutlet(
        this: CompiledAtomicValueInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): AtomicValueInitializerOutlet {
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        return {
            message: `${
                targetDesc.name || targetDesc.message
            } is "value-initialized" to the equivalent of a 0 value for its type.`,
        };
    }
}

export interface CompiledAtomicValueInitializer<
    T extends AtomicType = AtomicType
> extends AtomicValueInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target: ObjectEntity<T>;
}

export class RuntimeAtomicValueInitializer<
    T extends AtomicType = AtomicType
> extends RuntimeValueInitializer<T, CompiledAtomicValueInitializer<T>> {
    public constructor(
        model: CompiledAtomicValueInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
    }
...
export class ArrayValueInitializer extends ValueInitializer {
    ...
    public constructor(
        ...
        for (let i = 0; i < target.type.numElems; ++i) {
            let elemInit = ValueInitializer.create(
                context,
    ...
    public createRuntimeInitializer<T extends BoundedArrayType>(
        this: CompiledArrayValueInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeArrayValueInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledValueInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public createDefaultOutlet(
        this: CompiledArrayValueInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): ArrayValueInitializerOutlet {
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        if (targetType.numElems === 0) {
            ...
            return {
                message: `The elements of ${
                    targetDesc.name || targetDesc.message
                } will be "value-initialized" to the equivalent of a 0 value for their type.`,
            };
        } else {
            return {
                message:
                    "Each element of " +
                    (targetDesc.name || targetDesc.message) +
                    " will be value-initialized. For example, " +
                    this.elementInitializers![0].explain(sim, rtConstruct),
            };
        }
    }
...
export interface CompiledArrayValueInitializer<
    T extends BoundedArrayType = BoundedArrayType
> extends ArrayValueInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target: ObjectEntity<T>;
...
export class RuntimeArrayValueInitializer<
    T extends BoundedArrayType = BoundedArrayType
> extends RuntimeValueInitializer<T, CompiledArrayValueInitializer<T>> {
    public readonly elementInitializers?: RuntimeValueInitializer<
        T["elemType"]
    >[];
    ...
    public constructor(
        model: CompiledArrayValueInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        if (this.model.elementInitializers) {
...
export class ClassValueInitializer extends ValueInitializer {
    ...
    public createRuntimeInitializer<T extends CompleteClassType>(
        this: CompiledClassValueInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeClassValueInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledValueInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public createDefaultOutlet(
        this: CompiledClassValueInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): ClassValueInitializerOutlet {
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        if (!this.ctor) {
            return {
                message:
                    (targetDesc.name || targetDesc.message) +
                    " cannot be initialized because the " +
                    this.target.type +
            ...
            return {
                message:
                    (targetDesc.name || targetDesc.message) +
                    " will be zero-initialized, followed by initialization using " +
                    this.ctorCall!.describe(sim, rtConstruct).message,
            };
        } else {
            return {
...
export interface CompiledClassValueInitializer<
    T extends CompleteClassType = CompleteClassType
> extends ClassValueInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeClassValueInitializer<
    T extends CompleteClassType = CompleteClassType
> extends RuntimeValueInitializer<T, CompiledClassValueInitializer<T>> {
    public readonly ctorCall: RuntimeFunctionCall<FunctionType<VoidType>>;
    ...
    public constructor(
        model: CompiledClassValueInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
...
export abstract class DirectInitializer extends Initializer {
    ...
    public static create(
        ...
        if (target.type.isReferenceType()) {
            // check for presence of bindTo to detect reference entities
            return new ReferenceDirectInitializer(
                context,
                <UnboundReferenceEntity>target,
                args,
                kind
            ...
            return new AtomicDirectInitializer(
                context,
                <ObjectEntity<AtomicType>>target,
                args,
                kind
            ...
            return new ArrayDirectInitializer(
                context,
                <ObjectEntity<BoundedArrayType>>target,
                args,
                kind
            ...
            return new ClassDirectInitializer(
                context,
                <ObjectEntity<CompleteClassType>>target,
                args,
                kind
    ...
    public abstract createRuntimeInitializer<T extends ObjectEntityType>(
        this: CompiledDirectInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeDirectInitializer<T>;
}

export interface CompiledDirectInitializer<
    T extends ObjectEntityType = ObjectEntityType
> extends DirectInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target:
        | ObjectEntity<Exclude<T, ReferenceType>>
...
export abstract class RuntimeDirectInitializer<
    T extends ObjectEntityType = ObjectEntityType,
    C extends CompiledDirectInitializer<T> = CompiledDirectInitializer<T>
> extends RuntimeInitializer<C> {
    protected constructor(model: C, parent: RuntimeConstruct) {
        super(model, parent);
    }
...
export class ReferenceDirectInitializer extends DirectInitializer {
    ...
    public constructor(
        ...
        assert(
            args.length > 0,
            "Direct initialization must have at least one argument. (Otherwise it should be a default initialization.)"
        );
        ...
        if (arg.valueCategory === "lvalue") {
            if (isReferenceCompatible) {
                ...
                if (isReferenceRelated) {
                    ...
                    this.addNote(
                        CPPError.declaration.init.referenceConstness(
                            this,
                            arg.type,
                            targetType
                        )
                    ...
                    if (!targetType.refTo.isConst) {
                        ...
                        this.addNote(
                            CPPError.declaration.init.referencePrvalueConst(
                                this
                            )
                        );
                    }
            ...
            //arg.valueCategory === "prvalue"
            if (!targetType.refTo.isConst) {
                // can't make non-const reference to a prvalue
                this.addNote(
                    CPPError.declaration.init.referencePrvalueConst(this)
                );
            }
    ...
    public createRuntimeInitializer<
        T extends ReferenceType<CompleteObjectType>
    >(
        this: CompiledReferenceDirectInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeReferenceDirectInitializer<T>;
    public createRuntimeInitializer<
        T extends ReferenceType<CompleteObjectType>
    >(this: CompiledDirectInitializer<T>, parent: RuntimeConstruct): never;
    public createRuntimeInitializer<
        T extends ReferenceType<CompleteObjectType>
    >(
        this: CompiledReferenceDirectInitializer<T>,
    ...
    public createDefaultOutlet(
        this: CompiledReferenceDirectInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): ReferenceDirectInitializerOutlet {
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        return {
            message:
                (targetDesc.name || targetDesc.message) +
                " will be bound to " +
                (rhsDesc.name || rhsDesc.message) +
                ".",
        };
    }
}

export interface CompiledReferenceDirectInitializer<
    T extends ReferenceType = ReferenceType
> extends ReferenceDirectInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeReferenceDirectInitializer<
    T extends ReferenceType<CompleteObjectType> = ReferenceType<CompleteObjectType>
> extends RuntimeDirectInitializer<T, CompiledReferenceDirectInitializer<T>> {
    ...
    public constructor(
        model: CompiledReferenceDirectInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
    ...
    public stepForwardImpl() {
        this.model.target.bindTo(
            this,
            <CPPObject<ReferredType<T>>>this.arg.evalResult
        ); //TODO not sure at all why this cast is necessary
        // this.notifyPassing();
...
export class AtomicDirectInitializer extends DirectInitializer {
    ...
    public constructor(
        ...
        if (args.length > 1) {
            ...
            this.attachAll((this.args = args));
            this.addNote(
                CPPError.declaration.init.scalar_args(this, targetType)
            );
            return;
        }
    ...
    public createRuntimeInitializer<T extends AtomicType>(
        this: CompiledAtomicDirectInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeAtomicDirectInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledDirectInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    public createRuntimeInitializer<T extends AtomicType>(
        ...
        return new RuntimeAtomicDirectInitializer(
            <CompiledAtomicDirectInitializer<T>>this,
            parent
        );
    }

    public createDefaultOutlet(
        this: CompiledAtomicDirectInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): AtomicDirectInitializerOutlet {
        return this.kind === "direct"
...
export interface CompiledAtomicDirectInitializer<
    T extends AtomicType = AtomicType
> extends AtomicDirectInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeAtomicDirectInitializer<
    T extends AtomicType = AtomicType
> extends RuntimeDirectInitializer<T, CompiledAtomicDirectInitializer<T>> {
    ...
    public constructor(
        model: CompiledAtomicDirectInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        this.arg = createRuntimeExpression(this.model.arg, this);
...
export class ArrayDirectInitializer extends DirectInitializer {
    ...
    public constructor(
        ...
        let firstArg = args[0];
        if (
            targetType.elemType.isType(Char) &&
            args.length === 1 &&
            firstArg.isStringLiteralExpression()
        ) {
            this.arg = firstArg;

            if (firstArg.type.numElems > targetType.numElems) {
                this.addNote(
                    CPPError.declaration.init.stringLiteralLength(
                        this,
                        firstArg.type.numElems,
                        targetType.numElems
                    )
                );
            }
            ...
            this.addNote(
                CPPError.declaration.init.array_string_literal(this, targetType)
            );
        }
    }

    public createRuntimeInitializer(
        this: CompiledArrayDirectInitializer,
        parent: RuntimeConstruct
    ): RuntimeArrayDirectInitializer;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledDirectInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
        ...
        return {
            message:
                (targetDesc.name || targetDesc.message) +
                " (a character array) will be initialized from the string literal " +
                rhsDesc +
                ". Remember that a null character is automatically appended!",
        };
    }
}

export interface CompiledArrayDirectInitializer
    extends ArrayDirectInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeArrayDirectInitializer extends RuntimeDirectInitializer<
    BoundedArrayType<Char>,
    CompiledArrayDirectInitializer
> {
    ...
    public constructor(
        model: CompiledArrayDirectInitializer,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        this.arg = createRuntimeExpression(this.model.arg, this);
    ...
    public stepForwardImpl() {
        ...
        charsToWrite.forEach((c, i) =>
            arrayElemSubobjects[i].initializeValue(c)
        );
...
export class ClassDirectInitializer extends DirectInitializer {
    ...
    public constructor(
        ...
        if (!overloadResult.selected) {
            this.addNote(
                CPPError.declaration.init.matching_constructor(
                    this,
                    this.target,
                    argTypes
                )
    ...
    public createRuntimeInitializer<T extends CompleteClassType>(
        this: CompiledClassDirectInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeClassDirectInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledDirectInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public createDefaultOutlet(
        this: CompiledClassDirectInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): ClassDirectInitializerOutlet {
        return this.kind === "direct"
...
export interface CompiledClassDirectInitializer<
    T extends CompleteClassType = CompleteClassType
> extends ClassDirectInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeClassDirectInitializer<
    T extends CompleteClassType = CompleteClassType
> extends RuntimeDirectInitializer<T, CompiledClassDirectInitializer<T>> {
    public readonly ctorCall?: RuntimeFunctionCall<FunctionType<VoidType>>;

    private index = "callCtor";

    public constructor(
        model: CompiledClassDirectInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
    }
    ...
    protected upNextImpl() {
        if (this.index === "callCtor") {
            (<Mutable<this>>this).ctorCall =
                this.model.ctorCall.createRuntimeFunctionCall(
                    this,
                    this.model.target.runtimeLookup(this)
                );
            this.sim.push(this.ctorCall!);
...
export type MemberInitializer =
    | DefaultInitializer
    | ValueInitializer
    | DirectInitializer
    | ArrayMemberInitializer;
type BaseOrDelegateInitializer =
    | ClassDefaultInitializer
    | ClassValueInitializer
    | ClassDirectInitializer;

export class CtorInitializer extends BasicCPPConstruct<
    MemberBlockContext,
    CtorInitializerASTNode
> {
    ...
    public readonly memberInitializersByName: {
        [index: string]: MemberInitializer | undefined;
    } = {};

    public static createFromAST(
        ast: CtorInitializerASTNode,
        context: MemberBlockContext
    ) {
        return new CtorInitializer(
            context,
            ast,
            ast.initializers.map((memInitAST) => {
                ...
                let args = memInitAST.args.map((argAST) =>
                    createExpressionFromAST(argAST, context)
                );
    ...
    public constructor(
        context: MemberBlockContext,
        ast: CtorInitializerASTNode | undefined,
        components: readonly CtorInitializerComponent[]
    ) {
        super(context, ast);
        ...
        for (let i = 0; i < components.length; ++i) {
            ...
            if (comp.kind === "delegatedConstructor") {
                ...
                if (this.delegatedConstructorInitializer) {
                    delegatedCtor.addNote(
                        CPPError.declaration.ctor.init.multiple_delegates(
                            delegatedCtor
                        )
                    );
                } else {
                    ...
                    if (components.length > 1) {
                        ...
                        delegatedCtor.addNote(
                            CPPError.declaration.ctor.init.delegate_only(
                                delegatedCtor
                            )
                        );
                    }
                ...
                let baseInit =
                    comp.args.length === 0
                        ? new ClassValueInitializer(
                              context,
                              new BaseSubobjectEntity(this.target, baseType)
                          )
                        : new ClassDirectInitializer(
                              context,
                              new BaseSubobjectEntity(this.target, baseType),
                ...
                if (!this.baseInitializer) {
                    ...
                    baseInit.addNote(
                        CPPError.declaration.ctor.init.multiple_base_inits(
                            baseInit
                        )
                    );
                }
                ...
                let memEntity =
                    receiverType.classDefinition.memberVariableEntitiesByName[
                        memName
                    ];
                if (memEntity) {
                    let memInit: MemberInitializer | undefined;
                    if (
                        memEntity.isTyped(isBoundedArrayType) &&
                        comp.args.length === 1
                    ) {
                        let arg = <AnalyticExpression>comp.args[0];
                        if (
                            arg.construct_type === "dot_expression" &&
                    ...
                    if (!this.memberInitializersByName[memName]) {
                        ...
                        this.addNote(
                            CPPError.declaration.ctor.init.multiple_member_inits(
                                this
                            )
                        );
                    }
                    ...
                    this.addNote(
                        CPPError.declaration.ctor.init.improper_name(
                            this,
                            receiverType,
                            memName
                        )
        ...
        if (baseType && !this.baseInitializer) {
            this.baseInitializer = new ClassDefaultInitializer(
                createImplicitContext(context),
                new BaseSubobjectEntity(this.target, baseType)
            );
            this.attach(this.baseInitializer);
        }

        receiverType.classDefinition.memberVariableEntities.forEach(
            (memEntity) => {
    ...
    public createRuntimeCtorInitializer(
        this: CompiledCtorInitializer,
        parent: RuntimeConstruct
...
export interface CompiledCtorInitializer
    extends CtorInitializer,
        SuccessfullyCompiled {
    readonly delegatedConstructorInitializer?: CompiledClassDirectInitializer;
    readonly baseInitializer?:
        | CompiledClassDefaultInitializer
        | CompiledClassDirectInitializer;
    readonly memberInitializers: readonly (
...
export class RuntimeCtorInitializer extends RuntimeConstruct<CompiledCtorInitializer> {
    ...
    public readonly baseInitializer?:
        | RuntimeClassDefaultInitializer
        | RuntimeClassDirectInitializer;
    public readonly memberInitializers: readonly (
    ...
    public constructor(
        model: CompiledCtorInitializer,
        parent: RuntimeConstruct
    ) {
        super(model, "ctor-initializer", parent);
        this.delegatedConstructorInitializer =
            this.model.delegatedConstructorInitializer?.createRuntimeInitializer(
                this
            );

        // Dummy ternary needed by type system due to union and this parameter shenanagins
        this.baseInitializer =
            this.model.baseInitializer instanceof ClassDefaultInitializer
                ? this.model.baseInitializer?.createRuntimeInitializer(this)
                : this.model.baseInitializer?.createRuntimeInitializer(this);

        // Dummy ternary needed by type system due to union and this parameter shenanagins
        this.memberInitializers = this.model.memberInitializers.map((memInit) =>
            memInit instanceof DefaultInitializer
...
export class ArrayMemberInitializer extends Initializer {
    ...
    public constructor(
        context: TranslationUnitContext,
        target: MemberObjectEntity<BoundedArrayType>,
            ...
            let elemInit = DirectInitializer.create(
                ...
                [
                    new OpaqueExpression(context, {
                        ...
                        operate: (rt) =>
                            <
                                | CPPObject<CompleteClassType>
                                | CPPObject<AtomicType>
                            >otherEntity.runtimeLookup(rt),
                    }),
                ],
    ...
    public createRuntimeInitializer(
        this: CompiledArrayMemberInitializer,
        parent: RuntimeConstruct
    ) {
...
export interface CompiledArrayMemberInitializer
    extends ArrayMemberInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeArrayMemberInitializer extends RuntimeInitializer<CompiledArrayMemberInitializer> {
    ...
    public constructor(
        model: CompiledArrayMemberInitializer,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        this.elementInitializers = this.model.elementInitializers.map(
...
export abstract class ListInitializer extends Initializer {
    ...
    public static create(
        ...
        target: ObjectEntity<BoundedArrayType>,
        args: readonly Expression[]
    ): InvalidConstruct; //ArrayListInitializer;
    public static create(
        context: TranslationUnitContext,
        target: ObjectEntity<CompleteClassType>,
        args: readonly Expression[]
    ...
    public static create(
        ...
        target: ObjectEntity,
        args: readonly Expression[]
    ): /*ArrayListInitializer*/ ClassDirectInitializer | InvalidConstruct;
    public static create(
        context: TranslationUnitContext,
        target: ObjectEntity | UnboundReferenceEntity,
        args: readonly Expression[]
    ...
    public static create(
        ...
        target: ObjectEntity | UnboundReferenceEntity,
        args: readonly Expression[]
    ): ListInitializer | InvalidConstruct {
        if (target.type.isReferenceType()) {
            // check for presence of bindTo to detect reference entities
            return new InvalidConstruct(
                context,
                undefined,
                CPPError.declaration.init.list_reference_prohibited,
                args
            );
        } else if (target.type.isAtomicType()) {
            return new InvalidConstruct(
                context,
                undefined,
                CPPError.declaration.init.list_atomic_prohibited,
                args
            );
        } else if (target.type.isBoundedArrayType()) {
            return new ArrayAggregateInitializer(
                context,
                <ObjectEntity<BoundedArrayType>>target,
                args
            );
            ...
            if (target.type.isAggregate()) {
                return new InvalidConstruct(
                    context,
                    undefined,
                    CPPError.declaration.init.aggregate_unsupported,
                    args
                );
            }

            let initializerList = new InitializerListExpression(
                context,
                undefined,
                args
            );
    ...
    public abstract createRuntimeInitializer<T extends ObjectEntityType>(
        this: CompiledListInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeListInitializer<T>;
}

export interface CompiledListInitializer<
    T extends ObjectEntityType = ObjectEntityType
> extends ListInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    readonly target:
        | ObjectEntity<Exclude<T, ReferenceType>>
...
export abstract class RuntimeListInitializer<
    T extends ObjectEntityType = ObjectEntityType,
    C extends CompiledListInitializer<T> = CompiledListInitializer<T>
> extends RuntimeInitializer<C> {
    protected constructor(model: C, parent: RuntimeConstruct) {
        super(model, parent);
    }
...
export class ArrayAggregateInitializer extends ListInitializer {
    ...
    public readonly elemInitializers: readonly (
        | DirectInitializer
        | ValueInitializer
    )[];
    ...
    public constructor(
        ...
        this.explicitElemInitializers = args.map((arg, i) =>
            DirectInitializer.create(
                context,
                new ArraySubobjectEntity(target, i),
                [arg],
        ...
        for (let i = args.length; i < arraySize; ++i) {
            remainingElemInits.push(
                ValueInitializer.create(
                    context,
                    new ArraySubobjectEntity(target, i)
                )
            );
        ...
        this.elemInitializers = this.elemInitializers.concat(
            this.explicitElemInitializers,
            this.implicitElemInitializers
        );
        ...
        this.args = this.explicitElemInitializers.map(
            (elemInit) => elemInit.args[0]
        );
    }

    public createRuntimeInitializer<T extends BoundedArrayType>(
        this: CompiledArrayAggregateInitializer<T>,
        parent: RuntimeConstruct
    ): RuntimeArrayAggregateInitializer<T>;
    public createRuntimeInitializer<T extends CompleteObjectType>(
        this: CompiledListInitializer<T>,
        parent: RuntimeConstruct
    ): never;
    ...
    public createDefaultOutlet(
        this: CompiledArrayAggregateInitializer,
        element: JQuery,
        parent?: ConstructOutlet
    ): ArrayAggregateInitializerOutlet {
...
export interface CompiledArrayAggregateInitializer<
    T extends BoundedArrayType = BoundedArrayType
> extends ArrayAggregateInitializer,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
    ...
    readonly elemInitializers: readonly (
        | CompiledDirectInitializer
        | CompiledValueInitializer
    )[];
...
export class RuntimeArrayAggregateInitializer<
    T extends BoundedArrayType = BoundedArrayType
> extends RuntimeListInitializer<T, CompiledArrayAggregateInitializer<T>> {
    private index = 0;

    public readonly elemInitializers: readonly (
        | RuntimeDirectInitializer
        | RuntimeValueInitializer
    )[];
    ...
    public constructor(
        model: CompiledArrayAggregateInitializer<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        // Create argument initializer instances
        this.explicitElemInitializers = this.model.explicitElemInitializers.map(
            (init) => init.createRuntimeInitializer(this)
        );
        this.implicitElemInitializers = this.model.implicitElemInitializers.map(
            (init) => init.createRuntimeInitializer(this)
        );
        this.elemInitializers = [];
        this.elemInitializers = this.elemInitializers.concat(
