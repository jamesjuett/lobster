...
export interface CompiledInitializer<T extends ObjectEntityType = ObjectEntityType>
  extends Initializer,
    SuccessfullyCompiled {
...
export abstract class DefaultInitializer extends Initializer {
  ...
  ): AtomicDefaultInitializer | ArrayDefaultInitializer | ClassDefaultInitializer;
  public static create(
    context: TranslationUnitContext,
...
export interface CompiledDefaultInitializer<T extends CompleteObjectType = CompleteObjectType>
...
export interface CompiledAtomicDefaultInitializer<T extends AtomicType = AtomicType>
...
export interface CompiledArrayDefaultInitializer<T extends BoundedArrayType = BoundedArrayType>
...
export interface CompiledClassDefaultInitializer<T extends CompleteClassType = CompleteClassType>
...
export interface CompiledValueInitializer<T extends CompleteObjectType = CompleteObjectType>
...
export class AtomicValueInitializer extends ValueInitializer {
  ...
  public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
    let targetDesc = this.target.describe();
    return {
      message: `${targetDesc.name ||
        targetDesc.message} is "value-initialized" to the equivalent of a 0 value for its type.`,
    };
  }
}

export interface CompiledAtomicValueInitializer<T extends AtomicType = AtomicType>
...
export class ArrayValueInitializer extends ValueInitializer {
  ...
  public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
    ...
    if (targetType.numElems === 0) {
      ...
      return {
        message: `The elements of ${targetDesc.name ||
          targetDesc.message} will be "value-initialized" to the equivalent of a 0 value for their type.`,
      };
    } else {
      return {
        message:
...
export interface CompiledArrayValueInitializer<T extends BoundedArrayType = BoundedArrayType>
...
export interface CompiledClassValueInitializer<T extends CompleteClassType = CompleteClassType>
...
export abstract class DirectInitializer extends Initializer {
  ...
  public static create(
    ...
    args: readonly Expression[],
    kind: DirectInitializerKind
  ): AtomicDirectInitializer | ArrayDirectInitializer | ClassDirectInitializer;
  public static create(
    context: TranslationUnitContext,
    target: ObjectEntity | UnboundReferenceEntity,
    args: readonly Expression[],
...
export interface CompiledDirectInitializer<T extends ObjectEntityType = ObjectEntityType>
...
export class ReferenceDirectInitializer extends DirectInitializer {
  ...
  public createRuntimeInitializer<T extends ReferenceType<CompleteObjectType>>(
...
export interface CompiledReferenceDirectInitializer<T extends ReferenceType = ReferenceType>
...
export interface CompiledAtomicDirectInitializer<T extends AtomicType = AtomicType>
...
export interface CompiledClassDirectInitializer<T extends CompleteClassType = CompleteClassType>
...
> extends RuntimeDirectInitializer<T, CompiledClassDirectInitializer<T>> {
  ...
  protected upNextImpl() {
    if (this.index === 'callCtor') {
      (<Mutable<this>>this).ctorCall = this.model.ctorCall.createRuntimeFunctionCall(
        this,
        this.model.target.runtimeLookup(this)
      );
      this.sim.push(this.ctorCall!);
...
export class RuntimeCtorInitializer extends RuntimeConstruct<CompiledCtorInitializer> {
  ...
  public constructor(model: CompiledCtorInitializer, parent: RuntimeConstruct) {
    ...
    this.delegatedConstructorInitializer = this.model.delegatedConstructorInitializer?.createRuntimeInitializer(
      this
    );
      ...
      let elemInit = DirectInitializer.create(
        ...
        [
          new OpaqueExpression(context, {
            ...
            operate: rt =>
              <CPPObject<CompleteClassType> | CPPObject<AtomicType>>otherEntity.runtimeLookup(rt),
          }),
        ],
...
export class RuntimeArrayMemberInitializer extends RuntimeInitializer<
...
export interface CompiledListInitializer<T extends ObjectEntityType = ObjectEntityType>
...
export class ArrayAggregateInitializer extends ListInitializer {
  ...
  public constructor(
    ...
    this.explicitElemInitializers = args.map((arg, i) =>
      DirectInitializer.create(context, new ArraySubobjectEntity(target, i), [arg], 'copy')
    );

    let remainingElemInits: ValueInitializer[] = [];
    for (let i = args.length; i < arraySize; ++i) {
      remainingElemInits.push(
        ValueInitializer.create(context, new ArraySubobjectEntity(target, i))
      );
    }
...
export interface CompiledArrayAggregateInitializer<T extends BoundedArrayType = BoundedArrayType>
...
> extends RuntimeListInitializer<T, CompiledArrayAggregateInitializer<T>> {
  ...
  public constructor(model: CompiledArrayAggregateInitializer<T>, parent: RuntimeConstruct) {
    ...
    this.explicitElemInitializers = this.model.explicitElemInitializers.map(init =>
      init.createRuntimeInitializer(this)
    );
    this.implicitElemInitializers = this.model.implicitElemInitializers.map(init =>
      init.createRuntimeInitializer(this)
    );
    this.elemInitializers = [];
    this.elemInitializers = this.elemInitializers.concat(
