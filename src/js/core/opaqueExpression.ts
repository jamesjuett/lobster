...
export type OpaqueExpressionImpl<
...
> = {
    ...
    operate: (
        rt: RuntimeOpaqueExpression<T, VC>
    ) => T extends VoidType ? void : VCResultTypes<T, VC>;
};
...
export function registerOpaqueExpression<
    T extends ExpressionType = ExpressionType,
    VC extends ValueCategory = ValueCategory
>(id: string, impl: OpaqueExpressionImpl<T, VC>) {
    assert(!OPAQUE_EXPRESSIONS[id]);
    OPAQUE_EXPRESSIONS[id] = <any>impl;
}

export class OpaqueExpression<
    T extends ExpressionType = ExpressionType,
    VC extends ValueCategory = ValueCategory
> extends Expression<OpaqueExpressionASTNode> {
    ...
    public constructor(
        context: ExpressionContext,
        impl: OpaqueExpressionImpl<T, VC>,
        ast?: OpaqueExpressionASTNode
    ) {
...
export interface TypedOpaqueExpression<
    T extends ExpressionType = ExpressionType,
    V extends ValueCategory = ValueCategory
> extends OpaqueExpression<T, V>,
        t_TypedExpression {}

export interface CompiledOpaqueExpression<
    T extends ExpressionType = ExpressionType,
    V extends ValueCategory = ValueCategory
> extends TypedOpaqueExpression<T, V>,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}

export class RuntimeOpaqueExpression<
    T extends ExpressionType = ExpressionType,
    VC extends ValueCategory = ValueCategory
> extends RuntimeExpression<T, VC, CompiledOpaqueExpression<T, VC>> {
    public constructor(
        model: CompiledOpaqueExpression<T, VC>,
...
export function getLocal<T extends CompleteObjectType>(
    rt: RuntimeExpression,
    name: string
) {
    let local = <LocalObjectEntity<T> | LocalReferenceEntity<ReferenceType<T>>>(
        rt.model.context.contextualScope.lookup(name)
    );
