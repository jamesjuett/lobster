...
const StatementConstructsMap = {
  ...
  continue_statement: (ast: ContinueStatementASTNode, context: BlockContext) =>
  ...
  declaration_statement: (ast: DeclarationStatementASTNode, context: BlockContext) =>
    DeclarationStatement.createFromAST(ast, context),
  expression_statement: (ast: ExpressionStatementASTNode, context: BlockContext) =>
  ...
  anything_construct: (ast: AnythingConstructASTNode, context: BlockContext) =>
...
export type AnalyticCompiledStatement<
  C extends AnalyticStatement
> = CompiledStatementKinds[C['construct_type']];

const StatementConstructsRuntimeMap = {
  unsupported_statement: (construct: UnsupportedStatement, parent: RuntimeStatement) => {
  ...
  block: (construct: CompiledBlock, parent: RuntimeStatement | RuntimeFunction) =>
  ...
  while_statement: (construct: CompiledWhileStatement, parent: RuntimeStatement) =>
  ...
  for_statement: (construct: CompiledForStatement, parent: RuntimeStatement) =>
    new RuntimeForStatement(construct, parent),
  break_statement: (construct: CompiledBreakStatement, parent: RuntimeStatement) =>
  ...
  return_statement: (construct: CompiledReturnStatement, parent: RuntimeStatement) =>
    new RuntimeReturnStatement(construct, parent),
  declaration_statement: (construct: CompiledDeclarationStatement, parent: RuntimeStatement) =>
    new RuntimeDeclarationStatement(construct, parent),
  expression_statement: (construct: CompiledExpressionStatement, parent: RuntimeStatement) =>
    new RuntimeExpressionStatement(construct, parent),
  null_statement: (construct: CompiledNullStatement, parent: RuntimeStatement) =>
...
): ReturnType<typeof StatementConstructsRuntimeMap[ConstructType['construct_type']]>;
...
): ReturnType<typeof StatementConstructsRuntimeMap[CompiledConstructType['construct_type']]>;
export function createRuntimeStatement(
  ...
  public isSemanticallyEquivalent_impl(other: AnalyticConstruct, ec: SemanticContext): boolean {
    ...
    if (other.construct_type === this.construct_type) {
      ...
      if (
        ...
        chunks.every((c, i) => areChunksEquivalent(c, otherChunks[i], ec)) &&
        areSemanticallyEquivalent(this.localDeallocator, other.localDeallocator, ec)
      ) {
        return true;
      }
      ...
      if (
        ...
        if (currentChunk) {
          if (stmt.entitiesUsed().some(e => currentChunk!.entitiesUsed.has(e.entityId))) {
            ...
            currentChunk = {
              stmts: [stmt],
              entitiesUsed: new Set(stmt.entitiesUsed().map(e => e.entityId)),
            };
          } else {
    ...
    let body =
      ast.body.construct_type !== 'block'
        ...
        : new Block(
            bodyBlockContext,
            ast.body,
            ast.body.statements.map(s => createStatementFromAST(s, bodyBlockContext))
          );
