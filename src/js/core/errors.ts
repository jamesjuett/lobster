...
export var CPPError = {
    ...
    expr : {
        ...
        binary : {
            overload_not_found : function(src, op, operands){
                return CompilerNote.instance(src, CompilerNote.TYPE_ERROR, "expr.binary.overload_not_found", "An overloaded " + op + " operator for the types (" + operands.map((op)=>{return op.type;}).join(", ") + ") cannot be found.");
            },
            ...
            boolean_operand : function(src, operator, operand) {    
                return CompilerNote.instance(src, CompilerNote.TYPE_ERROR, "expr.binary.boolean_operand", "Invalid operand type (" + operand.type ") for operator " + operator + ", which requires operands that may be converted to boolean type.");
            },
