...
export class Simulation {
  ...
  private start() {
    ...
    (<Mutable<this>>(
      this
    )).globalAllocator = this.program.staticObjectAllocator.createRuntimeConstruct(this);
  ...
  public stepForward() {
    ...
    this.observable.send(
      'afterFullStep',
      this.execStack.length > 0 && this.execStack[this.execStack.length - 1]
    );
  }
  ...
  public atEndOfMain() {
    return (
      this.top()?.model === this.program.mainFunction.body.localDeallocator ||
  ...
  public parameterPassedByReference<T extends ReferenceType<PotentiallyCompleteObjectType>>(
