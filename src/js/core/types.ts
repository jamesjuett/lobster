...
abstract class TypeBase {
  ...
  public isIntegralType(): this is IntegralType {
    return (
      this instanceof Char || this instanceof Int || this instanceof Size_t || this instanceof Bool
    );
  }
...
export type Completed<T extends PotentiallyCompleteObjectType> = T extends CompleteObjectType
  ? T
...
export abstract class SimpleType extends AtomicType {
  ...
  public similarType(other: Type): boolean {
    return other instanceof SimpleType && other.simpleType === this.simpleType;
  }
...
> extends AtomicType {
  ...
  public similarType(other: Type): boolean {
    return other instanceof PointerType && this.ptrTo.similarType(other.ptrTo);
  }
  ...
  public englishString(plural: boolean) {
    return (
      (plural ? this.getCVString() + 'pointers to' : 'a ' + this.getCVString() + 'pointer to') +
      ' ' +
...
export class BoundedArrayType<Elem_type extends ArrayElemType = ArrayElemType> extends TypeBase
...
> extends TypeBase {
  ...
  public englishString(plural: boolean) {
    return (
      (plural ? 'arrays of unknown bound of ' : 'an array of unknown bound of ') +
...
class ClassTypeBase extends TypeBase implements Omit<ObjectTypeInterface, 'size'> {
  ...
  public typeString(excludeBase: boolean, varname: string, decorated?: boolean) {
    if (excludeBase) {
      ...
      return (
        this.getCVString() +
        (decorated ? htmlDecoratedType(this.className) : this.className) +
        (varname ? ' ' + varname : '')
      );
    }
  }
  ...
  public englishString(plural: boolean) {
    return (
      ...
      (plural
        ? this.className + 's'
        : (isVowel(this.className.charAt(0)) ? 'an ' : 'a ') + this.className)
    );
  }
  ...
  public isAggregate(this: CompleteClassType) {
    ...
    if (
      this.classDefinition.memberVariableEntities.some(
        memEnt => memEnt.firstDeclaration.context.accessLevel === 'private'
      )
    ) {
      return false;
    }
...
function sameClassType(thisClass: ClassTypeBase, otherClass: ClassTypeBase) {
  ...
  return (
    ...
    (!!(thisClass as any).shared.classDefinition &&
      (thisClass as any).shared.classDefinition === (otherClass as any).shared.classDefinition)
  );
}
...
> extends TypeBase {
  ...
  public constructor(
    ...
    this.paramTypes = paramTypes.map(ptype =>
      ptype.isPotentiallyCompleteClassType() ? ptype : ptype.cvUnqualified()
    );
