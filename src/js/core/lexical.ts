...
export const LOBSTER_MAGIC_FUNCTIONS = new Set(
    Object.keys(MAGIC_FUNCTION_NAMES)
);
...
export function checkIdentifier(
    src: TranslationUnitConstruct,
    name: LexicalIdentifier,
    noteHandler: NoteHandler
) {
    // Special case for qualified names
    if (typeof name !== "string") {
        name.components.forEach((elem) =>
            checkIdentifier(src, elem, noteHandler)
        );
        return;
    }
...
export function composeQualifiedName(
    prefix: LexicalIdentifier,
    name: LexicalIdentifier
) {
...
export function isUnqualifiedName(
    name: LexicalIdentifier
): name is UnqualifiedName {
...
export function isQualifiedIdentifier(
    id: LexicalIdentifier
): id is QualifiedName {
    return typeof id !== "string";
}
