...
export abstract class Expression<
  ASTType extends ExpressionASTNode = ExpressionASTNode
> extends PotentialFullExpression<ExpressionContext, ASTType> {
  ...
  public isPrvalue<T extends ExpressionType, V extends ValueCategory>(
    this: TypedExpression<T, V>
  ): this is TypedExpression<T, 'prvalue'> {
    return this.valueCategory === 'prvalue';
  }

  public isLvalue<T extends ExpressionType, V extends ValueCategory>(
    this: TypedExpression<T, V>
  ): this is TypedExpression<T, 'lvalue'> {
...
> extends Expression<ExpressionASTNode> {
  readonly type: T;
...
export interface CompiledExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends TypedExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}

export type SpecificCompiledExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> = V extends ValueCategory ? CompiledExpression<T, V> : never;

export interface WellTyped<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> {
  readonly type: T;
...
export type SpecificTypedExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> = TypedExpression<T, V>;

export function allWellTyped(expressions: Expression[]): expressions is TypedExpression[];
export function allWellTyped(
  expressions: readonly Expression[]
): expressions is readonly TypedExpression[];
export function allWellTyped(
  expressions: readonly Expression[]
): expressions is readonly TypedExpression[] {
  return expressions.every(expr => {
    return expr.isWellTyped();
  });
}

export function allObjectTyped(
  expressions: Expression[]
): expressions is TypedExpression<CompleteObjectType>[];
export function allObjectTyped(
  expressions: readonly Expression[]
): expressions is readonly TypedExpression<CompleteObjectType>[];
export function allObjectTyped(
  expressions: readonly Expression[]
): expressions is readonly TypedExpression<CompleteObjectType>[] {
  return expressions.every(expr => {
    return Predicates.isTypedExpression(expr, isCompleteObjectType);
  });
}

export type VCResultTypes<T extends Type, V extends ValueCategory> = T extends FunctionType
  ? V extends 'prvalue'
    ? never
    : V extends 'xvalue'
    ? never
  ...
  ? //      However, if T is actually ClassType, then it can't be an AtomicType and we go with the second option of only CPPObject<T>
    ...
    : V extends 'xvalue'
    ? CPPObject<T>
    : CPPObject<T> /*ObjectType extends T ?*/ // lvalue // That is, T is more general, so it's possible T is an AtomicType or an ObjectType
  : V extends 'prvalue'
  ? Value<AtomicType> | CPPObject<CompleteObjectType>
  : V extends 'xvalue'
...
export abstract class RuntimeExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory,
