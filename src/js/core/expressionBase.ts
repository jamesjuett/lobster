...
): expressions is TypedExpression[];
export function allWellTyped(
...
> = T extends FunctionType
    ...
    ? // e.g. If T is actually ObjectType, then it could be an AtomicType and we go with the first option Value<AtomicType> | CPPObject<T>.
      ...
      V extends "prvalue"
        ...
        : V extends "xvalue"
        ? CPPObject<T>
        : CPPObject<T> /*ObjectType extends T ?*/ // lvalue
    : // That is, T is more general, so it's possible T is an AtomicType or an ObjectType
    V extends "prvalue"
