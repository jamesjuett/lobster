...
export namespace Predicates {
  ...
  export function byKind<NarrowedKind extends ConstructKind<AnalyticConstruct>>(
    constructKind: NarrowedKind
  ) {
    return <
      /*¯\_(ツ)_/¯*/ <
        Original extends { construct_type: string },
        Narrowed extends DiscriminateUnion<Original, 'construct_type', NarrowedKind>
      >(
        construct: Original
  ...
  export function byKinds<NarrowedKind extends ConstructKind<AnalyticConstruct>>(
  ...
  >(typePredicate?: (o: ExpressionType) => o is NarrowedT, valueCategory?: NarrowedVC) {
    return <
      /*¯\_(ツ)_/¯*/ <
  ...
  export function byVariableUpdate(name: string) {
    ...
    >(construct => {
      if (
        Predicates.byKinds(['prefix_increment_expression', 'postfix_increment_expression'])(
          construct
        )
      ) {
        // check for var
      ...
      if (Predicates.byKind('assignment_expression')(construct)) {
        ...
        return (
          !!findFirstConstruct(construct.lhs, Predicates.byIdentifierName(name)) &&
  ...
  export function byFunctionCallName<N extends string>(name: N) {
    return <
      (construct: AnalyticConstruct) => construct is FunctionCallExpression & { call: FunctionCall }
    >(construct =>
      construct instanceof FunctionCallExpression && construct.call?.func.name === name);
  }
  ...
  export function byVariableAssignedTo<VE extends VariableEntity>(v: VE) {
    return <
      (
        construct: AnalyticConstruct
      ) => construct is AssignmentExpression | CompoundAssignmentExpression
    >(construct =>
      (construct.construct_type === 'assignment_expression' ||
        construct.construct_type === 'compound_assignment_expression') &&
  ...
  export function byVariableIncremented<VE extends VariableEntity>(v: VE) {
    return <
      (
        construct: AnalyticConstruct
      ) => construct is PrefixIncrementExpression | PostfixIncrementExpression
    >(construct =>
  ...
  export function byMemberAccessName<N extends string>(memberName: N) {
    return <
      (
        construct: AnalyticConstruct
      ) => construct is DotExpression | (ArrowExpression & { memberName: N })
    >(construct =>
