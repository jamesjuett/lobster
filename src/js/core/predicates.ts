import {
    AnalyticExpression,
    TypedExpressionKinds,
    CompiledExpressionKinds,
    TernaryExpression,
    TypedCommaExpression,
    AnalyticCompiledExpression,
    AnalyticTypedExpression,
...
import {
    ...
    VoidDeclaration,
    TypedUnknownBoundArrayDeclaration,
    FunctionDeclaration,
    TypedFunctionDeclaration,
    LocalVariableDefinition,
    TypedLocalVariableDefinition,
    GlobalVariableDefinition,
    TypedGlobalVariableDefinition,
    ...
    TypeSpecifier,
    StorageSpecifier,
    AnalyticTypedDeclaration,
    TypedDeclarationKinds,
...
export type ConstructKind<Cs extends { construct_type: string }> =
    Cs["construct_type"];
...
export namespace Predicates {
    ...
    export function byKind<
        NarrowedKind extends ConstructKind<AnalyticConstruct>
    >(constructKind: NarrowedKind) {
        return <
            /*¯\_(ツ)_/¯*/ <
                Original extends { construct_type: string },
                Narrowed extends DiscriminateUnion<
                    Original,
                    "construct_type",
                    NarrowedKind
                >
    ...
    export function byKinds<
        NarrowedKind extends ConstructKind<AnalyticConstruct>
    >(constructKinds: readonly NarrowedKind[]) {
        return <
            /*¯\_(ツ)_/¯*/ <
                Original extends { construct_type: string },
                Narrowed extends DiscriminateUnion<
        ...
        >((construct) =>
            (<readonly string[]>constructKinds).indexOf(
    ...
    export function byTypedExpression<
        NarrowedT extends ExpressionType,
        NarrowedVC extends ValueCategory
    >(
        typePredicate?: (o: ExpressionType) => o is NarrowedT,
        valueCategory?: NarrowedVC
    ) {
        return <
            /*¯\_(ツ)_/¯*/ <
                OriginalT extends ExpressionType,
                Original extends AnalyticConstruct & {
                    type?: OriginalT;
                    valueCategory?: ValueCategory;
                },
                Narrowed extends Original extends AnalyticExpression
                    ? AnalyticTypedExpression<Original, NarrowedT, NarrowedVC>
                    : never
            >(
                construct: Original
    ...
    export function isTypedExpression<
        OriginalT extends ExpressionType,
        NarrowedT extends ExpressionType,
        Original extends AnalyticConstruct & {
            type?: OriginalT;
    ...
    >(
        ...
        typePredicate?: (o: ExpressionType) => o is NarrowedT,
        valueCategory?: NarrowedVC
    ): construct is Narrowed extends Original
        ? Narrowed
        : Original extends Narrowed
        ? Original
        : never;
    export function isTypedExpression<
        OriginalT extends ExpressionType,
        NarrowedT extends ExpressionType,
        Original extends CPPConstruct & {
            type?: OriginalT;
            valueCategory?: ValueCategory;
        },
        NarrowedVC extends NonNullable<Original["valueCategory"]>,
        Narrowed extends Original extends Expression
            ? TypedExpression<NarrowedT, NarrowedVC>
            : never
    >(
        construct: Original,
        typePredicate: (o: ExpressionType) => o is NarrowedT,
    ...
    >(
        ...
        return !!(
            construct.type &&
            (!typePredicate || typePredicate(construct.type)) &&
            (!valueCategory || construct.valueCategory === valueCategory)
        );
    }

    // Basically copies of above but with Declaration swapping in for Expression and ValueCategory removed
    export function byTypedDeclaration<NarrowedT extends Type>(
        typePredicate?: (o: Type) => o is NarrowedT
    ) {
        return <
            /*¯\_(ツ)_/¯*/ <
                OriginalT extends Type,
                Original extends AnalyticConstruct & { type?: OriginalT },
                Narrowed extends Original extends AnalyticDeclaration
                    ? AnalyticTypedDeclaration<Original, NarrowedT>
                    : never
            >(
                construct: Original
            ) => construct is Narrowed extends Original ? Narrowed : never // TODO conditional on this line can probably be removed
    ...
    >(
        ...
        // TODO conditional on this line can probably be removed
        return !!(
            construct.type &&
            (!typePredicate || typePredicate(construct.type))
        );
    ...
    export function byVariableName(name: string) {
        return <
            (
                construct: AnalyticConstruct
            ) => construct is LocalVariableDefinition | GlobalVariableDefinition
        >((construct) =>
            (construct instanceof LocalVariableDefinition ||
                construct instanceof GlobalVariableDefinition) &&
            construct.name === name);
    }
    ...
    export function byVariableInitialValue(queryValue: number) {
        ...
        >((construct) => {
            ...
            return (
                expr instanceof NumericLiteralExpression &&
                expr.value.rawEquals(queryValue)
            );
        });
    }
    ...
    export function byVariableUpdate(name: string) {
        ...
        >((construct) => {
            if (
                ...
                // check for var
                return !!findFirstConstruct(
                    construct,
                    Predicates.byIdentifierName(name)
                );
            }
    ...
    export function byFunctionName(name: string) {
        return <
            (construct: AnalyticConstruct) => construct is FunctionDefinition
        >((construct) =>
            construct instanceof FunctionDefinition && construct.name === name);
    }
    ...
    export function byFunctionCallName<N extends string>(name: N) {
        return <
            (
                construct: AnalyticConstruct
            ) => construct is FunctionCallExpression & { call: FunctionCall }
        >((construct) =>
            construct instanceof FunctionCallExpression &&
            construct.call?.func.name === name);
    }
    ...
    export function byOperatorOverloadCall(operator: t_OverloadableOperators) {
        return <
            (
                construct: AnalyticConstruct
            ) => construct is OperatorOverloadExpression
        >((construct) =>
            isOperatorOverload(construct) && construct.operator === operator);
    }
    ...
    export function byIdentifierName<N extends string>(name: N) {
        return <
            (
                construct: AnalyticConstruct
            ) => construct is IdentifierExpression & { name: N }
        >((construct) =>
            construct.construct_type === "identifier_expression" &&
            construct.name === name);
    }
    ...
    export function byVariableIdentifier<VE extends VariableEntity>(v: VE) {
        return <
            (
                construct: AnalyticConstruct
            ) => construct is IdentifierExpression & { entity: VE }
        >((construct) =>
            construct.construct_type === "identifier_expression" &&
            construct.entity === v);
    }
    ...
    export function byVariableAssignedTo<VE extends VariableEntity>(v: VE) {
        return <
            (
                ...
                | CompoundAssignmentExpression
        >((construct) =>
            (construct.construct_type === "assignment_expression" ||
                construct.construct_type ===
                    "compound_assignment_expression") &&
            containsConstruct(
                construct.lhs,
                Predicates.byVariableIdentifier(v)
            ));
    }
    ...
    export function byVariableIncremented<VE extends VariableEntity>(v: VE) {
        return <
            (
                ...
                | PostfixIncrementExpression
        >((construct) =>
            (construct.construct_type === "prefix_increment_expression" ||
                construct.construct_type === "postfix_increment_expression") &&
            containsConstruct(
                construct.operand,
                Predicates.byVariableIdentifier(v)
            ));
    ...
    export function byMemberAccessName<N extends string>(memberName: N) {
        return <
            (
                ...
                | DotExpression
                | (ArrowExpression & { memberName: N })
        >((construct) =>
            (construct.construct_type === "dot_expression" ||
                construct.construct_type === "arrow_expression") &&
            construct.memberName === memberName);
    }
    ...
    export const isLoop = Predicates.byKinds([
        "while_statement",
        "for_statement",
    ]);
    export const isOperatorOverload = Predicates.byKinds([
        "non_member_operator_overload_expression",
        "member_operator_overload_expression",
        "invalid_operator_overload_expression",
    ]);

    export function isIndexingOperation(
        construct: AnalyticConstruct
    ): construct is SubscriptExpression | OperatorOverloadExpression {
        return (
            Predicates.byKind("subscript_expression")(construct) ||
            Predicates.byOperatorOverloadCall("[]")(construct)
        );
    }
