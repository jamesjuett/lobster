...
export class ClassScope extends NamedScope {
  ...
  public lookup(
    ...
    let baseMember =
      this.base &&
      !options.noBase &&
      this.base.lookup(name, Object.assign({}, options, { noParent: true }));
    if (baseMember) {
      return baseMember;
    }
    ...
    let parentMember =
      this.parent &&
      !options.noParent &&
      this.parent.lookup(name, Object.assign({}, options, { noBase: true }));
    if (parentMember) {
      return parentMember;
    }
...
export class LocalObjectEntity<T extends CompleteObjectType = CompleteObjectType>
  ...
  public describe() {
    return {
      name: this.name,
      message: `the ${this.isParameter ? 'parameter' : 'local variable'} ${this.name}`,
    };
  }
}
...
export class LocalReferenceEntity<T extends ReferenceType = ReferenceType>
  ...
  public describe() {
    return {
      name: this.name,
      message: `the ${this.isParameter ? 'reference parameter' : 'reference'} ${this.name}`,
    };
  }
}
 ...
 */
export class ReturnObjectEntity<T extends CompleteObjectType = CompleteObjectType>
  extends CPPEntity<T>
  implements ObjectEntity<T> {
  public readonly variableKind = 'object';
...
export class ReturnByReferenceEntity<T extends ReferenceType = ReferenceType> extends CPPEntity<T>
...
export class PassByValueParameterEntity<T extends CompleteObjectType = CompleteObjectType>
  extends CPPEntity<T>
  implements ObjectEntity<T> {
  public readonly variableKind = 'object';
...
export class PassByReferenceParameterEntity<T extends ReferenceType = ReferenceType>
  extends CPPEntity<T>
  implements UnboundReferenceEntity<T> {
  public readonly calledFunction: FunctionEntity;
  public readonly num: number;
...
export class ReceiverEntity extends CPPEntity<CompleteClassType>
...
export class NewObjectEntity<T extends NewObjectType = NewObjectType> extends CPPEntity<T>
  implements ObjectEntity<T> {
  public readonly variableKind = 'object';
  ...
  public describe() {
    return {
      ...
      message: 'the dynamically allocated object (of type ' + this.type + ') created by new',
    };
  }
}
...
> extends CPPEntity<T> {
  ...
  public describe() {
    return {
      ...
      message:
        'the dynamically allocated/sized array (of element type ' + this.type + ') created by new',
    };
  }
}

export class ArraySubobjectEntity<T extends ArrayElemType = ArrayElemType> extends CPPEntity<T>
  implements ObjectEntity<T> {
  public readonly variableKind = 'object';
...
export class DynamicLengthArrayNextElementEntity<T extends ArrayElemType = ArrayElemType>
  extends CPPEntity<T>
  implements ObjectEntity<T> {
  public readonly variableKind = 'object';
...
export class BaseSubobjectEntity extends CPPEntity<CompleteClassType>
  implements ObjectEntity<CompleteClassType> {
  public readonly variableKind = 'object';
  ...
  public describe() {
    return {
      name:
        'the ' + this.type.className + ' base class of ' + this.containingEntity.describe().name,
      message:
        'the ' +
        this.type.className +
...
export class TemporaryObjectEntity<T extends CompleteObjectType = CompleteObjectType>
  extends CPPEntity<T>
  implements ObjectEntity<T> {
  public readonly variableKind = 'object';
  ...
  public runtimeLookup(rtConstruct: RuntimeConstruct) {
    ...
    return <TemporaryObject<T>>(
      (<RuntimePotentialFullExpression>rtConstruct).containingFullExpression.temporaryObjects[
        this.entityId
      ]
    );
  }
  ...
  public link(def: FunctionDefinitionGroup | undefined) {
    ...
    if (def) {
      ...
      if (!overload) {
        if (this.isOdrUsed) {
          this.declarations.forEach(decl =>
            decl.addNote(CPPError.link.func.no_matching_overload(decl, this))
          );
        }
        return;
      }
      ...
      if (!this.type.sameReturnType(overload.declaration.type)) {
        this.declarations.forEach(decl =>
          decl.addNote(CPPError.link.func.returnTypesMatch(decl, this))
        );
        return;
      }
      ...
      if (this.isMemberFunction && this.isVirtual && !this.isPureVirtual) {
        ...
        this.declarations.forEach(decl =>
          decl.addNote(CPPError.link.func.virtual_def_required(decl, this))
        );
