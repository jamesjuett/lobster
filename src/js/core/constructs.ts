...
export function createBlockContext(
  ...
  return Object.assign({}, parentContext, {
    contextualScope:
      sharedScope || new BlockScope(parentContext.translationUnit, parentContext.contextualScope),
...
export function createOutOfLineFunctionDefinitionContext(
  ...
  return Object.assign({}, declarationContext, {
    contextualScope: declarationContext.contextualScope.createAlternateParentProxy(
...
export function areAllSemanticallyEquivalent(
  ...
  constructs = constructs.filter(
    c => c.construct_type !== 'TemporaryDeallocator' && c.construct_type !== 'LocalDeallocator'
  );
  ...
  protected constructor(context: ContextType, ast: ASTType | undefined) {
    ...
    if (ast) {
      ...
      if (this.context.translationUnit) {
        asMutable(this).sourceReference = this.context.translationUnit.getSourceReference(
          ast.source.line,
          ast.source.column,
          ast.source.start,
          ast.source.end
...
export type TranslationUnitConstruct<ASTType extends ASTNode = ASTNode> = CPPConstruct<
  TranslationUnitContext,
  ASTType
>;
