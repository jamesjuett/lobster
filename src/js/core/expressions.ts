...
import {
  ...
  createExpressionContextWithReceiverType,
...
import {
  ...
  RuntimeFunctionCallExpression,
} from './FunctionCallExpression';
import {
  RuntimeExpression,
  VCResultTypes,
  ValueCategory,
  ...
  CompiledExpression,
  TypedExpression,
  SpecificTypedExpression,
  t_TypedExpression,
  allWellTyped,
} from './expressionBase';
import {
  ConstructOutlet,
  TernaryExpressionOutlet,
  CommaExpressionOutlet,
  AssignmentExpressionOutlet,
  BinaryOperatorExpressionOutlet,
  UnaryOperatorExpressionOutlet,
  SubscriptExpressionOutlet,
  IdentifierOutlet,
  NumericLiteralOutlet,
  ParenthesesOutlet,
  MagicFunctionCallExpressionOutlet,
  StringLiteralExpressionOutlet,
  LValueToRValueOutlet,
  ArrayToPointerOutlet,
  TypeConversionOutlet,
  QualificationConversionOutlet,
  DotExpressionOutlet,
  ArrowExpressionOutlet,
  OutputOperatorExpressionOutlet,
  PostfixIncrementExpressionOutlet,
  InputOperatorExpressionOutlet,
  StreamToBoolOutlet,
  NonMemberOperatorOverloadExpressionOutlet,
  MemberOperatorOverloadExpressionOutlet,
  InitializerListOutlet as InitializerListExpressionOutlet,
  CompoundAssignmentExpressionOutlet,
  ThisExpressionOutlet,
  NullptrExpressionOutlet,
} from '../view/codeOutlets';
import { AnalyticConstruct, Predicates } from './predicates';
import {
  OpaqueExpression,
  RuntimeOpaqueExpression,
  TypedOpaqueExpression,
  CompiledOpaqueExpression,
...
import {
  createNewExpressionFromAST,
  DeleteExpression,
  NewExpression,
  TypedNewExpression,
  TypedDeleteExpression,
  CompiledNewExpression,
  CompiledDeleteExpression,
  RuntimeNewExpression,
  RuntimeDeleteExpression,
  NewObjectType,
  NewArrayExpression,
  CompiledNewArrayExpression,
  RuntimeNewArrayExpression,
  TypedNewArrayExpression,
  DeleteArrayExpression,
  CompiledDeleteArrayExpression,
  RuntimeDeleteArrayExpression,
  TypedDeleteArrayExpression,
} from './new_delete';
import {
  ...
  parseNumericLiteralValueFromAST,
...
import {
  ...
  checkForNullptrEquivalence,
  checkForZeroEquivalence,
} from '../analysis/semantic_equivalence';
...
export function readValueWithAlert(obj: CPPObject<AtomicType>, sim: Simulation) {
  ...
  if (!value.isValid) {
    ...
    var msg =
      'The value you just got out of ' +
      (objDesc.name || objDesc.message) +
      " isn't valid. It might be uninitialized or it could have come from a dead object.";
    if (value.rawValue === 0) {
      msg +=
...
const ExpressionConstructsMap = {
  comma_expression: (ast: CommaASTNode, context: ExpressionContext) =>
    CommaExpression.createFromAST(ast, context),

  ternary_expression: (ast: TernaryASTNode, context: ExpressionContext) =>
    TernaryExpression.createFromAST(ast, context),

  assignment_expression: (ast: AssignmentExpressionASTNode, context: ExpressionContext) =>
    AssignmentExpression.createFromAST(ast, context),
  compound_assignment_expression: (
    ast: CompoundAssignmentExpressionASTNode,
    context: ExpressionContext
  ) => CompoundAssignmentExpression.createFromAST(ast, context),

  // binary operators
  arithmetic_binary_operator_expression: (
    ast: ArithmeticBinaryOperatorExpressionASTNode,
    context: ExpressionContext
  ) => ArithmeticBinaryOperatorExpression.createFromAST(ast, context),
  relational_binary_operator_expression: (
    ast: RelationalBinaryOperatorExpressionASTNode,
    context: ExpressionContext
  ) => RelationalBinaryOperatorExpression.createFromAST(ast, context),
  logical_binary_operator_expression: (
    ast: LogicalBinaryOperatorExpressionASTNode,
    context: ExpressionContext
  ) => LogicalBinaryOperatorExpression.createFromAST(ast, context),

  pointer_to_member_expression: (
    ast: PointerToMemberExpressionASTNode,
    context: ExpressionContext
  ) => new UnsupportedExpression(context, ast, 'pointer-to-member'),

  c_style_cast_expression: (ast: CStyleCastExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'c-style cast'),

  // prefix operators
  prefix_increment_expression: (
    ast: PrefixIncrementExpressionASTNode,
    context: ExpressionContext
  ) => PrefixIncrementExpression.createFromAST(ast, context),
  dereference_expression: (ast: DereferenceExpressionASTNode, context: ExpressionContext) =>
    DereferenceExpression.createFromAST(ast, context),
  address_of_expression: (ast: AddressOfExpressionASTNode, context: ExpressionContext) =>
    AddressOfExpression.createFromAST(ast, context),
  unary_plus_expression: (ast: UnaryPlusExpressionASTNode, context: ExpressionContext) =>
    UnaryPlusExpression.createFromAST(ast, context),
  unary_minus_expression: (ast: UnaryMinusExpressionASTNode, context: ExpressionContext) =>
    UnaryMinusExpression.createFromAST(ast, context),
  logical_not_expression: (ast: LogicalNotExpressionASTNode, context: ExpressionContext) =>
    LogicalNotExpression.createFromAST(ast, context),
  bitwise_not_expression: (ast: BitwiseNotExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'bitwise not'),
  sizeof_expression: (ast: SizeofExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'sizeof'),
  sizeof_type_expression: (ast: SizeofTypeExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'sizeof (type)'),
  new_expression: (ast: NewExpressionASTNode, context: ExpressionContext) =>
    createNewExpressionFromAST(ast, context),
  delete_expression: (ast: DeleteExpressionASTNode, context: ExpressionContext) =>
    DeleteExpression.createFromAST(ast, context),
  delete_array_expression: (ast: DeleteArrayExpressionASTNode, context: ExpressionContext) =>
    DeleteArrayExpression.createFromAST(ast, context),

  // postfix operators
  static_cast_expression: (ast: StaticCastExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'static cast'),
  dynamic_cast_expression: (ast: DynamicCastExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'dynamic cast'),
  reinterpret_cast_expression: (
    ast: ReinterpretCastExpressionASTNode,
    context: ExpressionContext
  ) => new UnsupportedExpression(context, ast, 'reinterpret cast'),
  const_cast_expression: (ast: ConstCastExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'const cast'),
  subscript_expression: (ast: SubscriptExpressionASTNode, context: ExpressionContext) =>
    SubscriptExpression.createFromAST(ast, context),
  function_call_expression: (ast: FunctionCallExpressionASTNode, context: ExpressionContext) =>
    FunctionCallExpression.createFromAST(ast, context),
  dot_expression: (ast: DotExpressionASTNode, context: ExpressionContext) =>
    DotExpression.createFromAST(ast, context),
  arrow_expression: (ast: ArrowExpressionASTNode, context: ExpressionContext) =>
    ArrowExpression.createFromAST(ast, context),
  postfix_increment_expression: (
    ast: PostfixIncrementExpressionASTNode,
    context: ExpressionContext
  ) => PostfixIncrementExpression.createFromAST(ast, context),

  construct_expression: (ast: ConstructExpressionASTNode, context: ExpressionContext) =>
    new UnsupportedExpression(context, ast, 'construct expression'),

  identifier_expression: (ast: IdentifierExpressionASTNode, context: ExpressionContext) =>
    IdentifierExpression.createFromAST(ast, context),

  this_expression: (ast: ThisExpressionASTNode, context: ExpressionContext) =>
    ThisExpression.createFromAST(ast, context),
  nullptr_expression: (ast: NullptrExpressionASTNode, context: ExpressionContext) =>
    NullptrExpression.createFromAST(ast, context),

  numeric_literal_expression: (ast: NumericLiteralASTNode, context: ExpressionContext) =>
    NumericLiteralExpression.createFromAST(ast, context),
  string_literal_expression: (ast: StringLiteralASTNode, context: ExpressionContext) =>
    StringLiteralExpression.createFromAST(ast, context),
  parentheses_expression: (ast: ParenthesesExpressionASTNode, context: ExpressionContext) =>
    ParenthesesExpression.createFromAST(ast, context),
  initializer_list_expression: (
    ast: InitializerListExpressionASTNode,
    context: ExpressionContext
  ) => InitializerListExpression.createFromAST(ast, context),
  opaque_expression: (ast: OpaqueExpressionASTNode, context: ExpressionContext) =>
    OpaqueExpression.createFromAST(ast, context),
  anything_construct: (ast: AnythingConstructASTNode, context: ExpressionContext) =>
    new AnythingExpression(context, ast),
};
 ...
 */
export function createExpressionFromAST<ASTType extends ExpressionASTNode>(
  ast: ASTType,
  context: ExpressionContext
): ReturnType<typeof ExpressionConstructsMap[ASTType['construct_type']]> {
...
export type TypedExpressionKinds<T extends ExpressionType, V extends ValueCategory> = {
  ...
  comma_expression: T extends NonNullable<TypedCommaExpression['type']>
    ? V extends NonNullable<TypedCommaExpression['valueCategory']>
      ? TypedCommaExpression<T, V>
      : never
    : NonNullable<TypedCommaExpression['type']> extends T
    ? V extends NonNullable<TypedCommaExpression['valueCategory']>
      ? TypedCommaExpression
      : never
    : never;
  ternary_expression: T extends NonNullable<TypedTernaryExpression['type']>
    ? V extends NonNullable<TypedTernaryExpression['valueCategory']>
      ? TypedTernaryExpression<T, V>
      : never
    : NonNullable<TypedTernaryExpression['type']> extends T
    ? V extends NonNullable<TypedTernaryExpression['valueCategory']>
      ? TypedTernaryExpression
      : never
    : never;
  assignment_expression: T extends NonNullable<TypedAssignmentExpression['type']>
    ? V extends NonNullable<TypedAssignmentExpression['valueCategory']>
      ? TypedAssignmentExpression<T>
      : never
    : NonNullable<TypedAssignmentExpression['type']> extends T
    ? V extends NonNullable<TypedAssignmentExpression['valueCategory']>
      ? TypedAssignmentExpression
      : never
    : never;
  compound_assignment_expression: T extends NonNullable<TypedCompoundAssignmentExpression['type']>
    ? V extends NonNullable<TypedCompoundAssignmentExpression['valueCategory']>
      ? TypedCompoundAssignmentExpression<T>
      : never
    : NonNullable<TypedCompoundAssignmentExpression['type']> extends T
    ? V extends NonNullable<TypedCompoundAssignmentExpression['valueCategory']>
      ? TypedCompoundAssignmentExpression
      : never
    : never;
  arithmetic_binary_operator_expression: T extends NonNullable<
    TypedArithmeticBinaryOperatorExpression['type']
  >
    ? V extends NonNullable<TypedArithmeticBinaryOperatorExpression['valueCategory']>
      ? TypedArithmeticBinaryOperatorExpression<T>
      : never
    : NonNullable<TypedArithmeticBinaryOperatorExpression['type']> extends T
    ? V extends NonNullable<TypedArithmeticBinaryOperatorExpression['valueCategory']>
      ? TypedArithmeticBinaryOperatorExpression<ArithmeticType>
      : never
    : never;

  pointer_difference_expression: T extends NonNullable<TypedPointerDifferenceExpression['type']>
    ? V extends NonNullable<TypedPointerDifferenceExpression['valueCategory']>
      ? TypedPointerDifferenceExpression
      : never
    : NonNullable<TypedPointerDifferenceExpression['type']> extends T
    ? V extends NonNullable<TypedPointerDifferenceExpression['valueCategory']>
      ? TypedPointerDifferenceExpression
      : never
    : never;
  pointer_offset_expression: T extends NonNullable<TypedPointerOffsetExpression['type']>
    ? V extends NonNullable<TypedPointerOffsetExpression['valueCategory']>
      ? TypedPointerOffsetExpression<T>
      : never
    : NonNullable<TypedPointerOffsetExpression['type']> extends T
    ? V extends NonNullable<TypedPointerOffsetExpression['valueCategory']>
      ? TypedPointerOffsetExpression
      : never
    : never;
  output_operator_expression: T extends NonNullable<TypedOutputOperatorExpression['type']>
    ? V extends NonNullable<TypedOutputOperatorExpression['valueCategory']>
      ? TypedOutputOperatorExpression
      : never
    : NonNullable<TypedOutputOperatorExpression['type']> extends T
    ? V extends NonNullable<TypedOutputOperatorExpression['valueCategory']>
      ? TypedOutputOperatorExpression
      : never
    : never;
  input_operator_expression: T extends NonNullable<TypedInputOperatorExpression['type']>
    ? V extends NonNullable<TypedInputOperatorExpression['valueCategory']>
      ? TypedInputOperatorExpression
      : never
    : NonNullable<TypedInputOperatorExpression['type']> extends T
    ? V extends NonNullable<TypedInputOperatorExpression['valueCategory']>
      ? TypedInputOperatorExpression
      : never
    : never;
  relational_binary_operator_expression: T extends NonNullable<
    TypedRelationalBinaryOperatorExpression['type']
  >
    ? V extends NonNullable<TypedRelationalBinaryOperatorExpression['valueCategory']>
      ? TypedRelationalBinaryOperatorExpression
      : never
    : NonNullable<TypedRelationalBinaryOperatorExpression['type']> extends T
    ? V extends NonNullable<TypedRelationalBinaryOperatorExpression['valueCategory']>
      ? TypedRelationalBinaryOperatorExpression
      : never
    : never;
  pointer_comparison_expression: T extends NonNullable<TypedPointerComparisonExpression['type']>
    ? V extends NonNullable<TypedPointerComparisonExpression['valueCategory']>
      ? TypedPointerComparisonExpression
      : never
    : NonNullable<TypedPointerComparisonExpression['type']> extends T
    ? V extends NonNullable<TypedPointerComparisonExpression['valueCategory']>
      ? TypedPointerComparisonExpression
      : never
    : never;
  logical_binary_operator_expression: T extends NonNullable<
    TypedLogicalBinaryOperatorExpression['type']
  >
    ? V extends NonNullable<TypedLogicalBinaryOperatorExpression['valueCategory']>
      ? TypedLogicalBinaryOperatorExpression
      : never
    : NonNullable<TypedLogicalBinaryOperatorExpression['type']> extends T
    ? V extends NonNullable<TypedLogicalBinaryOperatorExpression['valueCategory']>
      ? TypedLogicalBinaryOperatorExpression
      : never
    : never;
  non_member_operator_overload_expression: T extends NonNullable<
    TypedNonMemberOperatorOverloadExpression['type']
  >
    ? V extends NonNullable<TypedNonMemberOperatorOverloadExpression['valueCategory']>
      ? TypedNonMemberOperatorOverloadExpression<T, V>
      : never
    : NonNullable<TypedNonMemberOperatorOverloadExpression['type']> extends T
    ? V extends NonNullable<TypedNonMemberOperatorOverloadExpression['valueCategory']>
      ? TypedNonMemberOperatorOverloadExpression
      : never
    : never;
  member_operator_overload_expression: T extends NonNullable<
    TypedMemberOperatorOverloadExpression['type']
  >
    ? V extends NonNullable<TypedMemberOperatorOverloadExpression['valueCategory']>
      ? TypedMemberOperatorOverloadExpression<T, V>
      : never
    : NonNullable<TypedMemberOperatorOverloadExpression['type']> extends T
    ? V extends NonNullable<TypedMemberOperatorOverloadExpression['valueCategory']>
      ? TypedMemberOperatorOverloadExpression
      : never
    : never;
  prefix_increment_expression: T extends NonNullable<TypedPrefixIncrementExpression['type']>
    ? V extends NonNullable<TypedPrefixIncrementExpression['valueCategory']>
      ? TypedPrefixIncrementExpression<T>
      : never
    : NonNullable<TypedPrefixIncrementExpression['type']> extends T
    ? V extends NonNullable<TypedPrefixIncrementExpression['valueCategory']>
      ? TypedPrefixIncrementExpression
      : never
    : never;
  dereference_expression: T extends NonNullable<TypedDereferenceExpression['type']>
    ? V extends NonNullable<TypedDereferenceExpression['valueCategory']>
      ? TypedDereferenceExpression<T>
      : never
    : NonNullable<TypedDereferenceExpression['type']> extends T
    ? V extends NonNullable<TypedDereferenceExpression['valueCategory']>
      ? TypedDereferenceExpression
      : never
    : never;
  address_of_expression: T extends NonNullable<TypedAddressOfExpression['type']>
    ? V extends NonNullable<TypedAddressOfExpression['valueCategory']>
      ? TypedAddressOfExpression<T>
      : never
    : NonNullable<TypedAddressOfExpression['type']> extends T
    ? V extends NonNullable<TypedAddressOfExpression['valueCategory']>
      ? TypedAddressOfExpression
      : never
    : never;
  new_expression: T extends NonNullable<TypedNewExpression['type']>
    ? V extends NonNullable<TypedNewExpression['valueCategory']>
      ? TypedNewExpression<T>
      : never
    : NonNullable<TypedNewExpression['type']> extends T
    ? V extends NonNullable<TypedNewExpression['valueCategory']>
      ? TypedNewExpression
      : never
    : never;
  new_array_expression: T extends NonNullable<TypedNewArrayExpression['type']>
    ? V extends NonNullable<TypedNewArrayExpression['valueCategory']>
      ? TypedNewArrayExpression<T>
      : never
    : NonNullable<TypedNewArrayExpression['type']> extends T
    ? V extends NonNullable<TypedNewArrayExpression['valueCategory']>
      ? TypedNewArrayExpression
      : never
    : never;
  delete_expression: T extends NonNullable<TypedDeleteExpression['type']>
    ? V extends NonNullable<TypedDeleteExpression['valueCategory']>
      ? TypedDeleteExpression
      : never
    : NonNullable<TypedDeleteExpression['type']> extends T
    ? V extends NonNullable<TypedDeleteExpression['valueCategory']>
      ? TypedDeleteExpression
      : never
    : never;
  delete_array_expression: T extends NonNullable<TypedDeleteArrayExpression['type']>
    ? V extends NonNullable<TypedDeleteArrayExpression['valueCategory']>
      ? TypedDeleteArrayExpression
      : never
    : NonNullable<TypedDeleteArrayExpression['type']> extends T
    ? V extends NonNullable<TypedDeleteArrayExpression['valueCategory']>
      ? TypedDeleteArrayExpression
      : never
    : never;
  this_expression: T extends NonNullable<TypedThisExpression['type']>
    ? V extends NonNullable<TypedThisExpression['valueCategory']>
      ? TypedThisExpression<T>
      : never
    : NonNullable<TypedThisExpression['type']> extends T
    ? V extends NonNullable<TypedThisExpression['valueCategory']>
      ? TypedThisExpression
      : never
    : never;
  nullptr_expression: T extends NonNullable<TypedNullptrExpression['type']>
    ? V extends NonNullable<TypedNullptrExpression['valueCategory']>
      ? TypedNullptrExpression
      : never
    : NonNullable<TypedNullptrExpression['type']> extends T
    ? V extends NonNullable<TypedNullptrExpression['valueCategory']>
      ? TypedNullptrExpression
      : never
    : never;
  unary_plus_expression: T extends NonNullable<TypedUnaryPlusExpression['type']>
    ? V extends NonNullable<TypedUnaryPlusExpression['valueCategory']>
      ? TypedUnaryPlusExpression<T>
      : never
    : NonNullable<TypedUnaryPlusExpression['type']> extends T
    ? V extends NonNullable<TypedUnaryPlusExpression['valueCategory']>
      ? TypedUnaryPlusExpression
      : never
    : never;
  unary_minus_expression: T extends NonNullable<TypedUnaryMinusExpression['type']>
    ? V extends NonNullable<TypedUnaryMinusExpression['valueCategory']>
      ? TypedUnaryMinusExpression<T>
      : never
    : NonNullable<TypedUnaryMinusExpression['type']> extends T
    ? V extends NonNullable<TypedUnaryMinusExpression['valueCategory']>
      ? TypedUnaryMinusExpression
      : never
    : never;
  logical_not_expression: T extends NonNullable<TypedLogicalNotExpression['type']>
    ? V extends NonNullable<TypedLogicalNotExpression['valueCategory']>
      ? TypedLogicalNotExpression
      : never
    : NonNullable<TypedLogicalNotExpression['type']> extends T
    ? V extends NonNullable<TypedLogicalNotExpression['valueCategory']>
      ? TypedLogicalNotExpression
      : never
    : never;
  postfix_increment_expression: T extends NonNullable<TypedPostfixIncrementExpression['type']>
    ? V extends NonNullable<TypedPostfixIncrementExpression['valueCategory']>
      ? TypedPostfixIncrementExpression<T>
      : never
    : NonNullable<TypedPostfixIncrementExpression['type']> extends T
    ? V extends NonNullable<TypedPostfixIncrementExpression['valueCategory']>
      ? TypedPostfixIncrementExpression
      : never
    : never;
  subscript_expression: T extends NonNullable<TypedSubscriptExpression['type']>
    ? V extends NonNullable<TypedSubscriptExpression['valueCategory']>
      ? TypedSubscriptExpression<T>
      : never
    : NonNullable<TypedSubscriptExpression['type']> extends T
    ? V extends NonNullable<TypedSubscriptExpression['valueCategory']>
      ? TypedSubscriptExpression
      : never
    : never;

  dot_expression: V extends NonNullable<DotExpression['valueCategory']>
    ? T extends CompleteObjectType
      ? TypedObjectDotExpression<T>
      : T extends FunctionType
      ? TypedFunctionDotExpression<T>
      : CompleteObjectType extends T
      ? FunctionType extends T
        ? TypedObjectDotExpression | TypedFunctionDotExpression
        : never
      : never
    : never;

  arrow_expression: V extends NonNullable<ArrowExpression['valueCategory']>
    ? T extends CompleteObjectType
      ? TypedObjectArrowExpression<T>
      : T extends FunctionType
      ? TypedFunctionArrowExpression<T>
      : CompleteObjectType extends T
      ? FunctionType extends T
        ? TypedObjectArrowExpression | TypedFunctionArrowExpression
        : never
      : never
    : never;

  identifier_expression: V extends NonNullable<IdentifierExpression['valueCategory']>
    ? T extends CompleteObjectType
      ? TypedObjectIdentifierExpression<T>
      : T extends FunctionType
      ? TypedFunctionIdentifierExpression<T>
      : CompleteObjectType extends T
      ? FunctionType extends T
        ? TypedObjectIdentifierExpression | TypedFunctionIdentifierExpression
        : never
      : never
    : never;

  numeric_literal_expression: T extends NonNullable<TypedNumericLiteralExpression['type']>
    ? V extends NonNullable<TypedNumericLiteralExpression['valueCategory']>
      ? TypedNumericLiteralExpression<T>
      : never
    : NonNullable<TypedNumericLiteralExpression['type']> extends T
    ? V extends NonNullable<TypedNumericLiteralExpression['valueCategory']>
      ? TypedNumericLiteralExpression
      : never
    : never;
  string_literal_expression: T extends NonNullable<TypedStringLiteralExpression['type']>
    ? V extends NonNullable<TypedStringLiteralExpression['valueCategory']>
      ? TypedStringLiteralExpression
      : never
    : NonNullable<TypedStringLiteralExpression['type']> extends T
    ? V extends NonNullable<TypedStringLiteralExpression['valueCategory']>
      ? TypedStringLiteralExpression
      : never
    : never;
  parentheses_expression: T extends NonNullable<TypedParenthesesExpression['type']>
    ? V extends NonNullable<TypedParenthesesExpression['valueCategory']>
      ? TypedParenthesesExpression<T, V>
      : never
    : NonNullable<TypedParenthesesExpression['type']> extends T
    ? V extends NonNullable<TypedParenthesesExpression['valueCategory']>
      ? TypedParenthesesExpression
      : never
    : never;
  initializer_list_expression: T extends NonNullable<TypedInitializerListExpression['type']>
    ? V extends NonNullable<TypedInitializerListExpression['valueCategory']>
      ? TypedInitializerListExpression<T>
      : never
    : NonNullable<TypedInitializerListExpression['type']> extends T
    ? V extends NonNullable<TypedInitializerListExpression['valueCategory']>
      ? TypedInitializerListExpression
      : never
    : never;
  opaque_expression: T extends NonNullable<TypedOpaqueExpression['type']>
    ? V extends NonNullable<TypedOpaqueExpression['valueCategory']>
      ? TypedOpaqueExpression<T, V>
      : never
    : NonNullable<TypedOpaqueExpression['type']> extends T
    ? V extends NonNullable<TypedOpaqueExpression['valueCategory']>
      ? TypedOpaqueExpression
      : never
    : never;
  auxiliary_expression: T extends NonNullable<TypedAuxiliaryExpression['type']>
    ? V extends NonNullable<TypedAuxiliaryExpression['valueCategory']>
      ? TypedAuxiliaryExpression<T, V>
      : never
    : NonNullable<TypedAuxiliaryExpression['type']> extends T
    ? V extends NonNullable<TypedAuxiliaryExpression['valueCategory']>
      ? TypedAuxiliaryExpression
      : never
    : never;
  magic_function_call_expression: T extends NonNullable<TypedMagicFunctionCallExpression['type']>
    ? V extends NonNullable<TypedMagicFunctionCallExpression['valueCategory']>
      ? TypedMagicFunctionCallExpression<T, V>
      : never
    : NonNullable<TypedMagicFunctionCallExpression['type']> extends T
    ? V extends NonNullable<TypedMagicFunctionCallExpression['valueCategory']>
      ? TypedMagicFunctionCallExpression
      : never
    : never;
  function_call_expression: T extends NonNullable<TypedFunctionCallExpression['type']>
    ? V extends NonNullable<TypedFunctionCallExpression['valueCategory']>
      ? TypedFunctionCallExpression<T, V>
      : never
    : NonNullable<TypedFunctionCallExpression['type']> extends T
    ? V extends NonNullable<TypedFunctionCallExpression['valueCategory']>
      ? TypedFunctionCallExpression
      : never
    : never;
  ImplicitConversion: T extends NonNullable<TypedImplicitConversion['type']>
    ? V extends NonNullable<TypedImplicitConversion['valueCategory']>
      ? TypedImplicitConversion<CompleteObjectType, ValueCategory, T, V>
      : never
    : NonNullable<TypedImplicitConversion['type']> extends T
    ? V extends NonNullable<TypedImplicitConversion['valueCategory']>
      ? TypedImplicitConversion
      : never
    : never;
};
...
export type CompiledExpressionKinds<T extends ExpressionType, V extends ValueCategory> = {
  ...
  anything_construct: never;
  invalid_operator_overload_expression: never;
  comma_expression: T extends NonNullable<CompiledCommaExpression['type']>
    ? V extends NonNullable<CompiledCommaExpression['valueCategory']>
      ? CompiledCommaExpression<T, V>
      : never
    : never;
  ternary_expression: T extends NonNullable<CompiledTernaryExpression['type']>
    ? V extends NonNullable<CompiledTernaryExpression['valueCategory']>
      ? CompiledTernaryExpression<T, V>
      : never
    : never;
  assignment_expression: T extends NonNullable<CompiledAssignmentExpression['type']>
    ? V extends NonNullable<CompiledAssignmentExpression['valueCategory']>
      ? CompiledAssignmentExpression<T>
      : never
    : never;
  compound_assignment_expression: T extends NonNullable<
    CompiledCompoundAssignmentExpression['type']
  >
    ? V extends NonNullable<CompiledCompoundAssignmentExpression['valueCategory']>
      ? CompiledCompoundAssignmentExpression<T>
      : never
    : never;

  arithmetic_binary_operator_expression: T extends NonNullable<
    CompiledArithmeticBinaryOperatorExpression['type']
  >
    ? V extends NonNullable<CompiledArithmeticBinaryOperatorExpression['valueCategory']>
      ? CompiledArithmeticBinaryOperatorExpression<T>
      : never
    : NonNullable<ArithmeticBinaryOperatorExpression['type']> extends T
    ? V extends NonNullable<CompiledArithmeticBinaryOperatorExpression['valueCategory']>
      ? CompiledArithmeticBinaryOperatorExpression<ArithmeticType>
      : never
    : never;

  pointer_difference_expression: T extends NonNullable<CompiledPointerDifferenceExpression['type']>
    ? V extends NonNullable<CompiledPointerDifferenceExpression['valueCategory']>
      ? CompiledPointerDifferenceExpression
      : never
    : never;
  pointer_offset_expression: T extends NonNullable<CompiledPointerOffsetExpression['type']>
    ? V extends NonNullable<CompiledPointerOffsetExpression['valueCategory']>
      ? CompiledPointerOffsetExpression<T>
      : never
    : never;
  output_operator_expression: T extends NonNullable<CompiledOutputOperatorExpression['type']>
    ? V extends NonNullable<CompiledOutputOperatorExpression['valueCategory']>
      ? CompiledOutputOperatorExpression
      : never
    : never;
  input_operator_expression: T extends NonNullable<CompiledInputOperatorExpression['type']>
    ? V extends NonNullable<CompiledInputOperatorExpression['valueCategory']>
      ? CompiledInputOperatorExpression
      : never
    : never;
  relational_binary_operator_expression: T extends NonNullable<
    CompiledRelationalBinaryOperatorExpression['type']
  >
    ? V extends NonNullable<CompiledRelationalBinaryOperatorExpression['valueCategory']>
      ? CompiledRelationalBinaryOperatorExpression
      : never
    : never;
  pointer_comparison_expression: T extends NonNullable<CompiledPointerComparisonExpression['type']>
    ? V extends NonNullable<CompiledPointerComparisonExpression['valueCategory']>
      ? CompiledPointerComparisonExpression
      : never
    : never;
  logical_binary_operator_expression: T extends NonNullable<
    CompiledLogicalBinaryOperatorExpression['type']
  >
    ? V extends NonNullable<CompiledLogicalBinaryOperatorExpression['valueCategory']>
      ? CompiledLogicalBinaryOperatorExpression
      : never
    : never;
  non_member_operator_overload_expression: T extends NonNullable<
    CompiledNonMemberOperatorOverloadExpression['type']
  >
    ? V extends NonNullable<CompiledNonMemberOperatorOverloadExpression['valueCategory']>
      ? CompiledNonMemberOperatorOverloadExpression<T, V>
      : never
    : never;
  member_operator_overload_expression: T extends NonNullable<
    CompiledMemberOperatorOverloadExpression['type']
  >
    ? V extends NonNullable<CompiledMemberOperatorOverloadExpression['valueCategory']>
      ? CompiledMemberOperatorOverloadExpression<T, V>
      : never
    : never;
  prefix_increment_expression: T extends NonNullable<CompiledPrefixIncrementExpression['type']>
    ? V extends NonNullable<CompiledPrefixIncrementExpression['valueCategory']>
      ? CompiledPrefixIncrementExpression<T>
      : never
    : never;
  dereference_expression: T extends NonNullable<CompiledDereferenceExpression['type']>
    ? V extends NonNullable<CompiledDereferenceExpression['valueCategory']>
      ? CompiledDereferenceExpression<T>
      : never
    : never;
  address_of_expression: T extends NonNullable<CompiledAddressOfExpression['type']>
    ? V extends NonNullable<CompiledAddressOfExpression['valueCategory']>
      ? CompiledAddressOfExpression<T>
      : never
    : never;
  new_expression: T extends NonNullable<CompiledNewExpression['type']>
    ? V extends NonNullable<CompiledNewExpression['valueCategory']>
      ? CompiledNewExpression<T>
      : never
    : never;
  new_array_expression: T extends NonNullable<CompiledNewArrayExpression['type']>
    ? V extends NonNullable<CompiledNewArrayExpression['valueCategory']>
      ? CompiledNewArrayExpression<T>
      : never
    : never;
  delete_expression: T extends NonNullable<CompiledDeleteExpression['type']>
    ? V extends NonNullable<CompiledDeleteExpression['valueCategory']>
      ? CompiledDeleteExpression
      : never
    : never;
  delete_array_expression: T extends NonNullable<CompiledDeleteArrayExpression['type']>
    ? V extends NonNullable<CompiledDeleteArrayExpression['valueCategory']>
      ? CompiledDeleteArrayExpression
      : never
    : never;
  this_expression: T extends NonNullable<CompiledThisExpression['type']>
    ? V extends NonNullable<CompiledThisExpression['valueCategory']>
      ? CompiledThisExpression<T>
      : never
    : never;
  nullptr_expression: T extends NonNullable<CompiledNullptrExpression['type']>
    ? V extends NonNullable<CompiledNullptrExpression['valueCategory']>
      ? CompiledNullptrExpression
      : never
    : never;
  unary_plus_expression: T extends NonNullable<CompiledUnaryPlusExpression['type']>
    ? V extends NonNullable<CompiledUnaryPlusExpression['valueCategory']>
      ? CompiledUnaryPlusExpression<T>
      : never
    : never;
  unary_minus_expression: T extends NonNullable<CompiledUnaryMinusExpression['type']>
    ? V extends NonNullable<CompiledUnaryMinusExpression['valueCategory']>
      ? CompiledUnaryMinusExpression<T>
      : never
    : never;
  logical_not_expression: T extends NonNullable<CompiledLogicalNotExpression['type']>
    ? V extends NonNullable<CompiledLogicalNotExpression['valueCategory']>
      ? CompiledLogicalNotExpression
      : never
    : never;
  postfix_increment_expression: T extends NonNullable<CompiledPostfixIncrementExpression['type']>
    ? V extends NonNullable<CompiledPostfixIncrementExpression['valueCategory']>
      ? CompiledPrefixIncrementExpression<T>
      : never
    : never;
  subscript_expression: T extends NonNullable<CompiledSubscriptExpression['type']>
    ? V extends NonNullable<CompiledSubscriptExpression['valueCategory']>
      ? CompiledSubscriptExpression<T>
      : never
    : never;
  dot_expression: V extends NonNullable<DotExpression['valueCategory']>
    ? T extends CompleteObjectType
      ? CompiledObjectDotExpression<T>
      : T extends FunctionType
      ? CompiledFunctionDotExpression<T>
      : never
    : never;
  arrow_expression: V extends NonNullable<ArrowExpression['valueCategory']>
    ? T extends CompleteObjectType
      ? CompiledObjectArrowExpression<T>
      : T extends FunctionType
      ? CompiledFunctionArrowExpression<T>
      : never
    : never;
  identifier_expression: V extends NonNullable<IdentifierExpression['valueCategory']>
    ? T extends CompleteObjectType
      ? CompiledObjectIdentifierExpression<T>
      : T extends FunctionType
      ? CompiledFunctionIdentifierExpression<T>
      : never
    : never;
  numeric_literal_expression: T extends NonNullable<CompiledNumericLiteralExpression['type']>
    ? V extends NonNullable<CompiledNumericLiteralExpression['valueCategory']>
      ? CompiledNumericLiteralExpression<T>
      : never
    : never;
  string_literal_expression: T extends NonNullable<CompiledStringLiteralExpression['type']>
    ? V extends NonNullable<CompiledStringLiteralExpression['valueCategory']>
      ? CompiledStringLiteralExpression
      : never
    : never;
  parentheses_expression: T extends NonNullable<CompiledParenthesesExpression['type']>
    ? V extends NonNullable<CompiledParenthesesExpression['valueCategory']>
      ? CompiledParenthesesExpression<T, V>
      : never
    : never;
  initializer_list_expression: T extends NonNullable<CompiledInitializerListExpression['type']>
    ? V extends NonNullable<CompiledInitializerListExpression['valueCategory']>
      ? CompiledInitializerListExpression<T>
      : never
    : never;
  opaque_expression: T extends NonNullable<CompiledOpaqueExpression['type']>
    ? V extends NonNullable<CompiledOpaqueExpression['valueCategory']>
      ? CompiledOpaqueExpression<T, V>
      : never
    : never;
  auxiliary_expression: T extends NonNullable<CompiledAuxiliaryExpression['type']>
    ? V extends NonNullable<CompiledAuxiliaryExpression['valueCategory']>
      ? CompiledAuxiliaryExpression<T, V>
      : never
    : never;
  magic_function_call_expression: T extends NonNullable<CompiledMagicFunctionCallExpression['type']>
    ? V extends NonNullable<CompiledMagicFunctionCallExpression['valueCategory']>
      ? CompiledMagicFunctionCallExpression<T>
      : never
    : never;
  function_call_expression: T extends NonNullable<CompiledFunctionCallExpression['type']>
    ? V extends NonNullable<CompiledFunctionCallExpression['valueCategory']>
      ? CompiledFunctionCallExpression<T>
      : never
    : never;
  ImplicitConversion: T extends NonNullable<CompiledImplicitConversion['type']>
    ? V extends NonNullable<CompiledImplicitConversion['valueCategory']>
      ? CompiledImplicitConversion<T>
      : never
    : never;
};

export type AnalyticTypedExpression<
  C extends AnalyticExpression,
  T extends ExpressionType = NonNullable<C['type']>,
  V extends ValueCategory = NonNullable<C['valueCategory']>
> = TypedExpressionKinds<T, V>[C['construct_type']];
export type AnalyticCompiledExpression<
  C extends AnalyticExpression,
  T extends ExpressionType = NonNullable<C['type']>,
  V extends ValueCategory = NonNullable<C['valueCategory']>
> = CompiledExpressionKinds<T, V>[C['construct_type']];

const ExpressionConstructsRuntimeMap = {
  unsupported_expression: (construct: UnsupportedExpression, parent: RuntimeConstruct) => {
    throw new Error('Cannot create a runtime instance of an unsupported construct.');
  },
  anything_construct: (construct: AnythingExpression, parent: RuntimeConstruct) => {
    throw new Error('Cannot create a runtime instance of an "anything" placeholder construct.');
  },
  invalid_operator_overload_expression: (
    construct: UnsupportedExpression,
    parent: RuntimeConstruct
  ) => {
    throw new Error('Cannot create a runtime instance of an invalid operator overload expression.');
  },
  comma_expression: <T extends CompiledCommaExpression['type'], V extends ValueCategory>(
    construct: CompiledCommaExpression<T, V>,
    parent: RuntimeConstruct
  ) => new RuntimeComma(construct, parent),
  ternary_expression: <T extends CompiledTernaryExpression['type'], V extends ValueCategory>(
    construct: CompiledTernaryExpression<T, V>,
    parent: RuntimeConstruct
  ) => new RuntimeTernary(construct, parent),
  assignment_expression: <T extends CompiledAssignmentExpression['type']>(
    construct: CompiledAssignmentExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeAssignment(construct, parent),
  compound_assignment_expression: <T extends CompiledCompoundAssignmentExpression['type']>(
    construct: CompiledCompoundAssignmentExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeCompoundAssignment(construct, parent),
  arithmetic_binary_operator_expression: <
    T extends CompiledArithmeticBinaryOperatorExpression['type']
  >(
    construct: CompiledArithmeticBinaryOperatorExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeArithmeticBinaryOperator(construct, parent),
  pointer_difference_expression: (
    construct: CompiledPointerDifferenceExpression,
    parent: RuntimeConstruct
  ) => new RuntimePointerDifference(construct, parent),
  pointer_offset_expression: <T extends CompiledPointerOffsetExpression['type']>(
    construct: CompiledPointerOffsetExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimePointerOffset(construct, parent),
  output_operator_expression: (
    construct: CompiledOutputOperatorExpression,
    parent: RuntimeConstruct
  ) => new RuntimeOutputOperatorExpression(construct, parent),
  input_operator_expression: (
    construct: CompiledInputOperatorExpression,
    parent: RuntimeConstruct
  ) => new RuntimeInputOperatorExpression(construct, parent),
  relational_binary_operator_expression: <
    T extends CompiledRelationalBinaryOperatorExpression['type']
  >(
    construct: CompiledRelationalBinaryOperatorExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeRelationalBinaryOperator(construct, parent),
  pointer_comparison_expression: (
    construct: CompiledPointerComparisonExpression,
    parent: RuntimeConstruct
  ) => new RuntimePointerComparisonExpression(construct, parent),
  logical_binary_operator_expression: (
    construct: CompiledLogicalBinaryOperatorExpression,
    parent: RuntimeConstruct
  ) => new RuntimeLogicalBinaryOperatorExpression(construct, parent),
  non_member_operator_overload_expression: <
    T extends CompiledNonMemberOperatorOverloadExpression['type'],
    V extends ValueCategory
  >(
    construct: CompiledNonMemberOperatorOverloadExpression<T, V>,
    parent: RuntimeConstruct
  ) => new RuntimeNonMemberOperatorOverloadExpression(construct, parent),
  member_operator_overload_expression: <
    T extends CompiledMemberOperatorOverloadExpression['type'],
    V extends ValueCategory
  >(
    construct: CompiledMemberOperatorOverloadExpression<T, V>,
    parent: RuntimeConstruct
  ) => new RuntimeMemberOperatorOverloadExpression(construct, parent),
  prefix_increment_expression: <T extends CompiledPrefixIncrementExpression['type']>(
    construct: CompiledPrefixIncrementExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimePrefixIncrementExpression(construct, parent),
  dereference_expression: <T extends CompiledDereferenceExpression['type']>(
    construct: CompiledDereferenceExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeDereferenceExpression(construct, parent),
  address_of_expression: <T extends CompiledAddressOfExpression['type']>(
    construct: CompiledAddressOfExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeAddressOfExpression(construct, parent),
  new_expression: <T extends CompiledNewExpression['type']>(
    construct: CompiledNewExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeNewExpression(construct, parent),
  new_array_expression: <T extends CompiledNewArrayExpression['type']>(
    construct: CompiledNewArrayExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeNewArrayExpression(construct, parent),
  delete_expression: (construct: CompiledDeleteExpression, parent: RuntimeConstruct) =>
    new RuntimeDeleteExpression(construct, parent),
  delete_array_expression: (construct: CompiledDeleteArrayExpression, parent: RuntimeConstruct) =>
    new RuntimeDeleteArrayExpression(construct, parent),
  this_expression: <T extends CompiledThisExpression['type']>(
    construct: CompiledThisExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeThisExpression(construct, parent),
  nullptr_expression: (construct: CompiledNullptrExpression, parent: RuntimeConstruct) =>
    new RuntimeNullptrExpression(construct, parent),
  unary_plus_expression: <T extends CompiledUnaryPlusExpression['type']>(
    construct: CompiledUnaryPlusExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeUnaryPlusExpression(construct, parent),
  unary_minus_expression: <T extends CompiledUnaryMinusExpression['type']>(
    construct: CompiledUnaryMinusExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeUnaryMinusExpression(construct, parent),
  logical_not_expression: (construct: CompiledLogicalNotExpression, parent: RuntimeConstruct) =>
    new RuntimeLogicalNotExpression(construct, parent),
  postfix_increment_expression: <T extends CompiledPostfixIncrementExpression['type']>(
    construct: CompiledPostfixIncrementExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimePostfixIncrementExpression(construct, parent),
  subscript_expression: <T extends CompiledSubscriptExpression['type']>(
    construct: CompiledSubscriptExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeSubscriptExpression(construct, parent),
  dot_expression: (
    construct: CompiledObjectDotExpression | CompiledFunctionDotExpression,
    parent: RuntimeConstruct
  ) => {
  ...
  arrow_expression: (
    construct: CompiledObjectArrowExpression | CompiledFunctionArrowExpression,
    parent: RuntimeConstruct
  ) => {
    if (construct.entity instanceof FunctionEntity) {
  ...
  identifier_expression: (
    construct: CompiledObjectIdentifierExpression | CompiledFunctionIdentifierExpression,
    parent: RuntimeConstruct
  ) => {
    if (construct.entity instanceof FunctionEntity) {
  ...
  numeric_literal_expression: <T extends CompiledNumericLiteralExpression['type']>(
    construct: CompiledNumericLiteralExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeNumericLiteral(construct, parent),
  string_literal_expression: (
    construct: CompiledStringLiteralExpression,
    parent: RuntimeConstruct
  ) => new RuntimeStringLiteralExpression(construct, parent),
  parentheses_expression: <
    T extends CompiledParenthesesExpression['type'],
    V extends ValueCategory
  >(
    construct: CompiledParenthesesExpression<T, V>,
    parent: RuntimeConstruct
  ) => new RuntimeParentheses(construct, parent),
  initializer_list_expression: <T extends CompiledInitializerListExpression['type']>(
    construct: CompiledInitializerListExpression<T>,
    parent: RuntimeConstruct
  ) => new RuntimeInitializerListExpression(construct, parent),
  opaque_expression: <T extends CompiledOpaqueExpression['type'], V extends ValueCategory>(
    construct: CompiledOpaqueExpression<T, V>,
    parent: RuntimeConstruct
  ) => new RuntimeOpaqueExpression(construct, parent),
  auxiliary_expression: <
    T extends CompiledExpression['type'] = CompiledExpression['type'],
    V extends ValueCategory = ValueCategory
  >(
    construct: CompiledExpression<T, V>,
    parent: RuntimeConstruct
  ) => {
    throw new Error('Auxiliary expressions must never be instantiated at runtime.');
  },
  magic_function_call_expression: <RT extends CompiledMagicFunctionCallExpression['type']>(
    construct: CompiledMagicFunctionCallExpression<RT>,
    parent: RuntimeConstruct
  ) => new RuntimeMagicFunctionCallExpression(construct, parent),
  function_call_expression: <RT extends CompiledFunctionCallExpression['type']>(
    construct: CompiledFunctionCallExpression<RT>,
    parent: RuntimeConstruct
  ) => new RuntimeFunctionCallExpression(construct, parent),
  ImplicitConversion: <
    FromType extends CompleteObjectType,
    FromVC extends ValueCategory,
    ToType extends CompleteObjectType,
    ToVC extends ValueCategory
  >(
    construct: CompiledImplicitConversion<FromType, FromVC, ToType, ToVC>,
    parent: RuntimeConstruct
  ) => construct.createRuntimeExpression(parent),
};

export function createRuntimeExpression(
  construct: UnsupportedExpression,
  parent: RuntimeConstruct
): never;
export function createRuntimeExpression(
  construct: AnythingExpression,
  parent: RuntimeConstruct
): never;
export function createRuntimeExpression(
  construct: InvalidOperatorOverloadExpression,
  parent: RuntimeConstruct
): never;
export function createRuntimeExpression<T extends ExpressionType, V extends ValueCategory>(
  construct: CompiledCommaExpression<T, V>,
  parent: RuntimeConstruct
): RuntimeComma<T, V>;
export function createRuntimeExpression<T extends ExpressionType, V extends ValueCategory>(
...
export function createRuntimeExpression<T extends AtomicType>(
  construct: CompiledAssignmentExpression<T>,
  parent: RuntimeConstruct
): RuntimeAssignment<T>;
export function createRuntimeExpression<T extends ArithmeticType | PointerToCompleteObjectType>(
  construct: CompiledCompoundAssignmentExpression<T>,
  parent: RuntimeConstruct
): RuntimeCompoundAssignment<T>;
export function createRuntimeExpression<T extends ArithmeticType>(
  construct: CompiledArithmeticBinaryOperatorExpression<T>,
  parent: RuntimeConstruct
): RuntimeArithmeticBinaryOperator<T>;
export function createRuntimeExpression(
  construct: CompiledPointerDifferenceExpression,
  parent: RuntimeConstruct
): RuntimePointerDifference;
export function createRuntimeExpression<T extends PointerToCompleteObjectType>(
  construct: CompiledPointerOffsetExpression<T>,
  parent: RuntimeConstruct
): RuntimePointerOffset<T>;
export function createRuntimeExpression(
  construct: CompiledPointerOffsetExpression,
  parent: RuntimeConstruct
): RuntimePointerOffset;
export function createRuntimeExpression<T extends ArithmeticType>(
  construct: CompiledRelationalBinaryOperatorExpression<T>,
  parent: RuntimeConstruct
): RuntimeRelationalBinaryOperator<T>;
export function createRuntimeExpression(
  construct: CompiledPointerComparisonExpression,
  parent: RuntimeConstruct
...
export function createRuntimeExpression(
  construct: CompiledLogicalBinaryOperatorExpression,
  parent: RuntimeConstruct
): RuntimeLogicalBinaryOperatorExpression;
export function createRuntimeExpression(
  construct: CompiledNonMemberOperatorOverloadExpression,
  parent: RuntimeConstruct
): RuntimeNonMemberOperatorOverloadExpression;
export function createRuntimeExpression(
  construct: CompiledMemberOperatorOverloadExpression,
  parent: RuntimeConstruct
): RuntimeMemberOperatorOverloadExpression;
export function createRuntimeExpression(
  construct: CompiledOutputOperatorExpression,
  parent: RuntimeConstruct
): RuntimeOutputOperatorExpression;
export function createRuntimeExpression(
  construct: CompiledInputOperatorExpression,
  parent: RuntimeConstruct
): RuntimeInputOperatorExpression;
export function createRuntimeExpression<T extends ArithmeticType | PointerToCompleteObjectType>(
  construct: CompiledPrefixIncrementExpression<T>,
  parent: RuntimeConstruct
): RuntimePrefixIncrementExpression<T>;
export function createRuntimeExpression<T extends CompleteObjectType>(
  construct: CompiledDereferenceExpression<T>,
  parent: RuntimeConstruct
): RuntimeDereferenceExpression<T>;
export function createRuntimeExpression<T extends PointerType>(
  construct: CompiledAddressOfExpression<T>,
  parent: RuntimeConstruct
): RuntimeAddressOfExpression<T>;
export function createRuntimeExpression<T extends PointerType<NewObjectType>>(
  construct: CompiledNewExpression<T>,
  parent: RuntimeConstruct
): RuntimeNewExpression<T>;
export function createRuntimeExpression<T extends PointerType<ArrayElemType>>(
  construct: CompiledNewArrayExpression<T>,
  parent: RuntimeConstruct
): RuntimeNewArrayExpression<T>;
export function createRuntimeExpression(
  construct: CompiledDeleteArrayExpression,
  parent: RuntimeConstruct
): RuntimeDeleteArrayExpression;
export function createRuntimeExpression<T extends PointerType<CompleteClassType>>(
  construct: CompiledThisExpression<T>,
  parent: RuntimeConstruct
): RuntimeThisExpression<T>;
export function createRuntimeExpression(
  construct: CompiledNullptrExpression,
  parent: RuntimeConstruct
): RuntimeNullptrExpression;
export function createRuntimeExpression<T extends ArithmeticType | PointerType>(
  construct: CompiledUnaryPlusExpression<T>,
  parent: RuntimeConstruct
): RuntimeUnaryPlusExpression<T>;
export function createRuntimeExpression<T extends ArithmeticType>(
  construct: CompiledUnaryMinusExpression<T>,
  parent: RuntimeConstruct
): RuntimeUnaryMinusExpression<T>;
export function createRuntimeExpression(
  construct: CompiledLogicalNotExpression,
  parent: RuntimeConstruct
): RuntimeLogicalNotExpression;
export function createRuntimeExpression<T extends ArithmeticType | PointerToCompleteObjectType>(
  construct: CompiledPostfixIncrementExpression<T>,
  parent: RuntimeConstruct
): RuntimePostfixIncrementExpression<T>;
export function createRuntimeExpression<T extends CompleteObjectType>(
  construct: CompiledSubscriptExpression<T>,
  parent: RuntimeConstruct
): RuntimeSubscriptExpression<T>;
export function createRuntimeExpression(
  construct: CompiledObjectIdentifierExpression,
  parent: RuntimeConstruct
): RuntimeObjectIdentifierExpression;
export function createRuntimeExpression(
  construct: CompiledFunctionIdentifierExpression,
  parent: RuntimeConstruct
): RuntimeFunctionIdentifierExpression;
export function createRuntimeExpression(
  construct: CompiledObjectIdentifierExpression | CompiledFunctionIdentifierExpression,
  parent: RuntimeConstruct
): RuntimeObjectIdentifierExpression | RuntimeFunctionIdentifierExpression;
export function createRuntimeExpression(
  construct: CompiledObjectDotExpression,
  parent: RuntimeConstruct
): RuntimeObjectDotExpression;
export function createRuntimeExpression(
  construct: CompiledFunctionDotExpression,
  parent: RuntimeConstruct
): RuntimeFunctionDotExpression;
export function createRuntimeExpression(
  construct: CompiledObjectDotExpression | CompiledFunctionDotExpression,
  parent: RuntimeConstruct
): RuntimeObjectDotExpression | RuntimeFunctionDotExpression;
export function createRuntimeExpression(
  construct: CompiledObjectArrowExpression,
  parent: RuntimeConstruct
): RuntimeObjectArrowExpression;
export function createRuntimeExpression(
  construct: CompiledFunctionArrowExpression,
  parent: RuntimeConstruct
): RuntimeFunctionArrowExpression;
export function createRuntimeExpression(
  construct: CompiledObjectArrowExpression | CompiledFunctionArrowExpression,
  parent: RuntimeConstruct
): RuntimeObjectArrowExpression | RuntimeFunctionDotExpression;
export function createRuntimeExpression<T extends ArithmeticType>(
  construct: CompiledNumericLiteralExpression<T>,
  parent: RuntimeConstruct
): RuntimeNumericLiteral<T>;
export function createRuntimeExpression(
  construct: CompiledStringLiteralExpression,
  parent: RuntimeConstruct
): RuntimeStringLiteralExpression;
export function createRuntimeExpression<T extends ExpressionType, V extends ValueCategory>(
  construct: CompiledParenthesesExpression<T, V>,
  parent: RuntimeConstruct
): RuntimeParentheses<T, V>;
export function createRuntimeExpression<T extends CompleteClassType>(
  construct: CompiledInitializerListExpression<T>,
  parent: RuntimeConstruct
): RuntimeInitializerListExpression<T>;
export function createRuntimeExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
>(construct: AuxiliaryExpression<T, V>, parent: RuntimeConstruct): never;
export function createRuntimeExpression<RT extends PeelReference<CompleteReturnType>>(
  construct: CompiledMagicFunctionCallExpression<RT>,
  parent: RuntimeConstruct
): RuntimeMagicFunctionCallExpression<RT>;
export function createRuntimeExpression<RT extends PeelReference<CompleteReturnType>>(
  construct: CompiledFunctionCallExpression<RT>,
  parent: RuntimeConstruct
): RuntimeFunctionCallExpression<RT>;
export function createRuntimeExpression<
  FromType extends CompleteObjectType,
  FromVC extends ValueCategory,
  ToType extends CompleteObjectType,
...
>(
  construct: CompiledImplicitConversion<FromType, FromVC, ToType, ToVC>,
  parent: RuntimeConstruct
): RuntimeImplicitConversion<FromType, FromVC, ToType, ToVC>;
export function createRuntimeExpression<
  T extends ExpressionType,
  V extends ValueCategory,
  ConstructType extends AnalyticExpression,
  CompiledConstructType extends AnalyticCompiledExpression<ConstructType, T, V>
>(
  construct: CompiledConstructType,
  parent: RuntimeConstruct
): ReturnType<typeof ExpressionConstructsRuntimeMap[CompiledConstructType['construct_type']]>;
export function createRuntimeExpression<T extends ExpressionType, V extends ValueCategory>(
  construct: CompiledExpression<T, V>,
  parent: RuntimeConstruct
): RuntimeExpression<T, V>;
export function createRuntimeExpression<
  T extends ExpressionType,
  V extends ValueCategory,
  ConstructType extends CompiledExpression<T, V> | UnsupportedExpression
>(construct: ConstructType, parent: RuntimeConstruct): RuntimeExpression<T, V> {
...
export class UnsupportedExpression extends Expression<ExpressionASTNode> {
  ...
  public isSemanticallyEquivalent_impl(other: AnalyticConstruct, ec: SemanticContext): boolean {
    return (
      other.construct_type === this.construct_type && other.unsupportedName === this.unsupportedName
    );
  }
}
...
export abstract class SimpleRuntimeExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory,
  C extends CompiledExpression<T, V> = CompiledExpression<T, V>
> extends RuntimeExpression<T, V, C> {
  private index: 0 | 1 = 0;
...
export interface TypedCommaExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends CommaExpression, t_TypedExpression {
  readonly type: T;
  readonly valueCategory: V;
  readonly right: TypedExpression<T, V>;
}

export interface CompiledCommaExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends TypedCommaExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly left: CompiledExpression;
  readonly right: CompiledExpression<T, V>;
}

export class RuntimeComma<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends SimpleRuntimeExpression<T, V, CompiledCommaExpression<T, V>> {
...
export class TernaryExpression extends Expression<TernaryASTNode> {
  ...
  public createDefaultOutlet(
    this: CompiledTernaryExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedTernaryExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends TernaryExpression, t_TypedExpression {
  readonly type: T;
  readonly valueCategory: V;
...
export interface CompiledTernaryExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends TypedTernaryExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeTernary<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends RuntimeExpression<T, V, CompiledTernaryExpression<T, V>> {
  ...
  protected stepForwardImpl() {
    this.setEvalResult(
      this.condition.evalResult.rawValue ? this.then.evalResult : this.otherwise.evalResult
    );
    this.startCleanup();
  }
}
...
export class AssignmentExpression extends Expression<AssignmentExpressionASTNode> {
  ...
  public static createFromAST(
    ast: AssignmentExpressionASTNode,
    context: ExpressionContext
  ): AssignmentExpression | OperatorOverloadExpression {
    ...
    if (Predicates.isTypedExpression(lhs, isPotentiallyCompleteClassType)) {
      return (
        selectOperatorOverload(context, ast, '=', [lhs, rhs]) ??
  ...
  public createDefaultOutlet(
    this: CompiledAssignmentExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
  ...
  public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
    ...
    return {
      message:
        'The value of ' +
        (rhs.name || rhs.message) +
        ' will be assigned to ' +
        (lhs.name || lhs.message) +
        '.',
    };
  }
...
export interface TypedAssignmentExpression<T extends AtomicType = AtomicType>
...
export interface CompiledAssignmentExpression<T extends AtomicType = AtomicType>
  extends TypedAssignmentExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeAssignment<T extends AtomicType = AtomicType> extends SimpleRuntimeExpression<
  T,
  'lvalue',
  CompiledAssignmentExpression<T>
> {
...
export class CompoundAssignmentExpression extends Expression<CompoundAssignmentExpressionASTNode> {
  ...
  private constructor(
    ...
    if (
      (this.equivalentBinaryOp === '+' || this.equivalentBinaryOp === '-') &&
      Predicates.isTypedExpression(lhs, isPointerToCompleteObjectType) &&
      Predicates.isTypedExpression(rhs, isIntegralType)
    ) {
    ...
    // otherwise it's an arithmetic binary operation
      ...
      if (
        (this.equivalentBinaryOp === '%' ||
          this.equivalentBinaryOp === '<<' ||
          this.equivalentBinaryOp == '>>') &&
        (!Predicates.isTypedExpression(lhs, isIntegralType) ||
          !Predicates.isTypedExpression(rhs, isIntegralType))
      ) {
      ...
      if (
        !Predicates.isTypedExpression(lhs, isArithmeticType) ||
        !Predicates.isTypedExpression(rhs, isArithmeticType)
      ) {
  ...
  public static createFromAST(
    ast: CompoundAssignmentExpressionASTNode,
    context: ExpressionContext
  ): CompoundAssignmentExpression | OperatorOverloadExpression {
    let lhs = createExpressionFromAST(ast.lhs, context);
    let rhs = createExpressionFromAST(ast.rhs, context);
    ...
    if (Predicates.isTypedExpression(lhs, isPotentiallyCompleteClassType)) {
      return (
        selectOperatorOverload(context, ast, ast.operator, [lhs, rhs]) ??
        new InvalidOperatorOverloadExpression(context, ast, ast.operator, [lhs, rhs])
      );
    }
  ...
  public createDefaultOutlet(
    this: CompiledCompoundAssignmentExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
  ...
  public explain(sim: Simulation, rtConstruct: RuntimeConstruct) {
    ...
    var rhs = this.rhs.describeEvalResult(0);
    return {
      message: `The value of ${lhs.name || lhs.message} ${this.equivalentBinaryOp} ${rhs.name ||
        rhs.message} will be assigned to ${lhs.name || lhs.message}.`,
    };
  }
...
export interface TypedCompoundAssignmentExpression<
  T extends PointerToCompleteObjectType | ArithmeticType =
    | PointerToCompleteObjectType
    | ArithmeticType
> extends CompoundAssignmentExpression, t_TypedExpression {
  readonly type: T;
  readonly lhs: TypedExpression<T>;
}

export interface CompiledCompoundAssignmentExpression<
  T extends PointerToCompleteObjectType | ArithmeticType =
    | PointerToCompleteObjectType
    | ArithmeticType
> extends TypedCompoundAssignmentExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly lhs: CompiledExpression<T, 'lvalue'>;
  readonly rhs: CompiledExpression<T, 'prvalue'>;
}

export class RuntimeCompoundAssignment<
  T extends PointerToCompleteObjectType | ArithmeticType =
    | PointerToCompleteObjectType
    | ArithmeticType
> extends SimpleRuntimeExpression<T, 'lvalue', CompiledCompoundAssignmentExpression<T>> {
  public readonly lhs: RuntimeExpression<T, 'lvalue'>;
  public readonly rhs: RuntimeExpression<T, 'prvalue'>;
  ...
  protected operate() {
    let lhsObj: CPPObject<ArithmeticType> | CPPObject<PointerToCompleteObjectType> = this.lhs
      .evalResult;

    if (lhsObj.isTyped(isArithmeticType)) {
      let newVal = ARITHMETIC_BINARY_OPERATIONS[this.model.equivalentBinaryOp](
        lhsObj.getValue(),
        <Value<ArithmeticType>>this.rhs.evalResult
      );
...
abstract class BinaryOperatorExpression<
  ASTType extends BinaryOperatorExpressionASTNode = BinaryOperatorExpressionASTNode
> extends Expression<ASTType> {
  ...
  protected constructor(
    context: ExpressionContext,
    ast: ASTType | undefined,
    operator: t_BinaryOperators
  ) {
    super(context, ast);
  ...
  public createDefaultOutlet(
    this: CompiledBinaryOperatorExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedBinaryOperatorExpression<T extends AtomicType = AtomicType>
...
export interface CompiledBinaryOperatorExpression<T extends AtomicType = AtomicType>
  extends TypedBinaryOperatorExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export interface RuntimeBinaryOperator
  extends RuntimeExpression<AtomicType, 'prvalue', CompiledBinaryOperatorExpression<AtomicType>> {
...
function mult(left: number, right: number) {
  return left * right;
}
function intDiv(left: number, right: number) {
  return Math.trunc(left / right);
}
function floatDiv(left: number, right: number) {
  return left / right;
}
function mod(left: number, right: number) {
  return left - intDiv(left, right) * right;
}
function bitAnd(left: number, right: number) {
  return left & right;
}
function bitXor(left: number, right: number) {
  return left ^ right;
}
function bitOr(left: number, right: number) {
  return left | right;
}
function bitShiftLeft(left: number, right: number) {
  return left << right;
}
function bitShiftRight(left: number, right: number) {
  return left >>> right;
} // TODO: is the sign preserving bit shift right more consistent with C++?
...
const ARITHMETIC_BINARY_OPERATIONS: {
  [k in t_ArithmeticBinaryOperators]: <T extends AtomicType>(
    left: Value<T>,
    right: Value<T>
  ) => Value<T>;
} = {
...
> {
  ...
  protected constructor(
    ...
    left: Expression,
    right: Expression,
    operator: t_ArithmeticBinaryOperators
  ) {
    ...
    if (
      !Predicates.isTypedExpression(left, isArithmeticType) ||
      !Predicates.isTypedExpression(right, isArithmeticType)
    ) {
    ...
    if (
      ...
      (!Predicates.isTypedExpression(left, isIntegralType) ||
        !Predicates.isTypedExpression(right, isIntegralType))
    ) {
    ...
    if (sameType(convertedLeft.type!, convertedRight.type!)) {
      ...
      this.addNote(
        CPPError.expr.invalid_binary_operands(this, this.operator, convertedLeft, convertedRight)
      );
    }
  ...
  public static createFromAST(
    ...
    if (
      Predicates.isTypedExpression(left, isPotentiallyCompleteClassType) ||
      Predicates.isTypedExpression(right, isPotentiallyCompleteClassType)
    ) {
    ...
    if (
      op === '-' &&
      (Predicates.isTypedExpression(left, isPointerType) ||
        Predicates.isTypedExpression(left, isBoundedArrayType, 'lvalue')) &&
      (Predicates.isTypedExpression(right, isPointerType) ||
        Predicates.isTypedExpression(right, isBoundedArrayType, 'lvalue'))
    ) {
      return new PointerDifferenceExpression(
        context,
        ast,
    ...
    if (op === '-' || op === '+') {
      if (
        ...
        return new PointerOffsetExpression(
          context,
          ast,
          <TypedExpression<PointerType, 'prvalue'> | TypedExpression<IntegralType, 'prvalue'>>(
            convertToPRValue(left)
          ),
          <TypedExpression<PointerType, 'prvalue'> | TypedExpression<IntegralType, 'prvalue'>>(
            convertToPRValue(right)
...
export interface TypedArithmeticBinaryOperatorExpression<T extends ArithmeticType = ArithmeticType>
...
export interface CompiledArithmeticBinaryOperatorExpression<
  T extends ArithmeticType = ArithmeticType
> extends TypedArithmeticBinaryOperatorExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeArithmeticBinaryOperator<
  T extends ArithmeticType = ArithmeticType
> extends SimpleRuntimeExpression<T, 'prvalue', CompiledArithmeticBinaryOperatorExpression<T>> {
  public readonly left: RuntimeExpression<T, 'prvalue'>;
  public readonly right: RuntimeExpression<T, 'prvalue'>;

  public constructor(
    model: CompiledArithmeticBinaryOperatorExpression<T>,
    parent: RuntimeConstruct
  ) {
    super(model, parent);
    this.left = createRuntimeExpression(this.model.left, this);
  ...
  public operate() {
    ...
    this.setEvalResult(
      <VCResultTypes<T, 'prvalue'>>(
        ARITHMETIC_BINARY_OPERATIONS[this.model.operator](
          this.left.evalResult,
          this.right.evalResult
        )
      )
    );
  }
...
> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: ArithmeticBinaryOperatorExpressionASTNode,
    left: TypedExpression<PointerType, 'prvalue'>,
    right: TypedExpression<PointerType, 'prvalue'>
  ) {
    super(context, ast, '-');
...
export interface TypedPointerDifferenceExpression
  extends PointerDifferenceExpression,
    t_TypedExpression {}

export interface CompiledPointerDifferenceExpression
  extends TypedPointerDifferenceExpression,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimePointerDifference extends SimpleRuntimeExpression<
  Int,
  'prvalue',
  CompiledPointerDifferenceExpression
> {
  ...
  public operate() {
    ...
    let leftArr = this.left.model.type.isType(ArrayPointerType)
      ? this.left.model.type.arrayObject
      : null;
    let rightArr = this.right.model.type.isType(ArrayPointerType)
      ? this.right.model.type.arrayObject
      : null;
    ...
    if (result.rawEquals(0)) {
      ...
      this.sim.eventOccurred(
        ...
        'The left pointer in this subtraction is not from an array, so the resulting difference is not meaningful.',
        true
      );
      ...
      this.sim.eventOccurred(
        SimulationEvent.UNDEFINED_BEHAVIOR,
        'The right pointer in this subtraction is not from an array, so the resulting difference is not meaningful.',
        true
      );
      result = result.invalidated();
      ...
      this.sim.eventOccurred(
        SimulationEvent.UNDEFINED_BEHAVIOR,
        'The pointers in this subtraction are pointing into two different arrays, so the resulting difference is not meaningful.',
        true
      );
      result = result.invalidated();
    }
...
> {
  ...
  public readonly left:
    | TypedExpression<PointerType, 'prvalue'>
    | TypedExpression<IntegralType, 'prvalue'>;
...
export interface TypedPointerOffsetExpression<T extends PointerType = PointerType>
  extends PointerOffsetExpression,
    t_TypedExpression {
  readonly type: T;
...
export interface CompiledPointerOffsetExpression<
  T extends PointerToCompleteObjectType = PointerToCompleteObjectType
> extends TypedPointerOffsetExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimePointerOffset<
  T extends PointerToCompleteObjectType = PointerToCompleteObjectType
> extends SimpleRuntimeExpression<T, 'prvalue', CompiledPointerOffsetExpression<T>> {
  public readonly left:
    | RuntimeExpression<T, 'prvalue'>
    | RuntimeExpression<IntegralType, 'prvalue'>;
  public readonly right:
    | RuntimeExpression<T, 'prvalue'>
    | RuntimeExpression<IntegralType, 'prvalue'>;
  ...
  public operate() {
    ...
    let result = this.pointer.evalResult.pointerOffset(
      this.offset.evalResult,
      this.model.operator === '-'
    );
    ...
    if (resultType.isType(ArrayPointerType)) {
      ...
      this.sim.eventOccurred(
        SimulationEvent.UNDEFINED_BEHAVIOR,
        "Uh, I don't think you're supposed to do arithmetic with that pointer. It's not pointing into an array.",
        true
      );
    }
  }
...
export class OutputOperatorExpression extends Expression<
  ArithmeticBinaryOperatorExpressionASTNode
> {
  // TODO: change to special Output AST?
  ...
  public constructor(
    ...
    if (right.isStringLiteralExpression()) {
      ...
      Predicates.isTypedExpression(right, isAtomicType) ||
      Predicates.isTypedExpression(right, isBoundedArrayType)
    ) {
  ...
  public createDefaultOutlet(
    this: CompiledOutputOperatorExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedOutputOperatorExpression
  extends OutputOperatorExpression,
    t_TypedExpression {}

export interface CompiledOutputOperatorExpression
  extends TypedOutputOperatorExpression,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeOutputOperatorExpression extends SimpleRuntimeExpression<
  PotentiallyCompleteClassType,
  'lvalue',
  CompiledOutputOperatorExpression
> {
  ...
  public operate() {
    if (this.right instanceof RuntimeStringLiteralExpression) {
      this.sim.cout(
        new Value(this.right.evalResult.address, new ArrayPointerType(this.right.evalResult))
      );
    } else {
...
export class InputOperatorExpression extends Expression<ArithmeticBinaryOperatorExpressionASTNode> {
  // TODO: change to special Input AST?
  ...
  public createDefaultOutlet(
    this: CompiledInputOperatorExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface CompiledInputOperatorExpression
  extends TypedInputOperatorExpression,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeInputOperatorExpression extends RuntimeExpression<
  PotentiallyCompleteClassType,
  'lvalue',
  CompiledInputOperatorExpression
> {
...
function lt(left: number, right: number) {
  return left < right;
}
function gt(left: number, right: number) {
  return left > right;
}
function lte(left: number, right: number) {
  return left <= right;
}
function gte(left: number, right: number) {
  return left >= right;
}
function eq(left: number, right: number) {
  return left == right;
}
function ne(left: number, right: number) {
  return left != right;
}

const RELATIONAL_BINARY_OPERATIONS: {
  [k in t_RelationalBinaryOperators]: <T extends AtomicType>(
    left: Value<T>,
    right: Value<T>
  ) => Value<Bool>;
} = {
...
> {
  ...
  protected constructor(
    ...
    left: Expression,
    right: Expression,
    operator: t_RelationalBinaryOperators
  ) {
    super(context, ast, operator);
  ...
  public static createFromAST(
    ast: RelationalBinaryOperatorExpressionASTNode,
    context: ExpressionContext
  ): RelationalBinaryOperatorExpression | PointerComparisonExpression | OperatorOverloadExpression {
    let left: Expression = createExpressionFromAST(ast.left, context);
    let right: Expression = createExpressionFromAST(ast.right, context);
    let op = ast.operator;
    ...
    if (
      ...
      if (
        ...
        return new PointerComparisonExpression(
          context,
          ast,
          convertToPRValue(left),
          convertToPRValue(right),
        ...
        return new PointerComparisonExpression(
          ...
          ast,
          convertedLeft,
          new NullPointerConversion(right, convertedLeft.type),
          op
        );
      }
    } else if (
      isIntegerLiteralZero(left) &&
      (Predicates.isTypedExpression(right, isPointerType) ||
        Predicates.isTypedExpression(right, isBoundedArrayType, 'lvalue'))
    ) {
      ...
      return new PointerComparisonExpression(
        context,
        ast,
        new NullPointerConversion(left, convertedRight.type),
        convertedRight,
        op
      );
    }
...
export interface TypedRelationalBinaryOperatorExpression
  extends RelationalBinaryOperatorExpression,
    t_TypedExpression {}

export interface CompiledRelationalBinaryOperatorExpression<
  OperandT extends ArithmeticType = ArithmeticType
> extends TypedRelationalBinaryOperatorExpression, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeRelationalBinaryOperator<
  OperandT extends ArithmeticType = ArithmeticType
> extends SimpleRuntimeExpression<
  Bool,
  'prvalue',
  CompiledRelationalBinaryOperatorExpression<OperandT>
> {
  public readonly left: RuntimeExpression<OperandT, 'prvalue'>;
  public readonly right: RuntimeExpression<OperandT, 'prvalue'>;

  public constructor(
    model: CompiledRelationalBinaryOperatorExpression<OperandT>,
    parent: RuntimeConstruct
  ) {
    super(model, parent);
    this.left = createRuntimeExpression(this.model.left, this);
  ...
  public operate() {
    // Not sure why the cast here is necessary but apparently Typescript needs it
    this.setEvalResult(
      RELATIONAL_BINARY_OPERATIONS[this.model.operator](this.left.evalResult, this.right.evalResult)
    );
  }
}
...
> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: RelationalBinaryOperatorExpressionASTNode | undefined,
    left: TypedExpression<PointerType, 'prvalue'>,
    right: TypedExpression<PointerType, 'prvalue'>,
    operator: t_RelationalBinaryOperators
  ) {
    ...
    if (
      !(
        similarType(left.type, right.type) ||
        subType(left.type, right.type) ||
        subType(right.type, left.type)
      )
    ) {
  ...
  public isSemanticallyEquivalent_impl(other: AnalyticConstruct, ec: SemanticContext): boolean {
    if (other.construct_type === this.construct_type) {
      return other.operator === this.operator && this.areChildrenSemanticallyEquivalent(other, ec);
    }
...
export interface TypedPointerComparisonExpression
  extends PointerComparisonExpression,
    t_TypedExpression {}

export interface CompiledPointerComparisonExpression<OperandT extends PointerType = PointerType>
  extends TypedPointerComparisonExpression,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimePointerComparisonExpression<
  OperandT extends PointerType = PointerType
> extends SimpleRuntimeExpression<Bool, 'prvalue', CompiledPointerComparisonExpression<OperandT>> {
  public left: RuntimeExpression<OperandT, 'prvalue'>;
  public right: RuntimeExpression<OperandT, 'prvalue'>;

  public constructor(
    model: CompiledPointerComparisonExpression<OperandT>,
    parent: RuntimeConstruct
  ) {
    super(model, parent);
    this.left = createRuntimeExpression(this.model.left, this);
  ...
  public operate() {
    ...
    if (this.model.operator !== '==' && this.model.operator !== '!=') {
      if (
        !leftResult.type.isArrayPointerType() ||
        !rightResult.type.isArrayPointerType() ||
        leftResult.type.arrayObject !== rightResult.type.arrayObject
      ) {
        this.sim.eventOccurred(
          SimulationEvent.UNSPECIFIED_BEHAVIOR,
          "It looks like you're trying to see which pointer comes before/after in memory, but this only makes sense if both pointers come from the same array. I don't think that's the case here.",
          true
        );
      }
    }

    let result = RELATIONAL_BINARY_OPERATIONS[this.model.operator](
      this.left.evalResult,
      this.right.evalResult
    );
    this.setEvalResult(result);
...
> {
  ...
  protected constructor(
    context: ExpressionContext,
    ast: LogicalBinaryOperatorExpressionASTNode | undefined,
    left: Expression,
    right: Expression,
    operator: t_LogicalBinaryOperators
  ) {
    super(context, ast, operator);
  ...
  public static createFromAST(
    ast: LogicalBinaryOperatorExpressionASTNode,
    context: ExpressionContext
  ): LogicalBinaryOperatorExpression | OperatorOverloadExpression {
...
export interface TypedLogicalBinaryOperatorExpression
  extends LogicalBinaryOperatorExpression,
    t_TypedExpression {}

export interface CompiledLogicalBinaryOperatorExpression
  extends TypedLogicalBinaryOperatorExpression,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeLogicalBinaryOperatorExpression extends RuntimeExpression<
  Bool,
  'prvalue',
  CompiledLogicalBinaryOperatorExpression
> {
...
abstract class UnaryOperatorExpression<
  ASTType extends UnaryOperatorExpressionASTNode = UnaryOperatorExpressionASTNode
> extends Expression<ASTType> {
  ...
  public createDefaultOutlet(
    this: CompiledUnaryOperatorExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedUnaryOperatorExpression<
  T extends CompleteObjectType | VoidType = CompleteObjectType | VoidType,
  V extends ValueCategory = ValueCategory
> extends UnaryOperatorExpression, t_TypedExpression {
  readonly type: T;
  readonly valueCategory: ValueCategory;
}

export interface CompiledUnaryOperatorExpression<
  T extends CompleteObjectType | VoidType = CompleteObjectType | VoidType,
  V extends ValueCategory = ValueCategory
> extends TypedUnaryOperatorExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly operand: CompiledExpression;
}

export interface RuntimeUnaryOperatorExpression
  extends RuntimeExpression<
    CompleteObjectType | VoidType,
    ValueCategory,
    CompiledUnaryOperatorExpression<CompleteObjectType | VoidType>
  > {
  readonly operand: RuntimeExpression;
}

export class PrefixIncrementExpression extends UnaryOperatorExpression<
  PrefixIncrementExpressionASTNode
> {
  ...
  public constructor(
    ...
    if (!operand.isLvalue()) {
      ...
      Predicates.isTypedExpression(operand, isArithmeticType) ||
      Predicates.isTypedExpression(operand, isPointerToCompleteObjectType)
    ) {
...
export interface TypedPrefixIncrementExpression<
  T extends ArithmeticType | PointerToCompleteObjectType =
    | ArithmeticType
    | PointerToCompleteObjectType
> extends PrefixIncrementExpression, t_TypedExpression {
  readonly type: T;
  readonly operand: TypedExpression<T, 'lvalue'>;
}

export interface CompiledPrefixIncrementExpression<
  T extends ArithmeticType | PointerToCompleteObjectType =
    | ArithmeticType
    | PointerToCompleteObjectType
> extends TypedPrefixIncrementExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure

  readonly operand: CompiledExpression<T, 'lvalue'>;
}

export class RuntimePrefixIncrementExpression<
  T extends ArithmeticType | PointerToCompleteObjectType
> extends SimpleRuntimeExpression<T, 'lvalue', CompiledPrefixIncrementExpression<T>> {
  ...
  protected operate() {
    ...
    let newValue = prevValue.isTyped(isType(Bool))
      ? new Value(1, Bool.BOOL)
      : prevValue.isTyped(isArithmeticType)
      ? prevValue.modify(v => v + delta)
...
export class DereferenceExpression extends UnaryOperatorExpression<DereferenceExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: DereferenceExpressionASTNode,
    operand: Expression
  ) {
    super(context, ast);
...
export interface TypedDereferenceExpression<T extends CompleteObjectType = CompleteObjectType>
  extends DereferenceExpression,
    t_TypedExpression {
  readonly type: T;
  readonly operand: TypedExpression<PointerType<T>, 'prvalue'>;
}

export interface CompiledDereferenceExpression<T extends CompleteObjectType = CompleteObjectType>
  extends TypedDereferenceExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeDereferenceExpression<
  T extends CompleteObjectType
> extends SimpleRuntimeExpression<T, 'lvalue', CompiledDereferenceExpression<T>> {
  ...
  protected operate() {
    ...
    if (PointerType.isNull(addr)) {
      this.sim.eventOccurred(
        ...
        'Ow! Your code just dereferenced a null pointer!',
        true
      );
    } else if (PointerType.isNegative(addr)) {
      this.sim.eventOccurred(
        SimulationEvent.CRASH,
        "Uh, wow. The pointer you're trying to dereference has a negative address.\nThanks a lot.",
        true
      );
      ...
      if (addr < ptr.type.min()) {
        this.sim.eventOccurred(
          SimulationEvent.UNDEFINED_BEHAVIOR,
          'That pointer has wandered off the beginning of its array. Dereferencing it might cause a segfault, or worse - you might just access/change other memory outside the array.',
          true
        );
      } else if (ptr.type.onePast() < addr) {
        this.sim.eventOccurred(
          SimulationEvent.UNDEFINED_BEHAVIOR,
          'That pointer has wandered off the end of its array. Dereferencing it might cause a segfault, or worse - you might just access/change other memory outside the array.',
          true
        );
        ...
        this.sim.eventOccurred(
          SimulationEvent.UNDEFINED_BEHAVIOR,
          'That pointer is one past the end of its array. Do you have an off-by-one error?. Dereferencing it might cause a segfault, or worse - you might just access/change other memory outside the array.',
          true
        );
      }
    }
...
export class AddressOfExpression extends UnaryOperatorExpression<AddressOfExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: AddressOfExpressionASTNode,
    operand: Expression
  ) {
    super(context, ast);
...
export interface TypedAddressOfExpression<T extends PointerType = PointerType>
  extends AddressOfExpression,
    t_TypedExpression {
  readonly type: T;
  readonly operand: TypedExpression<T['ptrTo']>;
}

export interface CompiledAddressOfExpression<T extends PointerType = PointerType>
  extends TypedAddressOfExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeAddressOfExpression<T extends PointerType> extends SimpleRuntimeExpression<
  T,
  'prvalue',
  CompiledAddressOfExpression<T>
> {
...
export class UnaryPlusExpression extends UnaryOperatorExpression<UnaryPlusExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: UnaryPlusExpressionASTNode,
    operand: Expression
  ) {
    super(context, ast);
...
export interface TypedUnaryPlusExpression<
  T extends ArithmeticType | PointerType = ArithmeticType | PointerType
> extends UnaryPlusExpression, t_TypedExpression {
  readonly type: T;
  readonly operand: TypedExpression<T, 'prvalue'>;
}

export interface CompiledUnaryPlusExpression<
  T extends ArithmeticType | PointerType = ArithmeticType | PointerType
> extends TypedUnaryPlusExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure

  readonly operand: CompiledExpression<T, 'prvalue'>;
}

export class RuntimeUnaryPlusExpression<
  T extends ArithmeticType | PointerType
> extends SimpleRuntimeExpression<T, 'prvalue', CompiledUnaryPlusExpression<T>> {
...
export class UnaryMinusExpression extends UnaryOperatorExpression<UnaryMinusExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: UnaryMinusExpressionASTNode,
    operand: Expression
  ) {
    super(context, ast);
...
export interface TypedUnaryMinusExpression<T extends ArithmeticType = ArithmeticType>
  extends UnaryMinusExpression,
    t_TypedExpression {
  readonly type: T;
  readonly operand: TypedExpression<T, 'prvalue'>;
}

export interface CompiledUnaryMinusExpression<T extends ArithmeticType = ArithmeticType>
  extends TypedUnaryMinusExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeUnaryMinusExpression<T extends ArithmeticType> extends SimpleRuntimeExpression<
  T,
  'prvalue',
  CompiledUnaryMinusExpression<T>
> {
...
export class LogicalNotExpression extends UnaryOperatorExpression<LogicalNotExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: LogicalNotExpressionASTNode,
    operand: Expression
  ) {
    super(context, ast);
...
export interface CompiledLogicalNotExpression
  extends TypedLogicalNotExpression,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeLogicalNotExpression extends SimpleRuntimeExpression<
  Bool,
  'prvalue',
  CompiledLogicalNotExpression
> {
...
export class SubscriptExpression extends Expression<SubscriptExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: SubscriptExpressionASTNode,
    operand: Expression,
    offset: Expression
  ) {
    super(context, ast);
    ...
    this.attach(
      (this.offset = offset.isWellTyped() ? standardConversion(offset, Int.INT) : offset)
    );
  ...
  public static createFromAST(
    ast: SubscriptExpressionASTNode,
    context: ExpressionContext
  ): SubscriptExpression | OperatorOverloadExpression {
    ...
    if (Predicates.isTypedExpression(operand, isPotentiallyCompleteClassType)) {
      return (
        selectOperatorOverload(context, ast, '[]', [operand, offset]) ??
  ...
  public createDefaultOutlet(
    this: CompiledSubscriptExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedSubscriptExpression<T extends CompleteObjectType = CompleteObjectType>
  extends SubscriptExpression,
    t_TypedExpression {
  readonly type: T;
...
export interface CompiledSubscriptExpression<T extends CompleteObjectType = CompleteObjectType>
  extends TypedSubscriptExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeSubscriptExpression<
  T extends CompleteObjectType = CompleteObjectType
> extends SimpleRuntimeExpression<T, 'lvalue', CompiledSubscriptExpression<T>> {
  public operand: RuntimeExpression<PointerType<T>, 'prvalue'>;
  ...
  protected operate() {
    ...
    if (PointerType.isNegative(addr)) {
      this.sim.eventOccurred(
        SimulationEvent.CRASH,
        'Good work. You subscripted so far backwards off the beginning of the array you went to a negative address. -__-',
        true
      );
    } else if (ptr.type.isArrayPointerType()) {
      // If it's an array pointer, make sure it's in bounds and not one-past
      if (addr < ptr.type.min()) {
        this.sim.eventOccurred(
          SimulationEvent.UNDEFINED_BEHAVIOR,
          'That subscript operation goes off the beginning of the array. This could cause a segfault, or worse - you might just access/change other memory outside the array.',
          true
        );
      } else if (ptr.type.onePast() < addr) {
        this.sim.eventOccurred(
          SimulationEvent.UNDEFINED_BEHAVIOR,
          'That subscript operation goes off the end of the array. This could cause a segfault, or worse - you might just access/change other memory outside the array.',
          true
        );
      } else if (addr == ptr.type.onePast()) {
        // TODO: technically this is not undefined behavior unless the result of the dereference undergoes an lvalue-to-rvalue conversion to look up the object
        this.sim.eventOccurred(
          SimulationEvent.UNDEFINED_BEHAVIOR,
          'That subscript accesses the element one past the end of the array. This could cause a segfault, or worse - you might just access/change other memory outside the array.',
          true
        );
      }
    }
...
export class DotExpression extends Expression<DotExpressionASTNode> {
  ...
  public constructor(
    ...
    switch (entityOrError) {
      case 'not_found':
        this.addNote(
          CPPError.expr.dot.no_such_member(this, classType, identifierToString(memberName))
        );
        break;
      case 'ambiguous':
    ...
    if (
      this.entity?.declarationKind === 'variable' &&
  ...
  public createDefaultOutlet(
    this: CompiledObjectDotExpression<CompleteObjectType> | CompiledFunctionDotExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedObjectDotExpression<
  T extends PotentiallyCompleteObjectType = PotentiallyCompleteObjectType
> extends DotExpression, t_TypedExpression {
  readonly type: T;
  readonly entity: MemberVariableEntity<T>;
  readonly operand: TypedExpression<CompleteClassType>;
}

export interface CompiledObjectDotExpression<
  T extends PotentiallyCompleteObjectType = PotentiallyCompleteObjectType
> extends TypedObjectDotExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly operand: CompiledExpression<CompleteClassType>;
}

export interface TypedFunctionDotExpression<T extends FunctionType = FunctionType>
...
export interface CompiledFunctionDotExpression<T extends FunctionType = FunctionType>
  extends TypedFunctionDotExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly operand: CompiledExpression<CompleteClassType>;
}

export class RuntimeObjectDotExpression<
  T extends CompleteObjectType = CompleteObjectType
> extends RuntimeExpression<T, 'lvalue', CompiledObjectDotExpression<T>> {
  ...
  protected upNextImpl() {
    if (!this.operand.isDone) {
      ...
      this.setEvalResult(
        <VCResultTypes<T, 'lvalue'>>this.operand.evalResult.getMemberObject(this.model.entity.name)!
      );
      this.startCleanup();
    }
  }
...
export class RuntimeFunctionDotExpression extends RuntimeExpression<
  FunctionType,
  'lvalue',
...
export class ArrowExpression extends Expression<ArrowExpressionASTNode> {
  ...
  public static createFromAST(
    ast: ArrowExpressionASTNode,
    context: ExpressionContext
  ): ArrowExpression {
    let operand: Expression = createExpressionFromAST(ast.operand, context);
    let receiverContext =
      operand.type?.isPointerType() && operand.type.ptrTo.isCompleteClassType()
  ...
  public constructor(
    ...
    switch (entityOrError) {
      case 'not_found':
        this.addNote(
          CPPError.expr.arrow.no_such_member(this, classType, identifierToString(memberName))
        );
        break;
      case 'ambiguous':
  ...
  public createDefaultOutlet(
    this: CompiledObjectArrowExpression<CompleteObjectType> | CompiledFunctionArrowExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedObjectArrowExpression<
  T extends PotentiallyCompleteObjectType = PotentiallyCompleteObjectType
> extends ArrowExpression, t_TypedExpression {
  readonly type: T;
  readonly entity: MemberVariableEntity<T>;
  readonly operand: TypedExpression<PointerType<CompleteClassType>>;
}

export interface CompiledObjectArrowExpression<
  T extends PotentiallyCompleteObjectType = PotentiallyCompleteObjectType
> extends TypedObjectArrowExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly operand: CompiledExpression<PointerType<CompleteClassType>>;
}

export interface TypedFunctionArrowExpression<T extends FunctionType = FunctionType>
  extends ArrowExpression,
    t_TypedExpression {
  readonly type: T;
  readonly entity: FunctionEntity<T>;
  readonly operand: TypedExpression<PointerType<CompleteClassType>>;
...
export interface CompiledFunctionArrowExpression<T extends FunctionType = FunctionType>
  extends TypedFunctionArrowExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly operand: CompiledExpression<PointerType<CompleteClassType>>;
}

export class RuntimeObjectArrowExpression<
  T extends CompleteObjectType = CompleteObjectType
> extends RuntimeExpression<T, 'lvalue', CompiledObjectArrowExpression<T>> {
  ...
  protected stepForwardImpl() {
    ...
    if (PointerType.isNull(addr.rawValue)) {
      this.sim.eventOccurred(
        SimulationEvent.CRASH,
        'Ow! Your code just tried to use the arrow operator on a null pointer!',
        true
      );
    }
...
export class RuntimeFunctionArrowExpression extends RuntimeExpression<
  FunctionType,
  'lvalue',
...
export class PostfixIncrementExpression extends Expression<PostfixIncrementExpressionASTNode> {
  ...
  public createDefaultOutlet(
    this: CompiledPostfixIncrementExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedPostfixIncrementExpression<
  T extends ArithmeticType | PointerToCompleteObjectType =
    | ArithmeticType
    | PointerToCompleteObjectType
> extends PostfixIncrementExpression, t_TypedExpression {
  readonly type: T;
  readonly operand: TypedExpression<T, 'lvalue'>;
}

export interface CompiledPostfixIncrementExpression<
  T extends ArithmeticType | PointerToCompleteObjectType =
    | ArithmeticType
    | PointerToCompleteObjectType
> extends TypedPostfixIncrementExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure

  readonly operand: CompiledExpression<T, 'lvalue'>;
}

export class RuntimePostfixIncrementExpression<
  T extends ArithmeticType | PointerToCompleteObjectType =
    | ArithmeticType
    | PointerToCompleteObjectType
> extends SimpleRuntimeExpression<T, 'prvalue', CompiledPostfixIncrementExpression<T>> {
  public operand: RuntimeExpression<T, 'lvalue'>;
...
export class IdentifierExpression extends Expression<IdentifierExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: IdentifierExpressionASTNode | undefined,
  ...
  public getEntity<T extends CompleteObjectType>(
    this: TypedExpression<T>
  ...
  public createDefaultOutlet(
    this:
      | CompiledObjectIdentifierExpression<CompleteObjectType>
...
export function entityLookup(
  ...
  lookupResult: DeclaredScopeEntry | undefined
): VariableEntity | FunctionEntity | EntityLookupError {
  if (!lookupResult) {
    return 'not_found';
    ...
    if (lookupResult.overloads.length === 1) {
      ...
      if (expression.context.contextualParameterTypes) {
        let overloadResult = overloadResolution(
          lookupResult.overloads,
          expression.context.contextualParameterTypes,
          expression.context.contextualReceiverType
        );
...
export interface TypedObjectIdentifierExpression<
  T extends PotentiallyCompleteObjectType = PotentiallyCompleteObjectType
> extends IdentifierExpression, t_TypedExpression {
  readonly type: T;
  readonly entity: LocalVariableEntity<T> | GlobalVariableEntity<T> | MemberVariableEntity<T>;
}

export interface CompiledObjectIdentifierExpression<
  T extends PotentiallyCompleteObjectType = PotentiallyCompleteObjectType
> extends TypedObjectIdentifierExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}

export interface TypedFunctionIdentifierExpression<T extends FunctionType = FunctionType>
...
export interface CompiledFunctionIdentifierExpression<T extends FunctionType = FunctionType>
  extends TypedFunctionIdentifierExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}

export class RuntimeObjectIdentifierExpression<
  T extends CompleteObjectType = CompleteObjectType
> extends RuntimeExpression<T, 'lvalue', CompiledObjectIdentifierExpression<T>> {
...
export class RuntimeFunctionIdentifierExpression extends RuntimeExpression<
  FunctionType,
  'lvalue',
  CompiledFunctionIdentifierExpression
> {
...
export interface TypedThisExpression<
  T extends PointerType<CompleteClassType> = PointerType<CompleteClassType>
> extends ThisExpression, t_TypedExpression {
  readonly type: T;
}

export interface CompiledThisExpression<
  T extends PointerType<CompleteClassType> = PointerType<CompleteClassType>
> extends TypedThisExpression<T>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}

export class RuntimeThisExpression<
  T extends PointerType<CompleteClassType> = PointerType<CompleteClassType>
> extends SimpleRuntimeExpression<T, 'prvalue', CompiledThisExpression<T>> {
...
export class NullptrExpression extends Expression<NullptrExpressionASTNode> {
  ...
  public createDefaultOutlet(
    this: CompiledNullptrExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export class RuntimeNullptrExpression extends SimpleRuntimeExpression<
  Int,
  'prvalue',
  CompiledNullptrExpression
> {
...
export class NumericLiteralExpression extends Expression<NumericLiteralASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: NumericLiteralASTNode | undefined,
    type: ArithmeticType,
    value: RawValueType
  ) {
    super(context, ast);
  ...
  public static createFromAST(ast: NumericLiteralASTNode, context: ExpressionContext) {
    return new NumericLiteralExpression(
      context,
      ast,
      literalTypes[ast.type],
      parseNumericLiteralValueFromAST(ast)
    );
  }
  ...
  public createDefaultOutlet(
    this: CompiledNumericLiteralExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedNumericLiteralExpression<T extends ArithmeticType = ArithmeticType>
  extends NumericLiteralExpression,
    t_TypedExpression {
  readonly type: T;
}

export interface CompiledNumericLiteralExpression<T extends ArithmeticType = ArithmeticType>
  extends TypedNumericLiteralExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}

export class RuntimeNumericLiteral<
  T extends ArithmeticType = ArithmeticType
> extends RuntimeExpression<T, 'prvalue', CompiledNumericLiteralExpression<T>> {
...
export class StringLiteralExpression extends Expression<StringLiteralASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: StringLiteralASTNode | undefined,
    str: string
  ) {
    super(context, ast);
    this.str = str;
  ...
  public createDefaultOutlet(
    this: CompiledStringLiteralExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
  ...
  public isSemanticallyEquivalent_impl(other: AnalyticConstruct, ec: SemanticContext): boolean {
    return other.construct_type === this.construct_type && this.str === other.str;
  }
}

export interface TypedStringLiteralExpression extends StringLiteralExpression, t_TypedExpression {}

export interface CompiledStringLiteralExpression
  extends TypedStringLiteralExpression,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}

export class RuntimeStringLiteralExpression extends RuntimeExpression<
  BoundedArrayType<Char>,
  'lvalue',
  CompiledStringLiteralExpression
> {
...
export class ParenthesesExpression extends Expression<ParenthesesExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: ParenthesesExpressionASTNode | undefined,
  ...
  public static createFromAST(
    ast: ParenthesesExpressionASTNode,
    context: ExpressionContext
  ): ParenthesesExpression {
    return new ParenthesesExpression(
      context,
      ast,
      createExpressionFromAST(ast.subexpression, context)
    );
  }

  public createDefaultOutlet(
    this: CompiledParenthesesExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedParenthesesExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends ParenthesesExpression, t_TypedExpression {
  readonly type: T;
  readonly valueCategory: V;
...
export interface CompiledParenthesesExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends TypedParenthesesExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeParentheses<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends RuntimeExpression<T, V, CompiledParenthesesExpression<T, V>> {
  public subexpression: RuntimeExpression<T, V>;

  private index:
    | typeof INDEX_PARENTHESES_SUBEXPRESSIONS
    | typeof INDEX_PARENTHESES_DONE = INDEX_PARENTHESES_SUBEXPRESSIONS;
...
export class InitializerListExpression extends Expression<InitializerListExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: InitializerListExpressionASTNode | undefined,
    ...
    this.elementsArray = this.createTemporaryObject(
      new BoundedArrayType(eltType.cvQualified(true), elements.length),
  ...
  public static createFromAST(
    ...
    return new InitializerListExpression(
      context,
      ast,
      ast.elements.map(eltAST => createExpressionFromAST(eltAST, context))
    );
  }

  public createDefaultOutlet(
    this: CompiledInitializerListExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedInitializerListExpression<T extends CompleteClassType = CompleteClassType>
  extends InitializerListExpression,
    t_TypedExpression {
  readonly type: T;
...
export interface CompiledInitializerListExpression<T extends CompleteClassType = CompleteClassType>
  extends TypedInitializerListExpression<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeInitializerListExpression<
  T extends CompleteClassType = CompleteClassType
> extends RuntimeExpression<T, 'lvalue', CompiledInitializerListExpression<T>> {
  ...
  protected upNextImpl() {
    if (this.elementIndex < this.elements.length) {
      ...
      this.elements.forEach((elem, i) =>
        this.elementsArray!.getArrayElemSubobject(i).setValue(
          <Value<ArithmeticType>>(
            (elem.evalResult instanceof CPPObject ? elem.evalResult.getValue() : elem.evalResult)
          )
        )
      );
      ...
      (<CPPObject<PointerType<ArithmeticType>>>(
        this.initializerList!.getMemberObject('begin')!
      )).setValue(eltsPointer);
      (<CPPObject<PointerType<ArithmeticType>>>(
        this.initializerList!.getMemberObject('begin')!
      )).setValue(eltsPointer.pointerOffsetRaw(this.elements.length));
...
export class AuxiliaryExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends Expression<never> {
  ...
  public createDefaultOutlet(
    this: CompiledAuxiliaryExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ): never {
    throw new Error(
      'Cannot create an outlet for an auxiliary expression. (They should never be used at runtime.)'
    );
  }
...
export interface TypedAuxiliaryExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends AuxiliaryExpression<T, V>, t_TypedExpression {}

export interface CompiledAuxiliaryExpression<
  T extends ExpressionType = ExpressionType,
  V extends ValueCategory = ValueCategory
> extends TypedAuxiliaryExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
}
...
export function overloadResolution<T extends FunctionType>(
  candidates: readonly FunctionEntity<T>[],
  argTypes: readonly (ExpressionType | undefined)[],
  receiverType?: CompleteClassType
): OverloadResolutionResult<T> {
  ...
  let resultCandidates: readonly OverloadCandidateResult[] = candidates.map(candidate => {
    ...
    // TODO: add back in with member functions
      ...
      candidate.isMemberFunction &&
      receiverType?.isConst &&
      !candidate.type.receiverType?.isConst
    ) {
      ...
      argTypes.forEach((argType, i) => {
        ...
        if (candidateParamType.isReferenceType()) {
          ...
          if (!referenceCompatible(argType, candidateParamType)) {
            notes.push(
              CPPError.param.paramReferenceType(
                candidate.firstDeclaration,
                argType,
                candidateParamType
              )
            );
          ...
          if (!sameType(convertedArg.type, candidateParamType)) {
            notes.push(
              CPPError.param.paramType(candidate.firstDeclaration, argType, candidateParamType)
            );
          }
        }
      });
    ...
    if (notes.length == 0) {
      // All notes in this function are errors, so if there are any it's not viable
  ...
  assert: {
    ...
    operate: (rt: RuntimeMagicFunctionCallExpression) => {
      ...
      if (!arg.rawValue) {
        ...
        rt.sim.eventOccurred(
          SimulationEvent.ASSERTION_FAILURE,
          `Assertion failed on line ${rt.model.getNearestSourceReference().line}.`,
          true
        );
      } else {
...
export class MagicFunctionCallExpression extends Expression<FunctionCallExpressionASTNode> {
  ...
  public constructor(
    ...
    functionName: MAGIC_FUNCTION_NAMES,
  ...
  public createDefaultOutlet(
    this: CompiledMagicFunctionCallExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedMagicFunctionCallExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends MagicFunctionCallExpression, t_TypedExpression {
  readonly type: T;
  readonly valueCategory: V;
}

export interface CompiledMagicFunctionCallExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends TypedMagicFunctionCallExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure

  readonly args: readonly CompiledExpression[];
}

export class RuntimeMagicFunctionCallExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends SimpleRuntimeExpression<T, V, CompiledMagicFunctionCallExpression<T, V>> {
...
export abstract class ImplicitConversion<
  FromType extends CompleteObjectType = CompleteObjectType,
  FromVC extends ValueCategory = ValueCategory,
  ToType extends CompleteObjectType = CompleteObjectType,
  ToVC extends ValueCategory = ValueCategory
> extends Expression {
  ...
  public createRuntimeExpression<
    FromType extends CompleteObjectType,
    FromVC extends ValueCategory,
    ToType extends CompleteObjectType,
    ToVC extends ValueCategory
  >(
    this: CompiledImplicitConversion<FromType, FromVC, ToType, ToVC>,
    parent: RuntimeConstruct
  ): RuntimeImplicitConversion<FromType, FromVC, ToType, ToVC>;
  public createRuntimeExpression<T extends ExpressionType, V extends ValueCategory>(
    this: CompiledExpression<T, V>,
    parent: RuntimeConstruct
  ): never;
  public createRuntimeExpression<
    FromType extends CompleteObjectType,
  ...
  public abstract operate(
    fromEvalResult: VCResultTypes<FromType, FromVC>
  ): VCResultTypes<ToType, ToVC>;
...
export interface TypedImplicitConversion<
  FromType extends CompleteObjectType = CompleteObjectType,
  FromVC extends ValueCategory = ValueCategory,
  ToType extends CompleteObjectType = CompleteObjectType,
  ToVC extends ValueCategory = ValueCategory
> extends ImplicitConversion<FromType, FromVC, ToType, ToVC>, t_TypedExpression {}

export interface CompiledImplicitConversion<
  FromType extends CompleteObjectType = CompleteObjectType,
  FromVC extends ValueCategory = ValueCategory,
  ToType extends CompleteObjectType = CompleteObjectType,
  ToVC extends ValueCategory = ValueCategory
> extends TypedImplicitConversion<FromType, FromVC, ToType, ToVC>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure

  readonly from: CompiledExpression<FromType, FromVC>;
}

export class RuntimeImplicitConversion<
  FromType extends CompleteObjectType = CompleteObjectType,
  FromVC extends ValueCategory = ValueCategory,
  ToType extends CompleteObjectType = CompleteObjectType,
  ToVC extends ValueCategory = ValueCategory
> extends SimpleRuntimeExpression<
  ToType,
  ToVC,
  CompiledImplicitConversion<FromType, FromVC, ToType, ToVC>
> {
  public readonly from: RuntimeExpression<FromType, FromVC>;

  public constructor(
    model: CompiledImplicitConversion<FromType, FromVC, ToType, ToVC>,
    parent: RuntimeConstruct
  ) {
    super(model, parent);
...
export class LValueToRValueConversion<T extends AtomicType> extends ImplicitConversion<
  T,
  'lvalue',
  T,
  'prvalue'
> {
  ...
  public createDefaultOutlet(
    this: CompiledLValueToRValueConversion,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedLValueToRValueConversion<T extends AtomicType = AtomicType>
  extends LValueToRValueConversion<T>,
    t_TypedExpression {}

export interface CompiledLValueToRValueConversion<T extends AtomicType = AtomicType>
  extends TypedLValueToRValueConversion<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly from: CompiledExpression<T, 'lvalue'>; // satisfies CompiledImplicitConversion and LValueToRValue structure
}

export class ArrayToPointerConversion<T extends BoundedArrayType> extends ImplicitConversion<
  T,
  'lvalue',
  PointerType,
  'prvalue'
> {
  ...
  public createDefaultOutlet(
    this: CompiledArrayToPointerConversion,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedArrayToPointerConversion<T extends BoundedArrayType = BoundedArrayType>
  extends ArrayToPointerConversion<T>,
    t_TypedExpression {}

export interface CompiledArrayToPointerConversion<T extends BoundedArrayType = BoundedArrayType>
  extends TypedArrayToPointerConversion<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly from: CompiledExpression<T, 'lvalue'>; // satisfies CompiledImplicitConversion and ArrayToPointer structure
}

export class StreamToBoolConversion extends ImplicitConversion<
  CompleteClassType,
  'lvalue',
  Bool,
  'prvalue'
> {
  ...
  public createDefaultOutlet(
    this: CompiledStreamToBoolConversion,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface CompiledStreamToBoolConversion
  extends TypedStreamToBoolConversion,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
abstract class TypeConversion<
  FromType extends AtomicType,
  ToType extends AtomicType
...
export interface TypedTypeConversion<
  FromType extends AtomicType = AtomicType,
  ToType extends AtomicType = AtomicType
> extends TypeConversion<FromType, ToType>, t_TypedExpression {}

export interface CompiledTypeConversion<
  FromType extends AtomicType = AtomicType,
  ToType extends AtomicType = AtomicType
> extends TypedTypeConversion<FromType, ToType>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly from: CompiledExpression<FromType, 'prvalue'>; // satisfies CompiledImplicitConversion and TypeConversion structure
}

abstract class NoOpTypeConversion<
  FromType extends AtomicType,
  ToType extends AtomicType
> extends TypeConversion<FromType, ToType> {
  ...
  public operate(fromEvalResult: VCResultTypes<FromType, 'prvalue'>) {
    return <VCResultTypes<ToType, 'prvalue'>>(
      new Value(fromEvalResult.rawValue, this.type, fromEvalResult.isValid)
    ); // Cast technically necessary here
  }
}

export type IntegerLiteralZero =
  | (CompiledNumericLiteralExpression<Int> & { value: Value<Int> & { rawValue: 0 } })
  | CompiledNullptrExpression;
...
export interface TypedNullPointerConversion<P extends PointerType>
  extends NullPointerConversion<P>,
    t_TypedExpression {}

export interface CompiledNullPointerConversion<P extends PointerType>
  extends TypedNullPointerConversion<P>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly from: IntegerLiteralZero;
}

export class PointerConversion<
  FromType extends PointerType,
  ToType extends PointerType
> extends NoOpTypeConversion<FromType, ToType> {
  // public readonly construct_type = "PointerConversion";
}

export interface TypedPointerConversion<FromType extends PointerType, ToType extends PointerType>
  extends TypedTypeConversion<FromType, ToType> {}

export interface CompiledPointerConversion<
  FromType extends PointerType,
  ToType extends PointerType
> extends CompiledTypeConversion<FromType, ToType> {}

export abstract class ToBooleanConversion<T extends AtomicType = AtomicType> extends TypeConversion<
  T,
  Bool
> {
  public constructor(from: TypedExpression<T, 'prvalue'>) {
...
export class PointerToBooleanConversion<
  T extends PointerType = PointerType
> extends ToBooleanConversion<T> {
...
export class FloatingToBooleanConversion<
  T extends FloatingPointType = FloatingPointType
> extends ToBooleanConversion<T> {
  // public readonly construct_type = "FloatingToBooleanConversion";
}
export class IntegralToBooleanConversion<
  T extends IntegralType = IntegralType
> extends ToBooleanConversion<T> {
...
export class IntegralPromotion<
  FromType extends IntegralType,
  ToType extends IntegralType
> extends NoOpTypeConversion<FromType, ToType> {
  // public readonly construct_type = "IntegralPromotion";
}

export class IntegralConversion<
  FromType extends IntegralType,
  ToType extends IntegralType
> extends NoOpTypeConversion<FromType, ToType> {
...
export class FloatingPointConversion<
  FromType extends FloatingPointType,
  ToType extends FloatingPointType
> extends NoOpTypeConversion<FromType, ToType> {
  // public readonly construct_type = "FloatingPointConversion";
}

export class IntegralToFloatingConversion<
  FromType extends IntegralType,
  ToType extends FloatingPointType
> extends NoOpTypeConversion<FromType, ToType> {
...
export class FloatingToIntegralConversion<T extends FloatingPointType> extends TypeConversion<
  T,
  IntegralType
> {
...
export class QualificationConversion<T extends AtomicType = AtomicType> extends ImplicitConversion<
  T,
  'prvalue',
  T,
  'prvalue'
> {
  ...
  public createDefaultOutlet(
    this: CompiledQualificationConversion,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
  ...
  public operate(fromEvalResult: VCResultTypes<T, 'prvalue'>) {
    return <VCResultTypes<T, 'prvalue'>>(
      fromEvalResult.cvQualified(this.type.isConst, this.type.isVolatile)
    );
  }
}

export interface TypedQualificationConversion<T extends AtomicType = AtomicType>
  extends QualificationConversion<T>,
    t_TypedExpression {}

export interface CompiledQualificationConversion<T extends AtomicType = AtomicType>
  extends TypedQualificationConversion<T>,
    SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
  readonly from: CompiledExpression<T, 'prvalue'>; // satisfies CompiledImplicitConversion and QualificationConversion structure
}

export function convertToPRValue<T extends AtomicType>(
  from: SpecificTypedExpression<T>
): TypedExpression<T, 'prvalue'>;
export function convertToPRValue<Elem_type extends ArrayElemType>(
  from: TypedExpression<BoundedArrayType<Elem_type>, 'lvalue'>
): TypedExpression<PointerType<Elem_type>, 'prvalue'>;
export function convertToPRValue(
  from: SpecificTypedExpression<PointerType> | TypedExpression<BoundedArrayType, 'lvalue'>
): TypedExpression<PointerType, 'prvalue'>;
export function convertToPRValue(
  from: SpecificTypedExpression<AtomicType> | TypedExpression<BoundedArrayType, 'lvalue'>
): TypedExpression<AtomicType, 'prvalue'>;
...
export function typeConversion(
  from: TypedExpression<PointerType, 'prvalue'>,
  toType: Bool
): TypedExpression<Bool, 'prvalue'>;
export function typeConversion(
  from: TypedExpression<Double, 'prvalue'>,
  toType: Float
): TypedExpression<Float, 'prvalue'>;
export function typeConversion(
  from: TypedExpression<IntegralType, 'prvalue'>,
  toType: IntegralType
): TypedExpression<IntegralType, 'prvalue'>;
export function typeConversion(
  from: TypedExpression<FloatingPointType, 'prvalue'>,
  toType: IntegralType
): TypedExpression<IntegralType, 'prvalue'>;
export function typeConversion(
  from: TypedExpression<IntegralType, 'prvalue'>,
  toType: FloatingPointType
): TypedExpression<FloatingPointType, 'prvalue'>;
export function typeConversion(
  from: TypedExpression<FloatingPointType, 'prvalue'>,
  toType: FloatingPointType
): TypedExpression<FloatingPointType, 'prvalue'>;
export function typeConversion<SimilarType extends AtomicType>(
  from: TypedExpression<SimilarType, 'prvalue'>,
  toType: SimilarType
): TypedExpression<SimilarType, 'prvalue'>;
export function typeConversion<FromType extends AtomicType, ToType extends AtomicType>(
  from: TypedExpression<FromType, 'prvalue'>,
  toType: ToType
): TypedExpression<FromType, 'prvalue'> | TypedExpression<ToType, 'prvalue'>;
export function typeConversion(from: TypedExpression<AtomicType, 'prvalue'>, toType: AtomicType) {
  ...
  if (
    ...
    toType.ptrTo.isPotentiallyCompleteClassType() &&
    Predicates.isTypedExpression(from, isPointerType) &&
    from.type.ptrTo.isPotentiallyCompleteClassType() &&
    ...
    return new PointerConversion(
      from,
      new PointerType(toType.ptrTo.cvQualified(from.type.ptrTo.isConst, from.type.ptrTo.isVolatile))
    );
  }
...
export function qualificationConversion(
  from: TypedExpression<AtomicType, 'prvalue'>,
  toType: AtomicType
) {
 ...
 * @param options
 */
export function standardConversion(
  from: TypedExpression,
  toType: ExpressionType,
  options: StandardConversionOptions = {}
) {
  if (
    Predicates.isTypedExpression(from, isCompleteClassType, 'lvalue') &&
    (from.type.className === 'ostream' || from.type.className === 'istream')
  ) {
  ...
  if (
    !(
      Predicates.isTypedExpression(from, isAtomicType) ||
      Predicates.isTypedExpression(from, isBoundedArrayType, 'lvalue')
    )
  ) {
    return from;
  }
...
export function integralPromotion(expr: TypedExpression<IntegralType, 'prvalue'>) {
  if (
    Predicates.isTypedExpression(expr, isIntegralType) &&
    !Predicates.isTypedExpression(expr, isType(Int))
  ) {
...
export function isIntegerLiteralZero(from: Expression): from is IntegerLiteralZero {
  return (
    from instanceof NullptrExpression ||
    (from instanceof NumericLiteralExpression &&
      isType(from.type, Int) &&
      from.value.rawValue === 0)
  );
}

export function isConvertibleToPointer(
  from: Expression
): from is
  | SpecificTypedExpression<PointerType>
  | TypedExpression<BoundedArrayType, 'lvalue'>
  | IntegerLiteralZero {
  if (!from.isWellTyped()) {
    return false;
  }
...
export function usualArithmeticConversions(
  leftOrig: SpecificTypedExpression<ArithmeticType>,
  rightOrig: SpecificTypedExpression<ArithmeticType>
) {
...
export function selectOperatorOverload(
  context: ExpressionContext,
  ast: ExpressionASTNode,
  operator: t_OverloadableOperators,
  originalArgs: Expression[]
) {
  ...
  if (leftmost.type.isCompleteClassType()) {
    ...
    lookupResult = leftmost.type.classScope.lookup(operatorFunctionName, {
      kind: 'normal',
      noParent: true,
    });
  ...
  let selected = overloadResolution(
    lookupResult.overloads,
    adjustedArgs.map(arg => arg.type),
    receiverType
  ).selected;

  if (selected) {
    if (selected.isMemberFunction) {
      return new MemberOperatorOverloadExpression(
        context,
        ast,
        operator,
        <TypedExpression<CompleteClassType>>leftmost,
        adjustedArgs,
        selected
      );
    } else {
      return new NonMemberOperatorOverloadExpression(
        context,
        ast,
        operator,
        adjustedArgs,
...
export class NonMemberOperatorOverloadExpression extends Expression<ExpressionASTNode> {
  ...
  public constructor(
    context: ExpressionContext,
    ast: ExpressionASTNode | undefined,
    operator: t_OverloadableOperators,
    args: readonly Expression[],
    ...
    if (!selectedFunctionEntity.returnsCompleteType()) {
      this.addNote(
        CPPError.expr.functionCall.incomplete_return_type(
          this,
          selectedFunctionEntity.type.returnType
        )
      );
      this.attachAll(args);
      return;
  ...
  public createDefaultOutlet(
    this: CompiledNonMemberOperatorOverloadExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedNonMemberOperatorOverloadExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends NonMemberOperatorOverloadExpression {
  readonly type: T;
  readonly valueCategory: V;
  readonly call: TypedFunctionCall<FunctionType<CompleteReturnType>>;
}

export interface CompiledNonMemberOperatorOverloadExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends TypedNonMemberOperatorOverloadExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeNonMemberOperatorOverloadExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends RuntimeExpression<T, V, CompiledNonMemberOperatorOverloadExpression<T, V>> {
  public readonly call: RuntimeFunctionCall<FunctionType<CompleteReturnType>>;

  public constructor(
    model: CompiledNonMemberOperatorOverloadExpression<T, V>,
    parent: RuntimeConstruct
  ) {
    super(model, parent);
...
export class MemberOperatorOverloadExpression extends Expression<ExpressionASTNode> {
  ...
  public constructor(
    ...
    ast: ExpressionASTNode | undefined,
    operator: t_OverloadableOperators,
    receiverExpression: TypedExpression<CompleteClassType>,
    args: readonly Expression[],
    selectedFunctionEntity: FunctionEntity
  ) {
    super(context, ast);
    ...
    this.attach(
      (this.call = new FunctionCall(context, selectedFunctionEntity, args, receiverExpression.type))
    );
  }

  public createDefaultOutlet(
    this: CompiledMemberOperatorOverloadExpression,
    element: JQuery,
    parent?: ConstructOutlet
  ) {
...
export interface TypedMemberOperatorOverloadExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends MemberOperatorOverloadExpression {
  readonly type: T;
  readonly valueCategory: V;
  readonly call: TypedFunctionCall<FunctionType<CompleteReturnType>>;
}

export interface CompiledMemberOperatorOverloadExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends TypedMemberOperatorOverloadExpression<T, V>, SuccessfullyCompiled {
  readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
export class RuntimeMemberOperatorOverloadExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends RuntimeExpression<T, V, CompiledMemberOperatorOverloadExpression<T, V>> {
  public readonly receiverExpression: RuntimeExpression<CompleteClassType>;
  public readonly call?: RuntimeFunctionCall<FunctionType<CompleteReturnType>>;

  public constructor(
    model: CompiledMemberOperatorOverloadExpression<T, V>,
    parent: RuntimeConstruct
  ) {
    super(model, parent);
  ...
  protected upNextImpl() {
    if (!this.receiverExpression.isDone) {
      ...
      (<Mutable<this>>this).call = this.model.call.createRuntimeFunctionCall(
        this,
        this.receiverExpression.evalResult
      );
      this.sim.push(this.call!);
    } else {
      // Note: cannot use this.model.type here, since that is the type of the function
...
export class InvalidOperatorOverloadExpression extends InvalidExpression {
  ...
  public constructor(
    context: ExpressionContext,
    ast: ExpressionASTNode,
    op: t_OverloadableOperators,
    originalArgs: readonly Expression[]
  ) {
    super(context, ast);
    this.operator = op;
