...
export function createNewExpressionFromAST(
...
export class NewExpression extends Expression<NewExpressionASTNode> {
    ...
    public constructor(
        ...
        assert(
            !createdType ||
                !declarator.type ||
                sameType(createdType, declarator.type)
        );
        ...
        let initializer = !initAST
            ...
            ? ListInitializer.create(
                  context,
                  newEntity,
                  initAST.arg.elements.map((a) =>
                      createExpressionFromAST(a, context)
                  )
              )
            : assertNever(initAST);
...
> extends RuntimeExpression<T, "prvalue", CompiledNewExpression<T>> {
    ...
    protected stepForwardImpl(): void {
        if (this.index === 0) {
            (<Mutable<this>>this).allocatedObject =
                this.sim.memory.heap.allocateNewObject(this.model.createdType);
            (<Mutable<this>>this).initializer =
                this.model.initializer?.createRuntimeInitializer(this);
            ++this.index;
        } else {
            this.setEvalResult(
                <VCResultTypes<T, "prvalue">>(
                    this.allocatedObject!.getPointerTo()
                )
            );
            this.startCleanup();
        }
    ...
    public constructor(
        ...
        if (createdType.isArrayOfUnknownBoundType()) {
            if (createdType.sizeExpressionAST) {
                let sizeExp = createExpressionFromAST(
                    createdType.sizeExpressionAST,
                    context
                );
                if (sizeExp.isWellTyped()) {
                    let convertedSizeExp = standardConversion(sizeExp, Int.INT);
                    if (!isType(convertedSizeExp.type, Int)) {
                        convertedSizeExp.addNote(
                            CPPError.expr.new_array.integer_length_required(
                                sizeExp
                            )
                        );
                    }
                    this.attach(
                        (this.dynamicLengthExpression = convertedSizeExp)
                    );
                } else {
        ...
        if (!initAST) {
            ...
            this.addNote(
                CPPError.expr.new_array.direct_initialization_prohibited(this)
            );
            ...
            this.individualElementInitializers = initAST.arg.elements.map(
                (arg, i) =>
                    DirectInitializer.create(
                        context,
                        new DynamicLengthArrayNextElementEntity(
                            this.allocatedArray
                        ),
        ...
        this.fallbackElementInitializer &&
            this.attach(this.fallbackElementInitializer);
        this.individualElementInitializers &&
            this.attachAll(this.individualElementInitializers);
    }
...
> extends NewArrayExpression,
    ...
    readonly allocatedArray: NewArrayEntity<
        PotentiallyCompleteArrayType<T["ptrTo"]>
    >;
}

export interface CompiledNewArrayExpression<
    T extends PointerType<ArrayElemType> = PointerType<ArrayElemType>
> extends TypedNewArrayExpression<T>,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
> extends RuntimeExpression<T, "prvalue", CompiledNewArrayExpression<T>> {
    ...
    public readonly allocatedObject?: DynamicObject<
        BoundedArrayType<T["ptrTo"]>
    >;
    public readonly nextElemToInit?: ArraySubobject<T["ptrTo"]>;

    public constructor(
        model: CompiledNewArrayExpression<T>,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
    ...
    protected upNextImpl() {
        if (
            this.dynamicLengthExpression &&
            !this.dynamicLengthExpression.isDone
        ) {
        ...
        if (this.elemInitIndex < this.allocatedObject.type.numElems) {
            asMutable(this).nextElemToInit =
                this.allocatedObject.getArrayElemSubobject(this.elemInitIndex);
    ...
    protected stepForwardImpl(): void {
        if (this.elemInitIndex === 0) {
            ...
            (<Mutable<this>>this).allocatedObject =
                this.sim.memory.heap.allocateNewObject(createdType);
            ...
            asMutable(this).elementInitializers =
                this.allocatedObject!.getArrayElemSubobjects().map(
                    (elemObj, i) =>
                        i < numInits
                            ? this.model.individualElementInitializers[
                                  i
                              ].createRuntimeInitializer(this)
                            : this.model.fallbackElementInitializer.createRuntimeInitializer(
                                  this
                              )
                );
        } else {
            this.setEvalResult(
                <VCResultTypes<T, "prvalue">>(
                    (<unknown>this.allocatedObject!.decayToPointer())
                )
            );
            this.startCleanup();
        }
...
export class DeleteExpression extends Expression<DeleteExpressionASTNode> {
    ...
    public constructor(
        ...
        if (!operand.type.isPointerToCompleteObjectType()) {
            this.addNote(
                CPPError.expr.delete.pointerToObjectType(this, operand.type)
            );
            this.attach((this.operand = operand));
            return;
        }
        ...
        if (destroyedType.isCompleteClassType()) {
            ...
            if (dtor) {
                ...
                this.addNote(
                    CPPError.expr.delete.no_destructor(this, destroyedType)
                );
            }
        }
    }
...
export interface TypedDeleteExpression
    extends DeleteExpression,
        t_TypedExpression {}

export interface CompiledDeleteExpression
    extends TypedDeleteExpression,
        SuccessfullyCompiled {
    readonly temporaryDeallocator?: CompiledTemporaryDeallocator; // to match CompiledPotentialFullExpression structure
...
> {
    public readonly operand: RuntimeExpression<
        PointerToCompleteObjectType,
        "prvalue"
    >;
    ...
    public constructor(
        model: CompiledDeleteExpression,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
    ...
    protected upNextImpl() {
        if (!this.operand.isDone) {
            ...
            if (!obj.hasStorage("dynamic")) {
                ...
                asMutable(this).destroyedObject = <
                    DynamicObject<NewObjectType>
                >obj;
                if (obj.isTyped(isCompleteClassType) && this.model.dtor) {
                    let dtor = this.model.dtor.createRuntimeFunctionCall(
                        this,
                        obj
                    );
...
export class DeleteArrayExpression extends Expression<DeleteArrayExpressionASTNode> {
    ...
    public constructor(
        ...
        if (!operand.type.ptrTo.isArrayElemType()) {
            this.addNote(
                CPPError.expr.delete.pointerToArrayElemType(this, operand.type)
            );
            this.attach((this.operand = operand));
            return;
        }
    ...
    public static createFromAST(
        ...
        return new DeleteArrayExpression(
            context,
            ast,
            createExpressionFromAST(ast.operand, context)
        );
...
export interface TypedDeleteArrayExpression
    extends DeleteArrayExpression,
        t_TypedExpression {}
...
> {
    ...
    public readonly operand: RuntimeExpression<
        ...
        "prvalue"
    >;
    public readonly dtors: readonly RuntimeFunctionCall<
    ...
    public constructor(
        model: CompiledDeleteArrayExpression,
        parent: RuntimeConstruct
    ) {
        super(model, parent);
        this.operand = createRuntimeExpression(this.model.operand, this);
    ...
    protected upNextImpl() {
        ...
        if (
            this.targetArray &&
            this.model.elementDtor &&
            this.index < this.targetArray.type.numElems
        ) {
            let elem = this.targetArray.getArrayElemSubobject(this.index++);
            if (elem.isTyped(isCompleteClassType)) {
                let dtor = this.model.elementDtor.createRuntimeFunctionCall(
                    this,
                    elem
                );
