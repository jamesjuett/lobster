...
export class Program {
  ...
  private link() {
    ...
    this.linkedObjectEntities.forEach(
      le => le.definition ?? le.link(this.linkedObjectDefinitions[le.qualifiedName.str])
    );
    this.linkedFunctionEntities.forEach(
      le => le.definition ?? le.link(this.linkedFunctionDefinitions[le.qualifiedName.str])
    );
    this.linkedClassEntities.forEach(
      le => le.definition ?? le.link(this.linkedClassDefinitions[le.qualifiedName.str])
    );
    ...
    if (mainLookup) {
      if (mainLookup.definitions.length === 1) {
        ...
        mainLookup.definitions.forEach(mainDef =>
          this.addNote(CPPError.link.main_multiple_def(mainDef.declaration))
        );
      }
    }
    ...
    if (this.mainFunction) {
      ...
      (<Mutable<this>>this).staticObjectDeallocator = createStaticDeallocator(
