...
export class FunctionCallExpression extends Expression<FunctionCallExpressionASTNode> {
  ...
  public constructor(
    ...
    if (!operand.entity.returnsCompleteType()) {
      this.attachAll(args);
      this.addNote(
        CPPError.expr.functionCall.incomplete_return_type(this, operand.entity.type.returnType)
      );
      return;
    }
  ...
  public static createFromAST(
    ...
    context: ExpressionContext
  ): FunctionCallExpression | MagicFunctionCallExpression | OperatorOverloadExpression {
    ...
    let operand = createExpressionFromAST(
      ast.operand,
      createExpressionContextWithParameterTypes(context, contextualParamTypes)
    );
    ...
    if (Predicates.isTypedExpression(operand, isPotentiallyCompleteClassType)) {
      return (
        selectOperatorOverload(context, ast, '[]', [operand, ...args]) ??
...
type PossibleVC<T extends PeelReference<CompleteReturnType>> = T extends CompleteReturnType
  ? ValueCategory
  : 'lvalue';
...
export interface CompiledFunctionCallExpression<
  T extends PeelReference<CompleteReturnType> = PeelReference<CompleteReturnType>,
  V extends ValueCategory = ValueCategory
> extends TypedFunctionCallExpression<T, V>, SuccessfullyCompiled {
