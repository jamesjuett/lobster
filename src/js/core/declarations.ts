import {
    ...
    ClassContext,
    isClassContext,
    createMemberSpecificationContext,
    ...
    EMPTY_SOURCE,
    createBlockContext,
    isMemberBlockContext,
    createOutOfLineFunctionDefinitionContext,
    SemanticContext,
...
} from "../parse/cpp_parser_util";
import { RuntimeFunctionCall } from "./FunctionCall";
import {
    StorageSpecifierASTNode,
    StorageSpecifierKey,
    TypeSpecifierASTNode,
    TypeSpecifierKey,
    ...
    SimpleDeclarationASTNode,
    ParameterDeclarationASTNode,
    ClassKey,
    AccessSpecifier,
    BaseSpecifierASTNode,
} from "../ast/ast_declarations";
import {
    DeclaratorASTNode,
    FunctionPostfixDeclaratorASTNode,
} from "../ast/ast_declarators";
...
> {
    ...
    public constructor(
        ...
        if (this.thread_local) {
            this.addNote(
                CPPError.lobster.unsupported_feature(this, "thread_local")
            );
        }

        if (this.register) {
            this.addNote(
                CPPError.lobster.unsupported_feature(this, "register")
            );
        }
        ...
        if (
            ...
            this.addNote(
                CPPError.declaration.storage.incompatible(this, specs)
            );
        }
...
export interface CompiledStorageSpecifier
    extends StorageSpecifier,
        SuccessfullyCompiled {}
    ...
    public constructor(
        ...
        specs.forEach((spec) => {
            if (
                ...
                spec.construct_type === "elaborated_type_specifier"
            ) {
                this.addNote(
                    CPPError.lobster.unsupported_feature(
                        this,
                        "class declarations or elaborated type specifiers"
                    )
                );
                return;
            }
            ...
            if (
                spec instanceof Object &&
                spec.construct_type === "class_definition"
            ) {
                this.addNote(
                    CPPError.lobster.unsupported_feature(
                        this,
            ...
            if (spec === "enum") {
                ...
                this.addNote(
                    CPPError.lobster.unsupported_feature(this, "mutable")
                );
                return;
            }

            // check to see if it's one of the possible type specifiers
            let possibleSpecs: readonly TypeSpecifierKey[] = [
                "const",
                "volatile",
                "signed",
                "unsigned",
            ...
            if (matchedSpec) {
                ...
                if (this[matchedSpec]) {
                    // it was a duplicate
                    this.addNote(
                        CPPError.declaration.typeSpecifier.once(
                            this,
                            matchedSpec
                        )
                    );
                ...
                // It's a typename
                if (this.typeName) {
                    // already had a typename, this is a duplicate
                    this.addNote(
                        CPPError.declaration.typeSpecifier.one_type(this, [
                            this.typeName,
                            spec,
                        ])
                    );
        ...
        if (this.unsigned && this.signed) {
            this.addNote(
                CPPError.declaration.typeSpecifier.signed_unsigned(this)
            );
        }
        ...
        if (this.typeName && isBuiltInTypeName(this.typeName)) {
            asMutable(this).baseType = new builtInTypes[this.typeName](
                this.const,
                this.volatile
            );
            return;
        ...
        if (customType?.declarationKind === "class") {
            asMutable(this).baseType = customType.type.cvQualified(
                this.const,
                this.volatile
            );
            return;
...
export interface CompiledTypeSpecifier<BaseType extends Type = Type>
...
const TopLevelDeclarationConstructsMap = {
    simple_declaration: (
        ast: NonMemberSimpleDeclarationASTNode,
        context: TranslationUnitContext
    ) => createTopLevelSimpleDeclarationFromAST(ast, context),
    function_definition: (
    ...
    class_definition: (
        ast: ClassDefinitionASTNode,
        context: TranslationUnitContext
    ) => ClassDefinition.createFromAST(ast, context),
};

export function createTopLevelDeclarationFromAST<
    ASTType extends TopLevelDeclarationASTNode
>(
    ast: ASTType,
    context: TranslationUnitContext
): ReturnType<
    typeof TopLevelDeclarationConstructsMap[ASTType["construct_type"]]
> {
    return <any>(
        TopLevelDeclarationConstructsMap[ast.construct_type](<any>ast, context)
    );
}

function createTopLevelSimpleDeclarationFromAST(
    ast: NonMemberSimpleDeclarationASTNode,
    context: TranslationUnitContext
) {
    assert(
        !isBlockContext(context),
        "Cannot create a top level declaration in a block context."
    );
    assert(
        !isClassContext(context),
        "Cannot create a top level declaration in a class context."
    );
    ...
    let storageSpec = StorageSpecifier.createFromAST(
        ast.specs.storageSpecs,
        context
    );
    ...
    return ast.declarators.map((declAST) => {
        ...
        if (!declaredType) {
            declaration = new UnknownTypeDeclaration(
                context,
                ast,
                typeSpec,
                storageSpec,
            ...
            declaration = new FriendDeclaration(
                context,
                ast,
                typeSpec,
                storageSpec,
            ...
            declaration = new TypedefDeclaration(
                context,
                ast,
                typeSpec,
                storageSpec,
            ...
            declaration = new VoidDeclaration(
                context,
                ast,
                typeSpec,
                storageSpec,
            ...
            declaration = new UnknownBoundArrayDeclaration(
                context,
                ast,
                typeSpec,
                storageSpec,
            ...
            declaredType.isCompleteObjectType() ||
            declaredType.isReferenceType()
        ) {
            declaration = new GlobalVariableDefinition(
                context,
                ast,
                typeSpec,
                storageSpec,
            ...
            declaration = new IncompleteTypeVariableDefinition(
                context,
                ast,
                typeSpec,
                storageSpec,
...
export function setInitializerFromAST(
    declaration: VariableDefinition | MemberVariableDeclaration,
    initAST:
        | DirectInitializerASTNode
        | CopyInitializerASTNode
        | ListInitializerASTNode
...
const LocalDeclarationConstructsMap = {
    ...
    ) => createLocalSimpleDeclarationFromAST(ast, context),
    function_definition: (
        ast: FunctionDefinitionASTNode,
        context: BlockContext
    ) => FunctionDefinition.createFromAST(ast, context),
    class_definition: (ast: ClassDefinitionASTNode, context: BlockContext) =>
        ClassDefinition.createFromAST(ast, context),
};

export function createLocalDeclarationFromAST<
    ASTType extends LocalDeclarationASTNode
>(
    ast: ASTType,
    context: BlockContext
): ReturnType<typeof LocalDeclarationConstructsMap[ASTType["construct_type"]]> {
    return <any>(
        LocalDeclarationConstructsMap[ast.construct_type](<any>ast, context)
    );
}

export function createLocalSimpleDeclarationFromAST(
    ast: NonMemberSimpleDeclarationASTNode,
    context: TranslationUnitContext
) {
    assert(
        isBlockContext(context),
        "A local declaration must be created in a block context."
    );
    ...
    return ast.declarators.map((declAST) => {
        ...
        if (!declaredType) {
            ...
            declaration = new LocalVariableDefinition(
                context,
                ast,
                typeSpec,
                storageSpec,
...
const MemberDeclarationConstructsMap = {
    simple_member_declaration: (
        ast: MemberSimpleDeclarationASTNode,
        context: MemberSpecificationContext
    ) => createMemberSimpleDeclarationFromAST(ast, context),
    function_definition: (
        ast: FunctionDefinitionASTNode,
        context: MemberSpecificationContext
    ) => createFunctionDeclarationFromDefinitionAST(ast, context),
    // Note: function_definition includes ctor and dtor definitions
};

export function createMemberDeclarationFromAST<
    ASTType extends MemberDeclarationASTNode
>(
    ast: ASTType,
    context: MemberSpecificationContext
): ReturnType<
    typeof MemberDeclarationConstructsMap[ASTType["construct_type"]]
> {
    return <any>(
        MemberDeclarationConstructsMap[ast.construct_type](<any>ast, context)
    );
}

export function createMemberSimpleDeclarationFromAST(
    ast: MemberSimpleDeclarationASTNode,
    context: MemberSpecificationContext
) {
    ...
    if (baseType?.sameType(context.containingClass.type)) {
        let testDeclarator = Declarator.createFromAST(
            ast.declarators[0],
            context,
            baseType
        );
        if (!testDeclarator.name) {
            typeSpec = TypeSpecifier.createFromAST(
                ast.specs.typeSpecs.filter(
                    (spec) => spec !== context.containingClass.name
                ),
                context
            );
        }
    ...
    return ast.declarators.map((declAST) => {
        ...
        if (!declaredType) {
            ...
            declaration = new MemberVariableDeclaration(
                context,
                ast,
                typeSpec,
                storageSpec,
            ...
            declaration = new IncompleteTypeMemberVariableDeclaration(
                context,
                ast,
                typeSpec,
                storageSpec,
...
export type AnalyticDeclaration =
    | Declaration
    | Declarator
...
export type TypedDeclarationKinds<T extends Type> = {
    ...
    unknown_type_declaration: T extends undefined
        ? UnknownTypeDeclaration
        : never;
    ...
    unknown_array_bound_declaration: T extends ArrayOfUnknownBoundType
        ? TypedUnknownBoundArrayDeclaration<T>
        : never;
    function_declaration: T extends FunctionDeclaration["type"]
        ? TypedFunctionDeclaration<T>
        : never;
    global_variable_definition: T extends GlobalVariableDefinition["type"]
        ? TypedGlobalVariableDefinition<T>
        : never;
    local_variable_definition: T extends LocalVariableDefinition["type"]
        ? TypedLocalVariableDefinition<T>
        : never;
    incomplete_type_variable_definition: T extends IncompleteTypeVariableDefinition["type"]
        ? TypedIncompleteTypeVariableDefinition<T>
        : never;
    parameter_declaration: T extends ParameterDeclaration["type"]
        ? TypedParameterDeclaration<T>
        : never;
    declarator: T extends Declarator["type"] ? TypedDeclarator<T> : never;
    function_definition: T extends FunctionDeclaration["type"]
        ? TypedFunctionDefinition<T>
        : never;
    class_declaration: T extends ClassDeclaration["type"]
        ? TypedClassDeclaration<T>
        : never;
    class_definition: T extends ClassDefinition["type"]
        ? TypedClassDefinition<T>
        : never;
    member_variable_declaration: T extends MemberVariableDeclaration["type"]
        ? TypedMemberVariableDeclaration<T>
        : never;
    incomplete_type_member_variable_declaration: T extends IncompleteTypeMemberVariableDeclaration["type"]
        ? TypedIncompleteTypeMemberVariableDeclaration<T>
        : never;
...
export type CompiledDeclarationKinds<T extends Type> = {
    ...
    function_declaration: T extends FunctionDeclaration["type"]
        ? CompiledFunctionDeclaration<T>
        : never;
    global_variable_definition: T extends GlobalVariableDefinition["type"]
        ? CompiledGlobalVariableDefinition<T>
        : never;
    local_variable_definition: T extends LocalVariableDefinition["type"]
        ? CompiledLocalVariableDefinition<T>
        : never;
    incomplete_type_variable_definition: never;
    parameter_declaration: T extends ParameterDeclaration["type"]
        ? CompiledParameterDeclaration<T>
        : never;
    declarator: T extends Declarator["type"] ? CompiledDeclarator<T> : never;
    function_definition: T extends FunctionDeclaration["type"]
        ? CompiledFunctionDefinition<T>
        : never;
    class_declaration: T extends ClassDeclaration["type"]
        ? CompiledClassDeclaration<T>
        : never;
    class_definition: T extends ClassDefinition["type"]
        ? CompiledClassDefinition<T>
        : never;
    member_variable_declaration: T extends MemberVariableDeclaration["type"]
        ? CompiledMemberVariableDeclaration<T>
        : never;
    incomplete_type_member_variable_declaration: never;
    // TODO: add rest of discriminants and their types
};

export type AnalyticTypedDeclaration<
    C extends AnalyticDeclaration,
    T extends Type = NonNullable<C["type"]>
> = TypedDeclarationKinds<T>[C["construct_type"]];
export type AnalyticCompiledDeclaration<
    C extends AnalyticDeclaration,
    T extends Type = NonNullable<C["type"]>
> = CompiledDeclarationKinds<T>[C["construct_type"]];

export abstract class SimpleDeclaration<
    ContextType extends TranslationUnitContext = TranslationUnitContext
> extends BasicCPPConstruct<ContextType, SimpleDeclarationASTNode> {
    ...
    protected constructor(
        context: ContextType,
        ast: SimpleDeclarationASTNode | undefined,
        typeSpec: TypeSpecifier,
        storageSpec: StorageSpecifier,
        declarator: Declarator,
        otherSpecs: OtherSpecifiers
    ) {
        super(context, ast);
...
export interface TypedSimpleDeclaration<T extends Type>
    extends SimpleDeclaration {
    readonly type: T;
    readonly declaredEntity: CPPEntity<T>;
}

export interface CompiledSimpleDeclaration<T extends Type = Type>
    extends TypedSimpleDeclaration<T>,
        SuccessfullyCompiled {
...
export class UnknownTypeDeclaration extends SimpleDeclaration {
    ...
    public constructor(
        context: TranslationUnitContext,
        ast: SimpleDeclarationASTNode | MemberSimpleDeclarationASTNode,
        typeSpec: TypeSpecifier,
        storageSpec: StorageSpecifier,
        declarator: Declarator,
        otherSpecs: OtherSpecifiers
...
export interface TypedUnknownBoundArrayDeclaration<
    T extends ArrayOfUnknownBoundType
> extends UnknownBoundArrayDeclaration {
    readonly type: T;
}
...
export class FunctionDeclaration extends SimpleDeclaration {
    ...
    public constructor(
        ...
        if (isClassContext(context)) {
            ...
            this.qualifiedName = composeQualifiedName(
            ...
            while (base) {
                let matchInBase =
                    base.classDefinition.memberFunctionEntities.find(
                        (baseFunc) =>
                            this.name === baseFunc.name &&
                            this.type.isPotentialOverriderOf(baseFunc.type)
                    );
                ...
                if (matchInBase?.isVirtual) {
                    ...
                    if (
                        ...
                        this.addNote(
                            CPPError.declaration.func.nonCovariantReturnType(
                                this,
                                this.type.returnType,
                                matchInBase.type.returnType
                            )
        ...
        assert(
            !!this.declarator.parameters,
            "The declarator for a function declaration must contain declarators for its parameters as well."
        );
        ...
        if (this.isConstructor) {
            ...
            if (this.declarator.baseType) {
                this.addNote(
                    CPPError.declaration.ctor.return_type_prohibited(this)
                );
            }

            if (otherSpecs.virtual) {
                // use otherSpecs here since this.isVirtual depends on being a member fn
                this.addNote(
                    CPPError.declaration.ctor.virtual_prohibited(this)
                );
            }
        } else {
            let entityOrError =
                this.context.contextualScope.declareFunctionEntity(
                    this.declaredEntity
                );
            ...
            if (entityOrError instanceof FunctionEntity) {
                ...
                if (actualDeclaredEntity === this.declaredEntity) {
                    ...
                    if (overrideTarget) {
                        overrideTarget.registerOverrider(
                            containingClass!,
                            actualDeclaredEntity
                        );
    ...
    public isSemanticallyEquivalent_impl(
        ...
        return (
            other.construct_type === this.construct_type &&
            this.declaredEntity.isSemanticallyEquivalent(
                other.declaredEntity,
                equivalenceContext
            )
        );
    }
...
export interface TypedFunctionDeclaration<T extends FunctionType>
...
export interface CompiledFunctionDeclaration<
    T extends FunctionType = FunctionType
> extends TypedFunctionDeclaration<T>,
        SuccessfullyCompiled {
    readonly typeSpecifier: CompiledTypeSpecifier;
    readonly storageSpecifier: CompiledStorageSpecifier;
...
abstract class VariableDefinitionBase<
    ContextType extends TranslationUnitContext = TranslationUnitContext
> extends SimpleDeclaration<ContextType> {
    public readonly initializer?: Initializer;
    ...
    public setDefaultInitializer() {
        return this.setInitializer(
            DefaultInitializer.create(this.context, this.declaredEntity)
        );
    }
    ...
    public setInitializerList(args: readonly Expression[]) {
        ...
        let init = ListInitializer.create(
            this.context,
            this.declaredEntity,
            args
        );
    ...
    public isSemanticallyEquivalent_impl(
        ...
        return (
            this.construct_type === other.construct_type &&
            other instanceof VariableDefinitionBase &&
            areEntitiesSemanticallyEquivalent(
                this.declaredEntity,
                other.declaredEntity,
                equivalenceContext
...
export class LocalVariableDefinition extends VariableDefinitionBase<BlockContext> {
    ...
    public constructor(
        ...
        this.declaredEntity = type.isReferenceType()
            ? new LocalReferenceEntity(type, this)
            : new LocalObjectEntity(type, this);
        ...
        let entityOrError = context.contextualScope.declareVariableEntity(
            this.declaredEntity
        );

        if (
            entityOrError instanceof LocalObjectEntity ||
            entityOrError instanceof LocalReferenceEntity
        ) {
...
export interface TypedLocalVariableDefinition<T extends VariableDefinitionType>
    extends LocalVariableDefinition {
    readonly type: T;
    readonly declarator: TypedDeclarator<T>;
    readonly declaredEntity:
        | LocalObjectEntity<Exclude<T, ReferenceType>>
        | LocalReferenceEntity<Extract<T, ReferenceType>>;
}

export interface CompiledLocalVariableDefinition<
    T extends VariableDefinitionType = VariableDefinitionType
> extends TypedLocalVariableDefinition<T>,
        SuccessfullyCompiled {
    readonly typeSpecifier: CompiledTypeSpecifier;
    readonly storageSpecifier: CompiledStorageSpecifier;
    readonly declarator: CompiledDeclarator<T>;
...
export class GlobalVariableDefinition extends VariableDefinitionBase<TranslationUnitContext> {
    ...
    public constructor(
        context: TranslationUnitContext,
        ast: NonMemberSimpleDeclarationASTNode | undefined,
        typeSpec: TypeSpecifier,
        storageSpec: StorageSpecifier,
        declarator: Declarator,
        otherSpecs: OtherSpecifiers,
        ...
        if (entityOrError instanceof GlobalObjectEntity) {
            this.declaredEntity = entityOrError;
            this.context.translationUnit.program.registerGlobalObjectDefinition(
                this.declaredEntity.qualifiedName,
                this
            );
        } else {
            this.addNote(entityOrError);
...
export interface TypedGlobalVariableDefinition<T extends VariableDefinitionType>
...
export interface CompiledGlobalVariableDefinition<
    T extends VariableDefinitionType = VariableDefinitionType
> extends TypedGlobalVariableDefinition<T>,
        SuccessfullyCompiled {
    readonly typeSpecifier: CompiledTypeSpecifier;
    readonly storageSpecifier: CompiledStorageSpecifier;
    readonly declarator: CompiledDeclarator<T>;
...
> {
    ...
    public constructor(
        context: TranslationUnitContext,
        ast: ParameterDeclarationASTNode,
        typeSpec: TypeSpecifier,
        storageSpec: StorageSpecifier,
        declarator: Declarator,
        otherSpecs: OtherSpecifiers
        ...
        if (declarator.name && isQualifiedName(declarator.name)) {
            storageSpec.addNote(
                CPPError.declaration.parameter.storage_prohibited(storageSpec)
            );
        }
        ...
        if (type && !type.isPotentialParameterType()) {
            this.addNote(
                CPPError.declaration.parameter.invalid_parameter_type(
                    this,
                    type
                )
            );
        ...
        if (this.isPotentialParameterDefinition()) {
            (<Mutable<this>>this).declaredEntity = this.type.isReferenceType()
                ? new LocalReferenceEntity(this.type, this, true)
    ...
    public static createFromAST(
        ...
        let typeSpec = TypeSpecifier.createFromAST(
            ast.specs.typeSpecs,
            context
        );

        // Compile declarator for each parameter (of the function-type argument itself)
        let declarator = Declarator.createFromAST(
            ast.declarator,
            context,
            typeSpec.baseType
        );
    ...
    public isPotentialParameterDefinition(): this is ParameterDefinition {
        return (
            !!this.name && !!this.type && this.type.isPotentialParameterType()
        );
    }
...
export interface TypedParameterDeclaration<
    T extends PotentialParameterType = PotentialParameterType
...
export interface CompiledParameterDeclaration<
    T extends PotentialParameterType = PotentialParameterType
> extends TypedParameterDeclaration<T>,
        SuccessfullyCompiled {
    readonly typeSpecifier: CompiledTypeSpecifier;
    readonly storageSpecifier: CompiledStorageSpecifier;
    readonly declarator: CompiledDeclarator<T>;
...
export interface TypedParameterDefinition<
    T extends CompleteParameterType = CompleteParameterType
> extends ParameterDeclaration {
    readonly type: T;
    readonly declarator: TypedDeclarator<T>;
    readonly declaredEntity:
...
export interface CompiledParameterDefinition<
    T extends CompleteParameterType = CompleteParameterType
> extends TypedParameterDefinition<T>,
        SuccessfullyCompiled {
    readonly typeSpecifier: CompiledTypeSpecifier;
    readonly storageSpecifier: CompiledStorageSpecifier;
    readonly declarator: CompiledDeclarator<T>;
...
export class IncompleteTypeVariableDefinition extends SimpleDeclaration<TranslationUnitContext> {
    ...
    public constructor(
        ...
        this.addNote(
            CPPError.declaration.incomplete_type_definition_prohibited(this)
        );
    }
...
export interface TypedIncompleteTypeVariableDefinition<
    T extends IncompleteObjectType
> extends IncompleteTypeVariableDefinition {
    readonly type: T;
    readonly declarator: TypedDeclarator<T>;
}
...
> {
    ...
    public static createFromAST(
        ast: DeclaratorASTNode | undefined,
        context: TranslationUnitContext,
    ...
    private determineNameAndType(ast: DeclaratorASTNode) {
        ...
        while (findName) {
            if (findName.name) {
                ...
                if (isUnqualifiedName(n)) {
                    ...
                    let newComponents = n.components.map((component) =>
                        component.replace(/<.*>/g, "")
                    );
                    n = {
        ...
        if (this.name && isQualifiedName(this.name)) {
            ...
            if (le && isClassContext(le.firstDeclaration.context)) {
                let className =
                    le.firstDeclaration.context.containingClass.name;
        ...
        while (decl) {
                ...
                for (let i = decl.postfixes.length - 1; i >= 0; --i) {
                    ...
                    if (postfix.kind === "array") {
                        if (type.isBoundedArrayType()) {
                            this.addNote(
                                CPPError.declaration.array.multidimensional_arrays_unsupported(
                                    this
                                )
                            );
                            return;
                        ...
                        if (!type.isArrayElemType()) {
                            this.addNote(
                                CPPError.declaration.array.invalid_element_type(
                                    this,
                                    type
                                )
                            );
                        ...
                        if (postfix.size) {
                            if (
                                ...
                                type = new ArrayOfUnknownBoundType(
                                    type,
                                    postfix.size
                                );
                        ...
                        let fnType = this.processFunctionDeclarator(
                            postfix,
                            type,
                            this
                        );
            ...
            if (decl.pointer) {
                if (!type.isPotentiallyCompleteObjectType()) {
                    if (type.isReferenceType()) {
                        this.addNote(
                            CPPError.declaration.pointer.reference(this)
                        );
                ...
                if (!type.isPotentiallyCompleteObjectType()) {
                    if (type.isReferenceType()) {
                        ...
                        this.addNote(
                            CPPError.declaration.ref.invalid_referred_type(
                                this,
                                type
                            )
                        );
    ...
    private processFunctionDeclarator(
        postfix: FunctionPostfixDeclaratorASTNode,
        type: Type,
        notes: NoteHandler
        ...
        let paramDeclarations = postfix.args.map((argAST) =>
            ParameterDeclaration.createFromAST(argAST, this.context)
        );
        ...
        if (
            ...
            for (let j = 0; j < paramTypes.length; ++j) {
                ...
                if (paramType && paramType.isVoidType()) {
                    notes.addNote(
                        CPPError.declaration.func.void_param(
                            paramDeclarations[j]
                        )
                    );
                }
        ...
        if (
            !paramTypes.every(
                (paramType) => paramType && paramType.isPotentialParameterType()
            )
        ) {
            notes.addNote(
                CPPError.declaration.func.some_invalid_parameter_types(this)
            );
            return;
        }
        ...
        return new FunctionType(
            type,
            <PotentialParameterType[]>paramTypes,
            this.context.containingClass?.type.cvQualified(!!postfix.const)
        );
    }
}
...
export interface CompiledDeclarator<T extends Type = Type>
    extends TypedDeclarator<T>,
        SuccessfullyCompiled {
...
> {
    ...
    public static createFromAST(
        ...
        context: TranslationUnitContext,
        declaration?: FunctionDeclaration
    ): FunctionDefinition | InvalidConstruct;
    public static createFromAST(
        ast: FunctionDefinitionASTNode,
        context: TranslationUnitContext,
        declaration?: FunctionDeclaration
        ...
        if (
            isMemberSpecificationContext(declaration.context) &&
            !isMemberSpecificationContext(context)
        ) {
            context = createOutOfLineFunctionDefinitionContext(
                declaration.context,
                context
            );
            outOfLine = true;
        ...
        if (declaration.isMemberFunction) {
            assert(
                declaration.type.receiverType?.isComplete(),
                "Member function definitions may not be compiled until their containing class definition has been completed."
            );
            receiverType = declaration.type.receiverType;
        }

        let functionContext = createFunctionContext(
            context,
        ...
        if (declaration.isConstructor && isMemberBlockContext(bodyContext)) {
            if (ast.ctor_initializer) {
                ctorInitializer = CtorInitializer.createFromAST(
                    ast.ctor_initializer,
                    bodyContext
                );
            } else {
                ctorInitializer = new CtorInitializer(
                    bodyContext,
                    undefined,
                    []
                );
            ...
            if (ast.ctor_initializer) {
                ctorInitializer = new InvalidConstruct(
                    bodyContext,
                    ast.ctor_initializer,
                    CPPError.declaration.ctor.init.constructor_only
                );
            }
        }
        ...
        let body = new Block(
            ...
            ast.body.statements.map((s) =>
                createStatementFromAST(s, bodyContext)
            )
        );
    ...
    public constructor(
        ...
        body: Block,
        outOfLineMemberFunction: boolean
    ) {
        super(context, ast);
        ...
        if (this.declaration.isDestructor) {
            ...
            this.attach(
                (this.memberDeallocator = createMemberDeallocator(
                    context,
                    new ReceiverEntity(
                        <CompleteClassType>this.type.receiverType
                    )
                ))
            );
        }
        ...
        this.context.translationUnit.program.registerFunctionDefinition(
            this.declaration.declaredEntity.qualifiedName,
            this
        );
    }

    public createRuntimeFunction<T extends FunctionType<CompleteReturnType>>(
        this: CompiledFunctionDefinition<T>,
        parent: RuntimeFunctionCall,
 ...
 * @param context
 */
function createFunctionDeclarationFromDefinitionAST(
    ast: FunctionDefinitionASTNode,
    context: TranslationUnitContext
) {
    // Need to create TypeSpecifier first to get the base type for the declarators
    ...
    if (declarator.name && isQualifiedName(declarator.name)) {
        let prevEntity = context.program.getLinkedFunctionEntity(
            declarator.name
        );
        if (prevEntity) {
    ...
    if (!declaredType?.isFunctionType()) {
        return new InvalidConstruct(
            context,
            ast,
            CPPError.declaration.func.definition_non_function_type
        );
    }
...
export interface TypedFunctionDefinition<T extends FunctionType>
...
export interface CompiledFunctionDefinition<
    T extends FunctionType = FunctionType
> extends TypedFunctionDefinition<T>,
        SuccessfullyCompiled {
...
export class ClassDeclaration extends BasicCPPConstruct<
    TranslationUnitContext,
    ASTNode
> {
    ...
    public constructor(
        ...
        let entityOrError = context.contextualScope.declareClassEntity(
            this.declaredEntity
        );
...
export interface TypedClassDeclaration<T extends PotentiallyCompleteClassType>
    extends ClassDeclaration,
        SuccessfullyCompiled {
    readonly type: T;
}

export interface CompiledClassDeclaration<
    T extends PotentiallyCompleteClassType = PotentiallyCompleteClassType
> extends TypedClassDeclaration<T>,
        SuccessfullyCompiled {}

export class ClassDefinition extends BasicCPPConstruct<
    ClassContext,
    ClassDefinitionASTNode
> {
    ...
    public readonly memberDeclarationsByName: {
    ...
    public readonly memberReferenceEntities: readonly MemberReferenceEntity[] =
        [];
    public readonly memberVariableEntitiesByName: {
        [index: string]: MemberVariableEntity | undefined;
    } = {};

    public readonly defaultConstructor?: FunctionEntity<FunctionType<VoidType>>;
    public readonly constCopyConstructor?: FunctionEntity<
        FunctionType<VoidType>
    >;
    public readonly nonConstCopyConstructor?: FunctionEntity<
        FunctionType<VoidType>
    >;
    ...
    public static createFromAST(
        ast: ClassDefinitionASTNode,
        tuContext: TranslationUnitContext
    ) {
        let classKey = ast.head.classKey;

        // Default access level is private for class, public for struct
        let defaultAccessLevel: AccessSpecifier =
            classKey === "class" ? "private" : "public";
        ...
        let bases = <CompiledBaseSpecifier[]>ast.head.bases
            .map((baseAST) => {
                let base = BaseSpecifier.createFromAST(
                    baseAST,
                    tuContext,
                    defaultAccessLevel
                );
        ...
        let declaration = new ClassDeclaration(
            tuContext,
            ast.head.name.identifier,
            classKey
        );
        ...
        let classContext = createClassContext(
            tuContext,
            declaration.declaredEntity,
            bases[0]?.baseEntity,
            templateType
        ...
        ast.memberSpecs.forEach((memSpec) => {
            ...
            let memberSpecContext = createMemberSpecificationContext(
                classContext,
                accessLevel
            );
            ...
            memSpec.members.forEach((memberAST) => {
                let decls = createMemberDeclarationFromAST(
                    memberAST,
                    memberSpecContext
                );
                if (Array.isArray(decls)) {
                    ...
                    memDecls.push(decls);
                    if (
                        decls.construct_type === "function_declaration" &&
                        memberAST.construct_type === "function_definition"
                    ) {
                        functionDefsToCompile.push([
                            memberAST,
                            memberSpecContext,
                            decls,
                        ]);
        ...
        functionDefsToCompile.forEach(([defAST, memberSpecContext, decl]) => {
            classDef.attachInlineFunctionDefinition(
                FunctionDefinition.createFromAST(
                    defAST,
                    memberSpecContext,
                    decl
                )
    ...
    public constructor(
        ...
        ast: ClassDefinitionASTNode | undefined,
        declaration: ClassDeclaration,
        baseSpecs: readonly BaseSpecifier[],
        memberDeclarations: readonly MemberDeclaration[]
    ) {
        super(context, ast);

        this.name = declaration.name;
        this.implicitPublicContext = createImplicitContext(
            createMemberSpecificationContext(context, "public")
        );
        ...
        memberDeclarations.forEach((decl) => {
            if (decl.construct_type === "member_variable_declaration") {
                asMutable(this.memberVariableEntities).push(
                    decl.declaredEntity
                );

                if (decl.declaredEntity instanceof MemberObjectEntity) {
                    asMutable(this.memberObjectEntities).push(
                        decl.declaredEntity
                    );
                } else {
                    asMutable(this.memberReferenceEntities).push(
                        decl.declaredEntity
                    );
                }
                ...
                if (!this.memberDeclarationsByName[decl.name]) {
                    ...
                    this.memberVariableEntitiesByName[decl.name] =
                        decl.declaredEntity;
                }
                ...
                asMutable(this.memberFunctionEntities).push(
                    decl.declaredEntity
                );
            }
        });
        ...
        memberDeclarations.forEach((mem) => {
            if (
                mem.construct_type === "function_declaration" &&
                mem.isConstructor
            ) {
                ...
                if (
                    this.constructors.some((prevCtor) =>
                        prevCtor.type.sameSignature(mem.type)
                    )
                ) {
                    mem.addNote(
                        CPPError.declaration.ctor.previous_declaration(mem)
                    );
                } else {
                    ...
                    if (ctorEntity.returnsVoid()) {
                        ...
                        if (ctorEntity.type.paramTypes.length === 0) {
                            (<Mutable<this>>this).defaultConstructor =
                                ctorEntity;
                        } else if (
                            ctorEntity.type.sameParamTypes([
                                new ReferenceType(
                                    this.declaration.type.cvQualified(true)
                                ),
                            ])
                        ) {
                            (<Mutable<this>>this).constCopyConstructor =
                                ctorEntity;
                        } else if (
                            ctorEntity.type.sameParamTypes([
                                new ReferenceType(
                                    this.declaration.type.cvUnqualified()
                                ),
                            ])
                        ) {
                            (<Mutable<this>>this).nonConstCopyConstructor =
                                ctorEntity;
                        }
                    }
                }
        ...
        this.context.program.registerClassDefinition(
            this.declaration.declaredEntity.qualifiedName,
            this
        );
    }
    ...
    private createImplicitlyDefinedDefaultConstructorIfAppropriate() {
        ...
        // default constructor.
        if (
            this.constructors.some(
                (ctor) => !ctor.firstDeclaration.context.implicit
            )
        ) {
            return;
        }
        ...
        if (
            !subobjectTypes.every(
                (t) => t.isDefaultConstructible() && t.isDestructible()
            )
        ) {
            return;
        }
        ...
        if (
            this.memberObjectEntities.some(
                (memObj) =>
                    memObj.type.isConst &&
                    !memObj.type.isDefaultConstructible(true)
            )
        ) {
            return;
        }
    ...
    private createImplicitlyDefinedCopyConstructorIfAppropriate() {
        ...
        let src = `//@className=${this.name}\n${this.name}(${
            refParamCanBeConst ? "const " : ""
        }${this.name} &other)`;
        let memInits: string[] = this.memberVariableEntities.map(
            (mem) => `${mem.name}(other.${mem.name})`
        );
        if (this.baseType) {
    ...
    public lookupAssignmentOperator(
        requireConstParam: boolean,
        isReceiverConst: boolean
    ) {
    ...
    private createImplicitlyDefinedCopyAssignmentOperatorIfAppropriate() {
        ...
        src += this.memberObjectEntities
            .map((mem) =>
                mem.isTyped(isBoundedArrayType)
    ...
    public getBaseAndMemberEntities() {
        return this.baseType
            ? [
                  new BaseSubobjectEntity(
                      new ReceiverEntity(this.type),
                      this.baseType
                  ),
...
export interface TypedClassDefinition<T extends CompleteClassType>
...
export interface CompiledClassDefinition<
    T extends CompleteClassType = CompleteClassType
> extends TypedClassDefinition<T>,
        SuccessfullyCompiled {
...
export class BaseSpecifier extends BasicCPPConstruct<
    TranslationUnitContext,
    BaseSpecifierASTNode
> {
    public isSemanticallyEquivalent_impl(
    ...
    public constructor(
        context: TranslationUnitContext,
        ast: BaseSpecifierASTNode,
        defaultAccessLevel: AccessSpecifier
    ) {
        super(context, ast);
...
export interface CompiledBaseSpecifier
...
export class MemberVariableDeclaration extends VariableDefinitionBase<MemberSpecificationContext> {
    ...
    public constructor(
        ...
        declarator: Declarator,
        otherSpecs: OtherSpecifiers,
        type: CompleteObjectType | ReferenceType
    ) {
        super(context, ast, typeSpec, storageSpec, declarator, otherSpecs);
        ...
        this.declaredEntity = type.isReferenceType()
            ? new MemberReferenceEntity(type, this)
            : new MemberObjectEntity(type, this);
        ...
        if (
            entityOrError instanceof MemberObjectEntity ||
            entityOrError instanceof MemberReferenceEntity
        ) {
            this.declaredEntity = entityOrError;
...
export interface TypedMemberVariableDeclaration<T extends ObjectEntityType>
    extends MemberVariableDeclaration {
    readonly type: T;
    readonly declarator: TypedDeclarator<T>;
    readonly declaredEntity:
        | MemberObjectEntity<Exclude<T, ReferenceType>>
        | MemberReferenceEntity<Extract<T, ReferenceType>>;
}

export interface CompiledMemberVariableDeclaration<
    T extends ObjectEntityType = ObjectEntityType
> extends TypedMemberVariableDeclaration<T>,
        SuccessfullyCompiled {
    readonly typeSpecifier: CompiledTypeSpecifier;
    readonly storageSpecifier: CompiledStorageSpecifier;
    readonly declarator: CompiledDeclarator<T>;
...
export class IncompleteTypeMemberVariableDeclaration extends SimpleDeclaration<TranslationUnitContext> {
    public readonly construct_type =
        "incomplete_type_member_variable_declaration";
    ...
    public constructor(
        ...
        this.addNote(
            CPPError.declaration.member.incomplete_type_declaration_prohibited(
                this
            )
        );
    }
...
export interface TypedIncompleteTypeMemberVariableDeclaration<
    T extends IncompleteObjectType
> extends IncompleteTypeMemberVariableDeclaration {
    readonly type: T;
    readonly declarator: TypedDeclarator<T>;
}
