...
export class StorageSpecifier extends BasicCPPConstruct<TranslationUnitContext, ASTNode> {
  ...
  public constructor(context: TranslationUnitContext, specs: readonly StorageSpecifierKey[]) {
    ...
    if (numSpecs < 2 || (numSpecs == 2 && this.thread_local && (this.static || this.extern))) {
      //ok
    } else {
...
const TopLevelDeclarationConstructsMap = {
  simple_declaration: (ast: NonMemberSimpleDeclarationASTNode, context: TranslationUnitContext) =>
  ...
  class_definition: (ast: ClassDefinitionASTNode, context: TranslationUnitContext) =>
    ClassDefinition.createFromAST(ast, context),
};

export function createTopLevelDeclarationFromAST<ASTType extends TopLevelDeclarationASTNode>(
  ast: ASTType,
  context: TranslationUnitContext
): ReturnType<typeof TopLevelDeclarationConstructsMap[ASTType['construct_type']]> {
...
export function setInitializerFromAST(
  ...
  initAST: DirectInitializerASTNode | CopyInitializerASTNode | ListInitializerASTNode | undefined,
  context: TranslationUnitContext
) {
  if (!initAST) {
...
const LocalDeclarationConstructsMap = {
  simple_declaration: (ast: NonMemberSimpleDeclarationASTNode, context: BlockContext) =>
    createLocalSimpleDeclarationFromAST(ast, context),
  function_definition: (ast: FunctionDefinitionASTNode, context: BlockContext) =>
    FunctionDefinition.createFromAST(ast, context),
  class_definition: (ast: ClassDefinitionASTNode, context: BlockContext) =>
    ClassDefinition.createFromAST(ast, context),
};

export function createLocalDeclarationFromAST<ASTType extends LocalDeclarationASTNode>(
  ast: ASTType,
...
const MemberDeclarationConstructsMap = {
  ...
  function_definition: (ast: FunctionDefinitionASTNode, context: MemberSpecificationContext) =>
    createFunctionDeclarationFromDefinitionAST(ast, context),
  // Note: function_definition includes ctor and dtor definitions
};

export function createMemberDeclarationFromAST<ASTType extends MemberDeclarationASTNode>(
  ast: ASTType,
  context: MemberSpecificationContext
): ReturnType<typeof MemberDeclarationConstructsMap[ASTType['construct_type']]> {
  ...
  if (baseType?.sameType(context.containingClass.type)) {
    ...
    if (!testDeclarator.name) {
      typeSpec = TypeSpecifier.createFromAST(
        ast.specs.typeSpecs.filter(spec => spec !== context.containingClass.name),
        context
      );
    }
  }
...
export interface TypedUnknownBoundArrayDeclaration<T extends ArrayOfUnknownBoundType>
...
export interface CompiledFunctionDeclaration<T extends FunctionType = FunctionType>
...
> extends SimpleDeclaration<ContextType> {
  ...
  public setDirectInitializer(args: readonly Expression[]) {
    return this.setInitializer(
      DirectInitializer.create(this.context, this.declaredEntity, args, 'direct')
    );
  }

  public setCopyInitializer(args: readonly Expression[]) {
    return this.setInitializer(
      DirectInitializer.create(this.context, this.declaredEntity, args, 'copy')
    );
  }
...
> extends TypedLocalVariableDefinition<T>, SuccessfullyCompiled {
  readonly typeSpecifier: CompiledTypeSpecifier;
  readonly storageSpecifier: CompiledStorageSpecifier;
  readonly declarator: CompiledDeclarator<T>;
...
export interface CompiledGlobalVariableDefinition<
  T extends VariableDefinitionType = VariableDefinitionType
> extends TypedGlobalVariableDefinition<T>, SuccessfullyCompiled {
  readonly typeSpecifier: CompiledTypeSpecifier;
  readonly storageSpecifier: CompiledStorageSpecifier;
  readonly declarator: CompiledDeclarator<T>;
...
export interface CompiledParameterDeclaration<
  T extends PotentialParameterType = PotentialParameterType
> extends TypedParameterDeclaration<T>, SuccessfullyCompiled {
  readonly typeSpecifier: CompiledTypeSpecifier;
  readonly storageSpecifier: CompiledStorageSpecifier;
  readonly declarator: CompiledDeclarator<T>;
}
...
export interface TypedParameterDefinition<T extends CompleteParameterType = CompleteParameterType>
  extends ParameterDeclaration {
  readonly type: T;
  readonly declarator: TypedDeclarator<T>;
  readonly declaredEntity:
...
> extends TypedParameterDefinition<T>, SuccessfullyCompiled {
  readonly typeSpecifier: CompiledTypeSpecifier;
  readonly storageSpecifier: CompiledStorageSpecifier;
  readonly declarator: CompiledDeclarator<T>;
}
...
export class IncompleteTypeVariableDefinition extends SimpleDeclaration<TranslationUnitContext> {
  ...
  public isSemanticallyEquivalent_impl(
    ...
    return other.construct_type === this.construct_type && sameType(this.type, other.type);
  }
}

export interface TypedIncompleteTypeVariableDefinition<T extends IncompleteObjectType>
  ...
  public static createFromAST(
    ...
    declaration.parameterDeclarations.forEach(paramDecl => {
      if (paramDecl.isPotentialParameterDefinition()) {
        ...
        paramDecl.addNote(
          CPPError.lobster.unsupported_feature(paramDecl, 'Unnamed parameter definitions.')
        );
      }
    });
    ...
    let body = new Block(
      bodyContext,
      ast.body,
      ast.body.statements.map(s => createStatementFromAST(s, bodyContext))
    );
    ...
    if (this.declaration.isDestructor) {
      ...
      this.attach(
        (this.memberDeallocator = createMemberDeallocator(
          context,
          new ReceiverEntity(<CompleteClassType>this.type.receiverType)
        ))
      );
    }
...
export interface CompiledFunctionDefinition<T extends FunctionType = FunctionType>
...
> extends TypedClassDeclaration<T>, SuccessfullyCompiled {}
...
export class ClassDefinition extends BasicCPPConstruct<ClassContext, ClassDefinitionASTNode> {
  ...
  public static createFromAST(ast: ClassDefinitionASTNode, tuContext: TranslationUnitContext) {
    ...
    functionDefsToCompile.forEach(([defAST, memberSpecContext, decl]) => {
      classDef.attachInlineFunctionDefinition(
        FunctionDefinition.createFromAST(defAST, memberSpecContext, decl)
      );
    });
          ...
          if (ctorEntity.returnsVoid()) {
            ...
            if (ctorEntity.type.paramTypes.length === 0) {
              ...
              ctorEntity.type.sameParamTypes([
                new ReferenceType(this.declaration.type.cvQualified(true)),
              ])
            ) {
              (<Mutable<this>>this).constCopyConstructor = ctorEntity;
            } else if (
              ctorEntity.type.sameParamTypes([
                new ReferenceType(this.declaration.type.cvUnqualified()),
              ])
            ) {
  ...
  private createImplicitlyDefinedDefaultConstructorIfAppropriate() {
    ...
    if (
      this.memberObjectEntities.some(
        memObj => memObj.type.isConst && !memObj.type.isDefaultConstructible(true)
      )
    ) {
      return;
    }
    ...
    let iddc = <FunctionDefinition>(
      FunctionDefinition.createFromAST(parseFunctionDefinition(src), this.implicitPublicContext)
    );
    this.attach(iddc);
  ...
  private createImplicitlyDefinedCopyConstructorIfAppropriate() {
    ...
    let src = `//@className=${this.name}\n${this.name}(${refParamCanBeConst ? 'const ' : ''}${
      this.name
    } &other)`;
  ...
  public getBaseAndMemberEntities() {
    return this.baseType
      ? [
          new BaseSubobjectEntity(new ReceiverEntity(this.type), this.baseType),
...
export interface CompiledClassDefinition<T extends CompleteClassType = CompleteClassType>
...
export interface CompiledMemberVariableDeclaration<T extends ObjectEntityType = ObjectEntityType>
 ...
 */
export class IncompleteTypeMemberVariableDeclaration extends SimpleDeclaration<
  TranslationUnitContext
> {
...
export interface TypedIncompleteTypeMemberVariableDeclaration<T extends IncompleteObjectType>
