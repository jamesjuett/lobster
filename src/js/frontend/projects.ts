...
export function parseProjectContents(projectData: ProjectData): {
    name: string;
    files: FileData[];
} {
...
type ProjectListMessages = "projectSelected" | "createProjectClicked";
...
export class ProjectList {
    ...
    public constructor(element: JQuery) {
        ...
        this.listElem = $(
            '<div class="list-group lobster-project-list"></div>'
        ).appendTo(element);
    }
    ...
    public setProjects(projects: readonly ProjectData[]) {
        ...
        $(
            `<a class="list-group-item" style="text-align: center">${icon_middle(
                ICON_PLUS
            )}</a>`
        )
            .appendTo(this.listElem)
    ...
    public setActiveProject(projectId: number | undefined) {
        if (this.activeProjectId) {
            this.listElem.children().removeClass("active");
        }

        (<Mutable<this>>this).activeProjectId = projectId;

        if (this.activeProjectId) {
            let activeIndex = this.projects.findIndex(
                (p) => p.id === projectId
            );
            if (activeIndex !== -1) {
                this.listElem.children().eq(activeIndex).addClass("active");
            }
        }
    }
    ...
    public editProject(projectId: number, data: Partial<ProjectData>) {
        let projectsCopy = this.projects.map((p) =>
            p.id === projectId ? Object.assign({}, p, data) : p
        );
...
export async function editProject(
    projectData: Partial<ProjectData> & { id: number }
) {
    return axios({
